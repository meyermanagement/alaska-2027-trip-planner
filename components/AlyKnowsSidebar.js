...
export default function AlyKnowsSidebar({
  ...
  const facts = useMemo(() => {
    ...
    if (address && address.trim().length > 0) {
      const state = detectStateFromAddress(address) || detectStateFromAddress(located?.address || "");
  ...
  return (
    <aside className="rounded-2xl border border-sand-deep bg-sand-soft/60 p-4 lg:sticky lg:top-4">
      ...
      <p className="mt-1 text-xs leading-relaxed text-ink-soft">
        Every field you type appears here as something Aly is already doing
        with it. Nothing here needs a save -- these are her notes as you go.
      </p>
      <div className="mt-3 space-y-2">
        {facts.length === 0 ? (
          <p className="text-sm italic text-ink-faint">
            As soon as you type a name or a home, Aly will start taking
            notes here.
          </p>
        ) : (
          facts.map((f) => (
