...
export default function PackIndex({ groups, picked, onPick }) {
  ...
  return (
    <>
      {/* Phone: one scrolling strip, in the same order as the column. */}
      <div className="no-print -mx-5 mb-4 flex gap-2 overflow-x-auto px-5 pb-1 lg:hidden">
        {rows.map((r) => {
          ...
          return (
            <button
              ...
              className={`flex shrink-0 items-center gap-1.5 rounded-full border px-3 py-1.5 text-sm transition ${
                on
                  ? "border-teal bg-teal-soft/60 font-semibold"
            ...
            >
              ...
              <span className="text-xs tabular-nums opacity-70">{r.count}</span>
            </button>
          );
        })}
      </div>
