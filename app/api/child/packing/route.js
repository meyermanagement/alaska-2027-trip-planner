...
export async function POST(request) {
  ...
  if (!validPacking(body)) return NextResponse.json({ error: "Invalid packing change." }, { status: 400, headers: privateHeaders });
  try {
    ...
    const { data, error } = await ctx.admin.rpc("set_child_packing", { view_hash: ctx.hash, item_id: body.itemId, packed: body.packed });
