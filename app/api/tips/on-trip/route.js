...
export async function POST(request) {
  ...
  try {
    const { data: rows, error } = await client.from("itinerary_items")
      .select("id,item_date,end_date,start_time,title,category,location,status,is_done")
      .eq("trip_id", trip.id).lte("item_date", window.through)
      .or(`item_date.gte.${window.today},end_date.gte.${window.today}`)
      .neq("status", "cancelled").order("item_date").order("start_time");
    if (error) throw new Error("Today's plans could not be read.");
    const items = (rows || []).filter(i => !i.is_done);
    ...
    const [{ data: currentTrip, error: currentTripError }, { data: currentItems, error: currentItemsError }] = await Promise.all([
      ...
      client.from("itinerary_items").select("id,item_date,end_date,start_time,title,category,location,status,is_done")
    ...
    const fields = ["item_date", "end_date", "start_time", "title", "category", "location", "status", "is_done"];
