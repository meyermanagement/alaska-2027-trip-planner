...
export function historyPeriod(value, today, { shortHistory = false } = {}) {
  ...
  if (shortHistory && monthsAgo <= 3) return { key: `month-${year}-${month}`, label: `${monthsAgo} months ago`, rank: monthsAgo, day };
  if (shortHistory) return { key: "older", label: "Older history", rank: 4, day };
...
export function groupHistory(items, getDate, today, options) {
  const groups = new Map();
  for (const item of items) {
    const period = historyPeriod(getDate(item), today, options);
