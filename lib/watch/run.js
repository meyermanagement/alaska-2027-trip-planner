...
//
// With one exception, learned the hard way. A claim is worth keeping only when
// somebody was actually told. If every phone in the household refuses the
// notification, the warning has not been given: the run falls back to email, and
// if that cannot go either the claim is handed back so the next run says it
// again. A deadline is a date, and a warning that was recorded but never arrived
// is worse than one that arrives twice.
...
import { retireFares } from "@/lib/deals/deadline";
import { retireOffers } from "@/lib/rewards-offers";
 ...
 * @param {string} input.familyId   the household to watch; required
...
export async function runDeadlineWatch({
  ...
  try {
    // Named, never guessed. This used to fall back to the first row of `families`
    // when no household was given, which is correct only while there is one
    // household and wrong from the moment there are two: the scheduled pass then
    // watched whichever row came back first and left every other household
    // unwatched, and filed its run under the household it had picked. Asking for
    // a watch without saying whose is now a refusal that says so, and the
    // scheduler's job is to name each household in turn.
    const family = familyId || null;
    if (!family) {
      outcome.error =
        "The deadline watch was asked to run without naming a household, so it did nothing. Call it once per household.";
      outcome.ok = false;
      return outcome;
    }
    ...
    const [{ data: deals }, { data: offers }] = await Promise.all([
      supabase
        .from("flight_deals")
        .select(
          "id, family_id, origin, destination, destination_code, price, award_pricing, currency, cabin, airline, book_by, book_by_inferred, deadline_said, travel_start, travel_end, travel_months, seats, source_name, status",
        )
        .eq("family_id", family)
        .eq("status", "open"),
      supabase
    ...
    const { alerts, expired, expiredOffers } = deadlinesInView({
      deals: deals || [],
      offers: offers || [],
      today,
      siteUrl,
    ...
    if (expired.length && !dryRun) {
      const { data: retired, error } = await retireFares(supabase, family, expired, today);
      if (!error) outcome.expired = retired?.length || 0;
      else throw error;
    } else if (expired.length) {
      outcome.expired = expired.length;
    }

    // And the same for a card offer whose end date has gone by, for the same
    // reason: the sentence that argued for it goes away with it, so the app stops
    // recommending a bonus that can no longer be claimed.
    if (expiredOffers.length && !dryRun) {
      ...
      outcome.expiredOffers = retired.length;
    } else if (expiredOffers.length) {
      outcome.expiredOffers = expiredOffers.length;
    }
    ...
    // email if none has. Not both for the same warning: two copies is how a
    // useful channel becomes one people mute. Email still covers a warning the
    // phones would not take, which is the one case where the second copy is the
    // only copy.
    ...
    // Who could be told, worked out before anything is claimed. The claim is what
    // stops the same warning going out twice, and a warning nobody was allowed to
    // receive has not been given -- claiming it anyway means the tester who turns
    // reminders back on never hears about this deadline at all.
    // Worked out even when push is the chosen channel, because it is also the
    // answer to "the phones refused it, who else could be told", and that
    // question arrives after the claim is already written.
    const recipients = await whoCanBeEmailed({ supabase, family });
    if (!usePush && !recipients.length) {
      outcome.failed.push({
        error:
      ...
      // Deliberately nothing claimed. The deadline stays unsaid, so it is still
      // there to say if the permission comes back before the date does.
      return outcome;
    }

    // Claimed before delivery, on purpose. See the note at the top of the file.
    const claimed = [];
    // Which row holds which claim, so a warning nothing accepted can be given
    // back rather than left sitting in the ledger as one that was sent.
    ...
    for (const alert of fresh) {
      ...
      if (!error) {
        claimed.push(alert);
        if (row?.id) claimIds.set(alert, row.id);
      }
    }
    ...
    if (usePush) {
      ...
      await emailAll({ to: recipients, alerts: claimed, siteUrl, outcome });
    }
...
/**
 * The scheduled pass: every household, one at a time, one record each.
 *
 * Households are listed and named rather than left to a first row, which is the
 * whole correction here. Each household is an independent pass: one that throws
 * or fails is written down as having failed and the next one still runs, because
 * a fare deadline in one household is nothing to do with a broken row in
 * another.
 *
 * Sequential on purpose. The pass sends email and pushes phones, and the run has
 * sixty seconds; a burst of parallel sends against a mail transport is how a
 * whole evening's warnings get refused at once. If the number of households ever
 * outgrows the window, the fix is a queue rather than concurrency here.
 *
 * @returns {Promise<{ok: boolean, today: string, households: object[], error: string|null}>}
 */
export async function runDeadlineWatchForAll({
  supabase,
  siteUrl,
  today = homeToday(),
  dryRun = false,
  ...
  if (error) {
    return {
      ...
      today,
      households: [],
      error: `The households could not be read, so nothing was watched: ${error.message}`,
    };
  }
  ...
  for (const familyId of ids) {
    try {
      households.push(
        await runDeadlineWatch({ supabase, siteUrl, today, familyId, dryRun }),
      );
    } catch (err) {
      households.push({
        ok: false,
        ...
        expiredOffers: 0,
        channel: null,
        error: String(err?.message || err) || "The watch failed.",
      });
    }
  }
  return {
    ok: households.every((h) => h.ok !== false),
    today,
    households,
    error: null,
  };
...
async function pushAll({ supabase, subs, alerts, outcome }) {
  ...
  for (const alert of alerts) {
    ...
    if (delivered > 0) {
      ...
      // Not recorded as failed here. Whether this warning was given depends on
      // what the fallback manages next, and the caller is what knows that.
      refused.push(alert);
    }
  }
  return refused;
...
/**
 * The warning the phones would not take.
 *
 * Every browser refusing is not a rare event -- a subscription outlives the
 * browser profile that made it -- and the claim written before the attempt is
 * what stops the next run trying again. So either somebody is emailed instead
 * and the ledger is corrected to say so, or the claim is deleted and the
 * deadline goes back to being one nobody has been warned about.
 */
async function coverForRefusedPush({
  supabase,
  refused,
  claimIds,
  recipients,
  ...
  if (recipients.length) {
    const before = outcome.sent.length;
    await emailAll({ to: recipients, alerts: refused, siteUrl, outcome });
    if (outcome.sent.length > before) {
      outcome.channel = "email";
      for (const alert of refused) {
        const id = claimIds.get(alert);
        if (id) {
          await supabase
            .from("deadline_alerts")
    ...
    for (const alert of refused) {
      outcome.failed.push({
        title: alert.title,
        error:
          "No browser accepted the notification, and nobody in the household can be emailed instead.",
      });
    }
  }

  // Nothing took it. Hand the claim back so the deadline is still there to say.
  for (const alert of refused) {
    const id = claimIds.get(alert);
    if (id) await supabase.from("deadline_alerts").delete().eq("id", id);
  }
}
 ...
 * they want reminders.
 */
async function emailAll({ to, alerts, siteUrl, outcome }) {
...
/**
 * Household members who can be emailed a deadline warning: an address, reminders
 * wanted on their traveler row, and the reminders permission still on for whoever
 * speaks for them.
 */
async function whoCanBeEmailed({ supabase, family }) {
  const { data: people } = await supabase
    .from("travelers")
    .select("id, name, email, is_person, wants_reminders, user_id, date_of_birth")
    .eq("family_id", family);
...
export function watchRecord({ outcome, source = "cron" }) {
  return {
    ...
    detail: {
      ...
      // The `expired` column counts fares, which is what it has always counted
      // and what the existing rows mean. Offers retired by the same pass are
      // recorded here rather than added into it, so no past row changes meaning.
      expiredOffers: Number(outcome?.expiredOffers || 0),
    },
  };
}
...
async function whoStillAllowsEmail(supabase, people, family) {
  ...
  for (const person of people) {
    if (isMinorTraveler(person)) continue;
    if (person.user_id) {
      if (on.has(person.user_id)) allowed.add(person.id);
      continue;
    }
    if (owners.length && owners.every((id) => on.has(id)))
