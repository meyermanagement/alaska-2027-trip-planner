  ...
  try {
    ...
    const [{ data: deals }, { data: offers }] = await Promise.all([
      supabase
        .from("flight_deals")
        .select(
          "id, family_id, origin, destination, destination_code, price, currency, cabin, airline, book_by, book_by_inferred, deadline_said, travel_start, travel_end, travel_months, seats, source_name, status",
        )
        .eq("family_id", family)
        .eq("status", "open"),
      supabase
