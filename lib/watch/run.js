...
export async function runDeadlineWatch({
  ...
  try {
    ...
    const { data: subs } = await supabase
      ...
      .select("id, endpoint, p256dh, auth, traveler_id")
      .eq("family_id", family)
      .eq("enabled", true);
...
async function emailAll({ supabase, family, alerts, siteUrl, outcome }) {
  const { data: people } = await supabase
    .from("travelers")
    .select("id, name, email, is_person, wants_reminders")
    .eq("family_id", family);

  const to = (people || []).filter(
    (p) => p && p.is_person !== false && p.wants_reminders && p.email,
  );
  if (!to.length) {
    outcome.failed.push({
      error:
        "Nothing could be sent: no browser has notifications turned on and nobody in the household has an email address with reminders switched on.",
    });
    return;
  }
