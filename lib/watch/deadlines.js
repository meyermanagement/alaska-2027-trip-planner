 ...
 * @returns {{alerts: Array, expired: Array, expiredOffers: Array}}
 ...
 *   expiredOffers: offer ids whose end date has passed and are still marked open
 */
export function deadlinesInView({
  ...
  for (const deal of deals || []) {
    ...
    if (left < 0) {
      ...
      if (!fareHasExpired(deal, today)) continue;
    ...
    alerts.push({
      ...
      path: "/someday#fares",
      url: link(siteUrl, "/someday#fares"),
    });
  }
  ...
  for (const offer of offers || []) {
    ...
    if (left === null) continue;
    if (left < 0) {
      // Same treatment as a fare, and safer: the end date is the issuer's own,
      // printed on the terms, not a number worked out from a sender's guess. So
      // there is no inferred case to hold back for. Nobody is told; the offer is
      // retired so Current offers stops presenting a bonus nobody can claim.
  ...
  return { alerts, expired, expiredOffers };
}
...
function fareTitle(deal, left) {
  ...
  if (deal.book_by_inferred)
    return `${farePriceLabel(deal)} to ${where} may be running out`;
  if (left === 0) return `Last day for ${farePriceLabel(deal)} to ${where}`;
  return `${countdown(left)} on ${farePriceLabel(deal)} to ${where}`;
}
