...
export const TRAVEL_STYLES = [
  {
    id: "cabin",
    label: "In the cabin",
    hint: "A travel preference, not confirmed eligibility. Check the airline's animal and carrier requirements.",
  },
  {
    id: "cargo",
    label: "As cargo",
    hint: "Confirm the airline's live-animal transport options before booking.",
  },
  {
    id: "car_only",
    label: "By car only",
  ...
  },
  {
    id: "trailer",
    label: "By trailer",
    hint: "Plan suitable transport and care at the destination.",
  },
  {
    id: "stays_home",
...
/**
 * The same five arrangements, named for the animal in front of you.
 *
 * "Boarding" is the only one that changes: a dog boards at a kennel, a horse
 * boards at a barn, and a rabbit goes to whoever will take it. Saying so matters
 * because the word is what the family will go looking for when they book it, and
 * a horse owner reading "Boarding" on a form written for dogs has to translate.
 */
export function arrangementsFor(species) {
  const word = speciesProfile(species).careWord;
  if (word === "boarding kennel") return ARRANGEMENTS;
  const label =
    word === "boarding barn" ? "Boarding at a barn" : "Boarding or a sitter";
  return ARRANGEMENTS.map((a) => (a.id === "boarding" ? { ...a, label } : a));
}

export function arrangementLabel(id, species) {
  const list = species ? arrangementsFor(species) : ARRANGEMENTS;
  return list.find((a) => a.id === id)?.label || "Not decided yet";
}
...
/** Planning context only. A saved weight is not an airline eligibility rule. */
export function cabinOutlook(pet) {
  ...
  if (pet?.is_service_animal)
    return {
      key: "service",
      text: "Recorded as a trained service animal. Confirm eligibility, documentation, and travel requirements with the carrier and destination.",
    };
  const lb = Number(pet?.weight_lb);
  if (!Number.isFinite(lb) || lb <= 0)
    return {
      key: "unknown",
      text: "Record the animal's weight and carrier details to help check travel requirements.",
    };
  return {
    key: "check",
    text: `Recorded weight: ${trimNumber(lb)} lb, without the carrier. Weight alone does not confirm cabin eligibility; check the airline's requirements before booking.`,
  };
}
