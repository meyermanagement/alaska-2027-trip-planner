...
import { packingWaitsForDraft } from "../packing/draft";
...
export async function syncPackingForPet({
  ...
  const [{ data: tripItems, error: readError }, { data: trip }] =
    await Promise.all([
      supabase
        .from("packing_items")
  ...
  // Putting an animal on a draft must not start a packing list for it -- see
  // lib/packing/draft.js. The hasList check below already covers the ordinary
  // case of an empty draft, but a draft that somehow has lines on it would
  // otherwise get the dog's whole template the moment somebody tapped its name
  // on the trip builder. Taking an animal off is still honored: lines for an
  // animal that is not going should not sit on any list, draft or not.
  const waits = packingWaitsForDraft(trip);
  ...
  if (waits)
    return {
      message: `${name} is on the trip. Nothing was added to a packing list — it is still a draft, so there is no list to add to yet.`,
      added: 0,
      removed: 0,
      restored,
    };
