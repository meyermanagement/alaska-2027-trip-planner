...
export function readRoute(raw, live) {
  ...
  //
  // The obvious version of this check does not work. Number("") is 0, not NaN, so
  // a route that came back without a duration at all passed the finite test, was
  // floored to one minute, and reported a journey time we had never been given --
  // the same coercion trap as Number(null). The string has to be recognized before
  // it is converted.
  const said = String(route.duration ?? "").trim();
  if (!/^\d+(\.\d+)?s$/.test(said)) return { error: "no-duration" };
  const seconds = Number(said.slice(0, -1));
...
export function travelSaid(t) {
  ...
  const label = `${durationSaid(t.minutes)} ${how}`;
