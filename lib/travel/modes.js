...
import { durationSaid } from "@/lib/format";
...
/** "12 min", "8-14 min", "5 hr 15 min", or null. */
export function minutesSaid(option) {
  if (Number.isFinite(option?.minutes)) return durationSaid(option.minutes);
  if (option?.range) {
    // The unit is written once when both ends are minutes, because
    // "8 min-14 min" is a stutter. Once either end is over an hour they each
    // need their own units or the range is unreadable.
    const { low, high } = option.range;
    return high < 60
      ? `${low}\u2013${high} min`
      : `${durationSaid(low)}\u2013${durationSaid(high)}`;
  }
  return null;
}
...
/**
 * Directions to a place on the itinerary, from wherever the phone is standing.
 *
 * Same argument as directionsUrl above and worth repeating: no origin is sent.
 * The route starts where the person is, which their phone knows to the metre and
 * this app knows at best from an address typed in months ago.
 *
 * Coordinates when the place has been geocoded, because handing Maps a hotel
 * name is how a family in Iowa gets routed to the same chain in Nevada. The
 * address is the next best thing, and the name is the last resort.
 *
 * @returns string|null  null when there is nothing to route to
 */
export function directionsToPlace(place) {
  ...
  if (Number.isFinite(lat) && Number.isFinite(lon)) {
    ...
    const named = [place.location, place.address, place.title, place.name]
      .map((v) => String(v || "").trim())
      .find(Boolean);
    if (!named) return null;
    params.set("destination", named);
  }
  return `https://www.google.com/maps/dir/?${params.toString()}`;
}
