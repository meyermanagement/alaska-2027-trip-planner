 ...
 *
 * Origin is deliberately optional, and omitting it is not a shortcut. On the
 * first leg of a day the time was measured from wherever the family last said
 * they were, which may be hours old; Maps with no origin uses the phone's live
 * position, which is a better answer than the one we had. Between two items the
 * origin is a fact -- the place they are leaving -- so it is passed.
 *
 * @returns string|null  null when there is no destination or no such mode
 */
export function directionsUrl({ to, from = null, mode } = {}) {
