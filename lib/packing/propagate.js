...
/**
 * Which templates each trip counts as using, and which trips each template
 * reaches.
 *
 * Pulled out of the planner below so that anything else asking "what does this
 * template affect" gets the same answer the push button acts on. The templates
 * screen links each list to the trips it would reach, and a screen that said
 * something different from what the button then did would be worse than no link
 * at all.
 *
 * The rule, in one place: the base list applies to every trip, always. An add-on
 * applies to a trip that has said it uses it. A trip that has never been asked is
 * inferred from what it is already carrying, which is what the app did before
 * trips could answer the question.
 *
 * @returns {{ byTrip: Map<string, string[]>, byTemplate: Map<string, string[]> }}
 *          template ids per trip, and trip ids per template, both in the order
...
export function templateScope({
  ...
  for (const t of templates) keysFor.set(t.id, new Set());
  for (const row of templateItems) {
    const k = itemKey(row);
    if (!k) continue;
    keysFor.get(row.template_id)?.add(k);
  }

  const mineByTrip = new Map();
  for (const row of tripItems) {
    if (!mineByTrip.has(row.trip_id)) mineByTrip.set(row.trip_id, new Set());
    const k = itemKey(row);
    if (k) mineByTrip.get(row.trip_id).add(k);
  }

  const byTrip = new Map();
  const byTemplate = new Map();
  for (const t of templates) byTemplate.set(t.id, []);

  for (const trip of trips) {
    const mine = mineByTrip.get(trip.id) || new Set();
    ...
    for (const t of templates) {
      ...
      const keys = keysFor.get(t.id) || new Set();
      ...
      if (chosen) {
        ...
        // inference got wrong.
        if (chosen.has(t.id)) ids.push(t.id);
        continue;
      }
      let hits = 0;
      for (const k of keys) if (mine.has(k)) hits += 1;
      if (hits >= ADDON_MIN && hits / size >= ADDON_SHARE) ids.push(t.id);
    }
    byTrip.set(trip.id, ids);
    for (const id of ids) byTemplate.get(id)?.push(trip.id);
  }

  return { byTrip, byTemplate };
}
...
export function planPropagation({
  ...
  for (const trip of trips) {
    ...
    // Which templates this trip counts as using, decided by the one function
    // above so that the templates screen's links and this plan cannot disagree.
    const inScope = (scope.byTrip.get(trip.id) || [])
