...
import { packingWaitsForDraft } from "./draft";
...
export async function syncPackingForTraveler({
  ...
  const [{ data: tripItems }, { data: trip }] = await Promise.all([
    supabase
      .from("packing_items")
      .select(
  ...
  // Putting somebody on a draft must not start a packing list for it. A draft is
  // still being worked out and nothing packs for one -- see lib/packing/draft.js
  // -- so the roster tap is recorded and the packing side of it waits. Taking
  // somebody off is still honored below, because a draft that somehow has lines
  // on it should not keep lines for a person who is not going.
  const draft = packingWaitsForDraft(trip);
  ...
  if (draft) {
    return {
      message: `Put ${name} on the trip. Nothing was added to a packing list — it is still a draft, so there is no list to add to yet.`,
      added: 0,
      removed: 0,
      restored,
      kept: [],
