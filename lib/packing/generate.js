...
export async function buildPackingList({
  ...
  const [tplRes, pastRes, prefRes, peopleRes, factsRes, itinRes, heldRes] =
    await Promise.all([
      supabase
  ...
  const { data: templateItems } = templateId
    ? await supabase
        ...
        .select("category, item, assignee, quantity, sort_order, last_minute")
  ...
  const templateKeys = new Set(
    (templateItems || []).map((row) => itemKey(row)).filter(Boolean),
  );

  const chosen = applyPackingFloor({
    items: generated.length ? generated : templateItems || [],
    facts,
    itinerary,
    going,
  });
  ...
  const rows = chosen.items.map((item, i) => ({
    ...
    quantity: item.quantity || null,
    last_minute: lastMinuteFor(item),
    from_template: templateKeys.has(itemKey(item)),
    sort_order: i + 1,
  }));
