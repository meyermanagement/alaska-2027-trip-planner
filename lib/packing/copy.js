...
/**
 * @param source rows with category, item, assignee, quantity, last_minute
...
export function copiedTemplateItems(
  ...
  for (const row of Array.isArray(source) ? source : []) {
    ...
    items.push({
      ...
      // Copied like everything else about the row. A list split out of another
      // one should not quietly forget which of its things cannot go in a bag.
