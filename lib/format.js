...
/** The chip for a task's priority, or null when there is nothing worth saying. */
export function priorityChip(task) {
  const priority = priorityOf(task);
  if (priority === "high") {
    return { label: "High priority", cls: "bg-rose/12 text-rose" };
  }
  if (priority === "low") {
    return { label: "Low priority", cls: "bg-sand-deep text-ink-soft" };
  }
  return null;
}
