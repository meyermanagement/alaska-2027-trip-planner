...
// Every task says where it stands, so the list can be read at a glance without
// opening anything. The three levels differ by fill as well as colour — solid,
// flat and outlined — so they are still told apart in print or by anyone who
// does not see the difference between the reds and the greys.
const PRIORITY_CHIPS = {
  high: {
    label: "High priority",
    cls: "bg-rose/15 text-rose ring-1 ring-inset ring-rose/30",
    mark: "\u25B2",
  },
  normal: {
    label: "Normal priority",
    cls: "bg-sand-deep text-ink-soft",
    mark: "\u25CF",
  },
  low: {
    label: "Low priority",
    cls: "bg-white text-ink-soft ring-1 ring-inset ring-[var(--line-strong)]",
    mark: "\u25BC",
  },
};

/** The chip for a task's priority. Every task has one. */
export function priorityChip(task) {
  return PRIORITY_CHIPS[priorityOf(task)];
}
