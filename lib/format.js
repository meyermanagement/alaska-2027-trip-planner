...
const PRIORITY_METERS = {
  ...
  // Normal and low were gray, on the reasoning that only the out-of-the-ordinary
  // should draw the eye. That reads as broken rather than as quiet the moment the
  // three of them sit side by side as filter chips: one lit in red and two in the
  // color of a disabled control, which is exactly what a filter looks like when
  // it does not apply. They are teal now -- the app's own color, the one it uses
  // for the ordinary and the working -- so the meters are three levels of
  // something rather than one level and two absences. Red stays reserved for the
  // level that is meant to jump, and a list of forty normal tasks stays calm
  // because teal at these weights is quieter than the words beside it.
  normal: {
...
/**
 * How many days until a date, counted from today where the family lives.
 *
 * The home zone rather than the machine's own clock, because this number is
 * rendered as words -- "3 days away" on the trip screen and in the tab bar -- and
 * a page is drawn twice: once on the server and again in the browser. The server
 * runs on UTC, so from seven in the evening in Missouri onwards it had already
 * turned tomorrow while the phone had not. The two renders disagreed by a day,
 * React found different text in the same place, and the trip screen threw a
 * hydration error every time somebody opened it after dinner.
 *
 * Both dates are built the same way from a YYYY-MM-DD string, so the subtraction
 * is a whole number of days wherever it runs. See HOME_ZONE.
 */
export function daysUntil(value) {
  const target = parseDate(value);
  const today = parseDate(homeToday());
...
/**
 * The day an instant fell on where the family lives, as YYYY-MM-DD.
 *
 * Never slice the front off an ISO timestamp to get this. That reads the UTC day,
 * which every evening after seven Central is tomorrow: a fare read at 9pm on the
 * 20th was labelled as received on the 21st, a date the family had not lived yet.
 */
export function homeDayOf(value) {
  if (!value) return "";
...
/**
 * A timestamp said in the family's own zone: the clock time if it landed today,
 * the month and day if it did not.
 *
 * The zone is named rather than left to the machine for the reason HOME_ZONE
 * exists, and the locale is named for the same reason. The inbox is drawn twice,
 * once on the server and again in the browser. The server runs on UTC and with
 * whatever locale the platform hands it, so a message that landed at eight in
 * the evening in Missouri had already turned tomorrow on the server -- the two
 * renders put different text in the same place and React threw a hydration
 * error over it every evening.
 */
export function stampSaid(iso, now = new Date()) {
  if (!iso) return "";
  const at = new Date(iso);
  if (Number.isNaN(at.getTime())) return "";
  const parts =
    homeToday(at) === homeToday(now)
...
/**
 * The day a timestamp landed on, spelled out, in the family's own zone. Empty
 * when there is no usable date, so the caller can pick its own words for that.
 * Same hydration reason as stampSaid.
 */
export function stampDaySaid(iso) {
  if (!iso) return "";
  const at = new Date(iso);
  if (Number.isNaN(at.getTime())) return "";
  return new Intl.DateTimeFormat("en-US", {
...
/**
 * A finished trip somebody has put away.
 *
 * The only thing archiving changes: the trip moves into a shut list at the foot of
 * the Past trips screen instead of sitting on the shelf with the rest. Everything
 * it knows still counts -- its reviews still teach Aly what this family likes, and
 * its packing list still seeds the next one. It is about what is worth looking at,
 * not about what is worth keeping.
 */
