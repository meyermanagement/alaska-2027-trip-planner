 ...
 *
 * A trip is six questions. Where do you want to go, when, how do you get there,
 * where do you sleep, what do you do, and how do you get around once you are
 * there. Everything else the app holds -- the itinerary, the packing list, the
 * reservations, the tasks -- is detail hung on those six. A trip with an answer
 * to each of them, however loose, is plannable. A trip missing one of them has a
 * hole in it that no amount of detail elsewhere fills: knowing the restaurant on
 * the fourth night is worth nothing if nobody has worked out how you land.
 *
 * So this module exists to answer two questions and nothing else: which of the
 * six does this trip have, and which one should be asked about next. That is the
...
/**
 * The six, with the words used to ask about them.
 *
...
/**
 * Where each of the six lives on a trip row.
 *
 * Four of them are their own column and read straight across. The other two were
 * already there under other names: where is the destination, and when is the
 * date range -- or, on a draft, whatever the family said about when instead of a
 * date range.
 */
...
/** Whether a trip has any kind of answer to one of the six. */
export function hasBasic(trip, id) {
  return basicValue(trip, id).length > 0;
}

/**
 * The six with their answers, in asking order -- the shape the draft screen
...
/**
 * How far along a draft is, as a count rather than a percentage, because six is
 * few enough to say out loud and "50% planned" is a claim this app cannot make.
 */
 ...
 * complete answer to all six, which is the point: the conversation fills the rest
 ...
 * the six before anybody has asked anything, and being asked "where do you want
 ...
 * for is the screen you type on: showing three of six lighting up as you write
...
/**
 * Read a free-text trip idea and say which of the six it seems to touch.
 *
...
/** How many of the six an opening line appears to cover. */
 ...
 *
 * Never a scold and never a requirement. Six of six is not the goal -- Aly asks
...
export function coverageLine(text) {
  ...
  if (covered === 0) {
    return "Aly will ask about all six. You can start with one line.";
  }
  const names = ids
...
/**
 * The one question about a new trip that is not one of the six.
 *
 * Everything on this screen used to land as a draft, because most new trips are
 * ideas and a draft is the safe place for an idea: out of Upcoming, off the
 * calendar, never the next trip. But the family also arrives here having already
 * paid for something -- the Baoase booking existed before any of the Curacao
 * planning did -- and filing that as an idea is wrong in a way that matters. A
 * booked trip belongs on the calendar, gets a countdown, gets its passports
 * checked and its packing list built. An idea should do none of those things.
 *
 * The app cannot infer this. "We're going to Hawaii in April" is exactly what
 * somebody says about a trip they have paid for and about one they are dreaming
 * about, so guessing from the sentence would be confident and wrong. So it is
 * asked, in three words, before the conversation starts.
 *
 * `ask` is the sentence handed to Aly, because the answer has to survive the
 * trip from this screen into the conversation that creates the trip. `lands` is
 * what the screen says will happen, so the choice has a visible consequence
 * rather than being a setting somebody has to trust.
 */
export const HOW_REAL = [
  {
    id: "idea",
    status: "draft",
    label: "Still an idea",
    hint: "Nothing is paid for. We might not go.",
    ask: "This is still just an idea — we have not decided to go. Create it as a draft.",
    lands:
      "It lands in Drafts. Nothing goes on the family calendar until you move it across.",
  },
  {
    id: "going",
    status: "planning",
    label: "We're going",
    hint: "Decided on, but nothing booked yet.",
    ask: "We have decided to take this trip, but nothing is booked yet. Create it as a real trip with status planning, not a draft.",
    lands: "It lands in Upcoming trips, with everything still to book.",
  },
  {
    id: "booked",
    status: "booked",
    label: "Already booked",
    hint: "Flights, a hotel or a cruise are paid for.",
    ask: "This trip is already booked and paid for. Create it as a real trip with status booked, not a draft. Ask me for the actual dates if I have not given them, because a booked trip cannot have approximate ones, and then ask me what is already booked so it goes on the itinerary.",
    lands:
      "It lands in Upcoming trips and on the family calendar, and Aly will ask what is already booked.",
  },
];

/** One of the three, or null for a screen where nobody has answered yet. */
export function howReal(id) {
  return HOW_REAL.find((option) => option.id === id) || null;
}

/**
 * The idea and the answer, as one thing to say to Aly.
 *
 * When nobody has chosen, Aly is told to ask rather than being left to assume:
 * the whole point of the question is that the wrong assumption is expensive in
 * both directions.
 */
export function ideaWithReality(text, id) {
  const said = String(text || "").trim();
  const choice = howReal(id);
  if (!said) return "";
  if (!choice) {
...
export function draftSuggestions(
  ...
  if (hasWhere && hasWhen && packing.length === 0) {
    out.push({
      id: "packing",
      label: "Start the packing list",
      why: "Built from past trips, the place and the time of year.",
