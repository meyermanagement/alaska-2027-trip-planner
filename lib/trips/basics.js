 ...
 *
 * Every example now ends with a rough figure. Budget was the seven's most-often-
 * dark chip on this screen: without a dollar sign in the sentence the detector
 * has nothing to catch, and without an example that mentions money at all, nobody
 * learned that mentioning money was a thing to do. A number tacked onto the end
 * of a sentence about manta rays is exactly how a real person says it -- "and
 * around $8k if that is realistic" -- and it teaches the box's shape without
 * turning any of these into a form.
 *
 * The first is Mark's, kept in shape and voice, with a budget line added on to
 * bring it in line with the other three.
 */
export const TRIP_IDEA_EXAMPLES = [
  "I want to go to the big island of Hawaii for spring break next year so that I can swim with the manta rays and see Volcanoes National Park. All in around $8k for the four of us if that is realistic.",
  "Somewhere in Portugal for about ten days, sometime next spring. We would fly, and we want to be in one apartment the whole time rather than moving every two nights. Mostly food and walking, one day trip to the coast. Budget around $6,000 not counting flights.",
  "Long weekend in Chicago with the kids. Train there, hotel near the river, and the only thing we have actually promised them is the science museum. Trying to keep the whole thing under $1500.",
  "A cruise. No idea where — we have never done one, we are not sure we will like it, and we would want to fly somewhere warm to get on it rather than drive. Something in the $5k range all in for two.",
];
...
export function coverageLine(text) {
  ...
  if (covered === 0) {
    return "I will ask about all seven. You can start with one line.";
  }
  const names = ids
  ...
  if (left === 0) return `That covers all six — ${list}. I can draft it now.`;
  return `That already covers ${list}. I will ask about the other ${left === 1 ? "one" : left}.`;
}
