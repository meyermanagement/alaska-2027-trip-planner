...
export function hasAwardPricing(text) {
  ...
  return String(text || "").split(/\n/).some((line) => {
    ...
    return /\b\d[\d,.]*k?\s*(?:(?:to|[-–—])\s*\d[\d,.]*k?\s*)?(?:aa\s+)?(?:miles|points|avios)\b/i.test(line);
  });
}
function hasPoints(text, value) {
  const tokens = String(text).match(/\b\d[\d,]*(?:\.\d+)?k\b|\b\d[\d,]*\s*(?=(?:aa\s+)?(?:miles|points|avios)\b)/gi) || [];
  for (const match of String(text).matchAll(/\b(\d[\d,.]*k?)\s*(?:to|[-–—])\s*(\d[\d,.]*k?)\s*(?:aa\s+)?(?:miles|points|avios)\b/gi)) {
...
export function validateAwardPricing(award, text) {
  ...
  for (const raw of award.options) {
    ...
    if (!["points", "miles", "avios"].includes(raw.points_unit) || !new RegExp(`\\b${raw.points_unit}\\b`, "i").test(proof)) return null;
    ...
    // A fee the sender only estimates ("~$220 roundtrip"): kept as their words,
    // never as cash_amount, so a card can say fees exist without claiming one.
    const fees = String(raw.fees_text || "").trim();
    if (fees && (!/\$\s*\d/.test(fees) || !normalized(text).includes(normalized(fees)))) return null;
    options.push({
      ...(fees ? { fees_text: fees } : {}),
      program, points_min: min, points_max: max,
      points_unit: raw.points_unit,
...
export function awardOptionLabel(option) {
  ...
  const main = `${points} ${option.program} ${option.points_unit === "avios" ? "Avios" : option.points_unit}`;
  if (option.cash_amount === null) return `${main} · ${basis(option.points_basis)} · ${/\$\s*[\d]/.test(`${option.pricing_text || ""} ${option.fees_text || ""}`) ? "exact fees not confirmed" : "fees not stated"}`;
...
/** The points and the program, with no basis or fees. For a lock screen. */
export function awardPointsPhrase(option) {
  if (!option) return "";
  const points = option.points_min === option.points_max
    ? count(option.points_min)
    : `${count(option.points_min)}\u2013${count(option.points_max)}`;
  return `${points} ${option.program} ${option.points_unit === "avios" ? "Avios" : option.points_unit}`;
}
export function cashPhrase(price, priceBasis) {
  return `${dollars(Number(price))} ${basis(priceBasis)}`;
}
// A newsletter lists whichever program it wants first, often the dearest one.
// Rank on what a seat actually costs the household: points for a single
// direction, so a round-trip quote is not judged against a one-way one.
export function awardOnewayPoints(option) {
  const points = Number(option?.points_min);
  if (!Number.isFinite(points)) return Infinity;
  return option.points_basis === "round_trip" ? points / 2 : points;
}
const onewayPoints = awardOnewayPoints;
export function cheapestAwardOption(options) {
  const list = Array.isArray(options) ? options.filter(Boolean) : [];
  if (!list.length) return null;
  return list.reduce((best, option) => onewayPoints(option) < onewayPoints(best) ? option : best);
}
/** The email's options, cheapest first, so a card reads in the same order. */
export function rankedAwardOptions(options) {
  const list = Array.isArray(options) ? options.filter(Boolean) : [];
  return [...list].sort((a, b) => onewayPoints(a) - onewayPoints(b));
}
/** Every program the email priced, cheapest first, named once each. */
export function awardProgramNames(options) {
  const list = Array.isArray(options) ? options.filter(Boolean) : [];
  const ranked = [...list].sort((a, b) => onewayPoints(a) - onewayPoints(b));
  return [...new Set(ranked.map((option) => String(option.program || "").trim()).filter(Boolean))];
}
export function farePriceLabel(deal) {
  const options = deal.award_pricing?.options;
  if (options?.length) {
    const best = cheapestAwardOption(options);
    const others = awardProgramNames(options).filter((name) => name !== best.program);
    return `${awardOptionLabel(best)} per person${others.length ? ` · also ${others.join(", ")}` : ""}`;
  }
  return deal.price == null ? "Price not stated" : `${dollars(Number(deal.price))} per person · ${basis(deal.price_basis)}`;
}
export function fareIdentity(deal) {
  return JSON.stringify([
    ...
    deal.award_pricing ? null : deal.price_basis || "unspecified",
