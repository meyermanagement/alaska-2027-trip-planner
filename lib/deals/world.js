...
import { fareForToday } from "@/lib/deals/deadline";
import { homeToday } from "@/lib/format";

/**
 * What a screen has to read to say when a fare arrived.
 *
 * The row's own created_at is when this app saved the fare, which is not when the
 * email came in -- a reread months later would claim the alert arrived today. The
 * date the family recognizes lives on the message, so it is joined rather than
 * guessed.
 */
export const FARE_SELECT = "*, inbox_messages ( received_at )";
...
export function judged(deals = [], world = {}, today = new Date()) {
  return (deals || []).map((original) => {
    const { inbox_messages: message, ...deal } = original;
    return {
      ...fareForToday(deal, typeof today === "string" ? today : homeToday(today)),
      // The joined row is flattened here so no nested message object is shipped to
      // the browser with it.
      email_received_at: deal.email_received_at || message?.received_at || null,
