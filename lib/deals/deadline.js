...
// Fold already-declined past-deadline offers into history even when the date was
// estimated. This only organizes history; active estimates never auto-dismiss.
...
export function fareListsForToday(deals = [], today = homeToday()) {
  ...
  for (const original of deals) {
    ...
    if (deal.status === "open") lists.open.push(deal);
      ...
      const expired = original.status === "expired" || fareDeadlinePassed(deal, today);
