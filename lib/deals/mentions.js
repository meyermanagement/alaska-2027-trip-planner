...
import airportsJson from "./geography.json";
...
function destinationCodes(clean, { sectionOnly = false } = {}) {
  ...
  return [...new Set([...text.matchAll(/\(([A-Z]{3})\)/g)].map((match) => match[1]))]
    .filter((code) => airportsJson[code]);
}

const cityOf = (code) => String(airportsJson[code][0]).replace(/\s*\(.*$/, "").trim();
 ...
 */
export function fittingMentions(text, someday = [], { airports = [] } = {}) {
  ...
  return {
    places,
    months,
    monthsSaid: monthsSaid(months),
    // The fare the email states outright, when it states one from an airport
    // they fly from. Null is the ordinary case, and the card then says the match
    // without offering to put a price on a trip.
    fare: fareChoices(text, airports),
  };
}

/**
 * What an alert of this shape lets a household choose from, read with no model
 * involved.
 *
 * A newsletter like this prices departure cities -- "Dallas (DFW) - 63k" -- and
 * then lists onward cities with nothing against any of them. The reader saves
 * nothing, and rightly: the email never says what Oslo costs. What it does say is
 * which airports are priced, at how many points, and which cities are in the
 * offer, so the pairing is the one thing missing and the one thing a person can
 * supply. This returns both lists; the person picks.
 *
 * Only departures they fly from come back, because a fare from an airport they
 * have never saved is not a fare they can take.
 */
export function fareChoices(text, airports = []) {
  const clean = readableFareText(text);
  const mine = new Set(
    (airports || []).map((row) => String(row?.code || row).toUpperCase()).filter(Boolean),
  );
  const block = clean.match(
    /(?:^|\n)\s*Departure Cities\s*\n([\s\S]*?)(?=\n\s*Destinations\b|\n\s*How to Book\b|$)/i,
  );
  if (!block) return null;
  const priced = [...block[1].matchAll(/([A-Za-z .'-]+?)\s*\(([A-Z]{3})\)\s*[-\u2013]\s*([\d.]+)k/g)]
    .map((match) => ({
      city: match[1].trim(),
      code: match[2],
      points: Math.round(Number(match[3]) * 1000),
      said: `${match[1].trim()} (${match[2]}) - ${match[3]}k`,
    }))
    .filter((row) => row.points > 0);
  const departures = priced.filter((row) => mine.has(row.code));
  if (!departures.length) return null;
  const section = clean.match(
    /(?:^|\n)\s*Destinations\s*\n([\s\S]*?)(?=\n\s*How to Book\b|\n\s*Booking Options\b|$)/i,
  );
  const destinations = section
    ? [...new Set([...section[1].matchAll(/\(([A-Z]{3})\)/g)].map((match) => match[1]))]
      .filter((code) => airportsJson[code])
      .map((code) => ({ code, city: cityOf(code) }))
    : [];
  if (!destinations.length) return null;
  const currency = block[1].match(/Pricing is in ([A-Za-z]+)\s*\n+\s*([A-Za-z]+)/);
  const program = currency ? `${currency[1]} ${currency[2]}` : null;
  return {
    departures,
    destinations,
    program,
    unit: /avios/i.test(program || "") ? "avios" : "points",
    airline: clean.match(/Flying ([A-Z][A-Za-z]+)/)?.[1] || null,
    cabin: /\bbiz class\b|\bbusiness class\b/i.test(clean) ? "business" : null,
    // The hub the email says these fares fly nonstop to, when it says one.
    nonstop: clean.match(/nonstop to ([A-Z]{3})\b/)?.[1] || null,
    headPoints: clean.match(/from ([\d.]+)k points/i)?.[1] || null,
    others: priced.filter((row) => !mine.has(row.code)).map((row) => row.code),
  };
}
