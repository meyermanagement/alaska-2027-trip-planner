...
export function manyBrief(text, hard, source, airports = []) {
  return [
    ...
    'AWARD ALERTS: Never use taxes/fees as the fare price. Omit price and return award_pricing: {options:[{program,points_min,points_max,points_unit:"miles"|"points",points_basis:"one_way"|"round_trip"|"unspecified",cash_amount:number|null,cash_currency:"USD",cash_basis:"one_way"|"round_trip"|"unspecified",round_trip_cash:number|null,pricing_text,route_text}]}. Keep points and cash together. Quote pricing_text and route_text exactly from the email, including the program name and trip basis. Use the route-specific miles when listed; keep overall ranges in the quoted context. Unknown fees are null, never zero. One row per route/cabin, with booking programs inside options, not separate rows for one-way versus round-trip fees. Never multiply one-way points to invent a return price. Do not attach return-only partner offers to an outbound route.',
    "",
...
export async function readForwardedFares({
  ...
  if (!hard.prices.length && !hasAwardPricing(text))
  ...
  try {
    ...
    await pushFareArrival({ supabase: admin, familyId, messageId, count: rows.length });
  } catch (err) {
