...
//
// Missing household facts stay missing. A drive time that was never typed is
// absent rather than estimated, a missing ceiling produces no price verdict, and
// geographic relationships come from the airport reference, not a model guess.
// Every function can return "cannot tell" instead of a confident guess.
//
...
import { formatFullDay, homeToday } from "@/lib/format";
...
import { geographyScore } from "@/lib/deals/geography";
...
/** The bucket-list line this fare is about, if it is about one. */
export function somedayFor(deal, someday = []) {
  ...
  for (const row of rows) {
    ...
    const geographic = geographyScore(deal, row);
    if (!onPlace && !onRegion && !geographic) continue;
    // A broad experience match must respect its season. A directly named city
    // still stays visible with the existing "wrong months" verdict.
    const seasonal = row.months?.length ? row
      : /\bchristmas\b/i.test(row.place) ? { ...row, months: [11, 12] } : row;
    const season = monthCheck(deal, seasonal).verdict;
    if (((!onPlace && !onRegion) || /\bchristmas\b/i.test(row.place))
      && season === "misses")
      continue;
    // A winter Copenhagen fare belongs to Christmas markets, not a separate
    // Copenhagen-in-summer wish. Specificity breaks ties AFTER seasonal fit.
    const score = (season === "fits" ? 10 : 0) + (onPlace ? 5 : onRegion ? 4 : geographic);
    if (score > bestScore) {
      best = row;
      bestScore = score;
    }
...
export function ceilingCheck(deal, place) {
  if (deal?.award_pricing || deal?.price_basis !== "round_trip") return { verdict: null };
...
export function monthCheck(deal, place) {
  ...
  // Most fare alerts give a season rather than a window -- "availability October
  // to February" -- and that is enough to answer this question, which is about
  // months and never about days. So the months read off the email stand in when
  // nobody wrote a date, and the answer says which it used, because "the dates
  // fall outside July and August" would be claiming dates nobody has.
  if (!start && !end) {
    const named = (deal?.travel_months || []).filter(
      (m) => Number.isInteger(Number(m)) && Number(m) >= 1 && Number(m) <= 12,
    );
    if (!named.length) return { verdict: null };
    const worksOut = named.some((m) =>
      monthWanted(months, `2000-${String(m).padStart(2, "0")}-01`),
    );
    return {
      verdict: worksOut ? "fits" : "misses",
      said: monthsSaid(months),
      from: "months",
      offered: monthsSaid(named.map(Number)),
    };
  }
...
export function tripFor(deal, trips = [], today) {
  if (deal?.status === "taken")
    return deal.trip_id ? trips.find((trip) => trip.id === deal.trip_id) || null : null;
  const named = [deal?.destination, deal?.destination_code]
    .filter(Boolean)
    .join(" ");
  const live = (trips || []).filter(
    (trip) => canAttachFare(trip, today),
  );
...
/**
 * Whether the flights on that trip have already been bought.
 *
 * This does not decide whether the fare is kept. A cheaper fare on a trip whose
 * flights are booked is still worth seeing -- some of them are cheap enough to
 * rebook against -- so the fare stays and the card says the flights are already
 * paid for, which turns it from a decision into information. The alternative,
 * throwing it away, quietly loses the one case where the news is best.
 *
 * Reads the trip's own itinerary rows: a flight the family marked confirmed is
 * the only signal in the app that a seat has actually been bought.
 */
function flightsCheck(trip, itinerary = []) {
  if (!trip) return { verdict: null, booked: 0, planned: 0 };
  const flights = (itinerary || []).filter(
    (item) => item && item.trip_id === trip.id && item.category === "flight",
  );
  const booked = flights.filter((item) => item.status === "confirmed").length;
  const planned = flights.length - booked;
  if (booked) return { verdict: "booked", booked, planned };
  return { verdict: "needed", booked: 0, planned };
}
 ...
 *
 * A deadline can arrive two ways and they are not the same claim. One is written
 * on the page. The other is the sender guessing -- "we think this will last less
 * than 24 hours" -- counted forward from the hour their mail arrived. The second
 * is worth having and is not this app's deadline to announce, so it comes back
 * marked, with the sender's own words attached, and the card says who expected
 * what.
 */
export function urgency(deal, today = new Date()) {
  const by = deal?.book_by;
  const inferred = Boolean(deal?.book_by_inferred);
  const words = String(deal?.deadline_said || "").trim() || null;
  // Named, because an expectation belongs to whoever held it.
  const source = String(deal?.source_name || "").trim() || "The sender";
  if (!by) return { verdict: null, inferred: false, words, source };
  const then = new Date(`${by}T12:00:00`);
  if (Number.isNaN(then.getTime()))
    return { verdict: null, inferred, words, source };
  const start = new Date(today);
  start.setHours(12, 0, 0, 0);
  const days = Math.round((then - start) / 86400000);
  const said = formatFullDay(by);
  if (days < 0) return { verdict: "gone", days, said, inferred, words, source };
  return {
...
export function pointsFor(deal, rewards = []) {
  // Flexible balances are not proof of a transfer path into an award program.
  // Do not recommend spending or transferring them on an unverified match.
  if (deal?.award_pricing) return [];
...
export function dealVerdict(deal, world = {}) {
  ...
  const trip = tripFor(deal, trips, typeof today === "string" ? today : homeToday(today));
  ...
  const flights = flightsCheck(trip, itinerary);
  const clashes = clashesWith(deal, { trips, itinerary, trip });
  const points = pointsFor(deal, rewards);

  const facts = [];
  if (deal.award_pricing) {
    facts.push("This fare needs miles or points as well as cash. The fees alone are not compared with your cash fare limit or trip budget.");
    facts.push("Check award seats and the return price before transferring points.");
  }

  if (trip) {
    facts.push(`This is ${trip.name}, which you already have.`);
    if (flights.verdict === "booked")
      facts.push(
        `${flights.booked === 1 ? "A flight is" : `${flights.booked} flights are`} already booked on ${trip.name}, so this only helps if it beats what you paid.`,
      );
  } else if (place) {
    facts.push(`${place.place} is on your bucket list.`);
    if (!placeMatches([deal.destination, deal.destination_code].join(" "), place.place)
      && geographyScore(deal, place)) {
      facts.push(`${deal.destination} is a possible gateway for that trip. Check the specific travel dates, local event dates and onward travel before booking.`);
    }
  }
  ...
  // No third branch here on purpose. A place with no ceiling said is the
  // ordinary case rather than an omission -- nothing on the bucket list carries
  // one today -- so a line announcing the absence appeared on every fare, said
  // nothing about this fare, and pushed the facts that did down the card.
  // Silence is the honest answer: the other lines still say what is known.

  // Said two ways, because a fare that named its months and one that named its
  // days are not making the same claim and the line should not pretend they are.
  if (month.verdict === "misses")
    facts.push(
      month.from === "months"
        ? `It is good for ${month.offered}, and you said ${month.said}.`
        : `The travel dates fall outside ${month.said}.`,
    );
  else if (month.verdict === "fits" && month.said !== "any month")
    facts.push(
      month.from === "months"
        ? `It is good for ${month.offered}, which lands in ${month.said}, when you said you can go.`
        : `The dates are inside ${month.said}, which is when you can go.`,
    );
  ...
  // Everything in this block is attributed when the date was worked out from a
  // sender's guess rather than read off the page, because the difference between
  // "it has to be booked by Thursday" and "they expected it gone by Thursday" is
  // the difference between a fact and an invented one.
  if (when.inferred) {
    const quoted = when.words ? ` They wrote: "${when.words}".` : "";
    if (when.verdict === "gone")
      facts.push(`${when.source} expected this gone by ${when.said}.${quoted}`);
    else if (when.verdict === "today" || when.verdict === "soon")
      facts.push(
        `${when.source} expected this gone by ${when.said}, ${when.days === 0 ? "today" : `${when.days} ${when.days === 1 ? "day" : "days"} off`}. No booking date was written in the email.${quoted}`,
      );
  } else if (when.verdict === "gone")
...
function headlineFor({ place, trip, ceiling, month, origin, seats, when }) {
  // A deadline nobody wrote down cannot make the card say the thing is over.
  if (when.verdict === "gone")
    return when.inferred ? "Probably gone by now" : "Too late";
...
export function dealLines(deals = [], world = {}) {
  ...
  const lines = [
    "FARES FORWARDED IN (open, with the verdict already worked out for you):",
  ];
  for (const deal of open) {
    const v = dealVerdict(deal, world);
    const months = (deal.travel_months || []).map(Number).filter(Boolean);
    const window =
      deal.travel_start || deal.travel_end
        ? `${deal.travel_start || "?"} to ${deal.travel_end || "?"}`
        : months.length
          ? `good for ${monthsSaid(months)}, no exact dates given`
          : "no dates given";
    lines.push(
      `  ${deal.origin} to ${deal.destination}, ${farePriceLabel(deal)} ${deal.cabin}${deal.airline ? ` on ${deal.airline}` : ""}, ${window}, from ${deal.source_name} [id: ${deal.id}]`,
    );
  ...
  if (refused.length) {
    ...
    for (const row of refused.slice(0, 8))
      lines.push(
        `  ${row.origin} to ${row.destination}, ${farePriceLabel(row)}${row.dismissed_reason ? ` -- ${row.dismissed_reason}` : ""}`,
      );
    lines.push(
