...
function answerToPrefs(answer) {
  ...
  const pickedText = Array.isArray(picked)
    ...
    : typeof picked === "string"
      ? picked.trim()
      : "";
  // A band answer is two hours -- "9:30 am to midnight" -- and an animals
  // answer is a sentence about the animals; neither says what question it
  // answers once it is a line in a prompt, so both carry the question's own
  // label the way the real answer route files them.
  if (pickedText) {
    out.push({
      slot,
      body:
        (answer?.kind === "band" || answer?.kind === "pets") && answer?.label
