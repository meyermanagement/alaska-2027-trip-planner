...
function answerToPrefs(answer) {
  ...
  // where an option's label is a picture of the preference, and the same rank,
  // multi and band sentences the answer route composes. `picked` is the words on
  // the screen, which is right for the recap and wrong for a prompt: it would
  // file the picture and leave a family on record as answering a question they
  // were never asked.
