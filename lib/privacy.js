...
export const PROCESSORS = [
  ...
  },
  {
    ...
    role: "Alyeska's answers and reading forwarded booking email",
    sends:
      "Your question and the trip context sent with it, or the forwarded email being read, only when you have turned on Alyeska's AI or the matching feature.",
    where: "Not committed by OpenAI for our account",
  },
  {
    // READER_PROVIDER already says Google, so wrapping it in "Google (...)" read
    // as a company inside itself.
    name: READER_PROVIDER,
    role: "Web lookups, reading documents, fare alerts, tips, and generated trip images",
    sends:
      "The question or file being read and the trip context it needs, only when you have turned on Alyeska's AI or the matching feature.",
    where: "United States",
  },
  {
...
export const AI_SECTION = {
  heading: "Alyeska and the AI providers",
  body: `Alyeska's assistant is not a person and not our own model. When you ask it something, your question and the parts of your trip needed to answer it are sent to the ${ASSISTANT_PROVIDER}, and so are the booking confirmations you forward. Everything else goes to the ${READER_PROVIDER}, including questions that need the web, documents you ask it to read, fare alerts, and tips, processed on servers in the United States. If OpenAI is unavailable, the same work may go to Google instead. These are the only model providers Alyeska sends to on its own (an assistant you connect yourself is covered in the next section); if that ever changes we will name the new one here and ask you again before anything reaches it. This is the one place your household's information leaves our own systems for a purpose other than storing or delivering it, so it is a choice you make rather than a default you discover.`,
  points: [
    "You are asked before any of it is sent, on your first run and never by a pre-ticked box.",
    "Turning off Alyeska's AI in Settings stops the sending. It is enforced where the model is called, so the answer is refused rather than the button hidden.",
    "Turning it off also stops our overnight reminder and deadline jobs from sending anything, even though nobody is signed in when they run.",
    ...
    "Your uploaded files stay in our storage unless you use Read this document on one, which sends that one file to be read and needs its own permission. Alyeska does not open your documents to answer an ordinary question.",
  ],
};

/**
 * Outside assistants a person connects over MCP (Claude, ChatGPT, Gemini). They
 * are not our processors: the person chooses the assistant and its company
 * handles what it reads under their own agreement. What is disclosed here has to
 * match lib/mcp/trust.js (the consent screen) and lib/mcp/protocol.js (what the
 * tools share and redact); change them together.
 */
export const ASSISTANTS_SECTION = {
  heading: "Assistants you connect",
  body: "You can let an outside assistant, such as Claude from Anthropic, ChatGPT from OpenAI, or Gemini from Google, reach your household's trips through Alyeska. Nothing is shared until you sign in to Alyeska from that assistant and approve it on a screen that lists what it can read and change. Once you approve it, what it reads is handled by that assistant's company under your agreement with them, not ours.",
  points: [
    "Your approval covers only the wording you saw. If the wording changes, you are asked again.",
    "You can remove a connection in Settings, under Connect an assistant. After that, the assistant is refused.",
    "It can read trips, daily plans, packing and day packs, reminders, adult travelers, preferences, budget, wallet programs, document and pet expiration dates, insurance coverage, bucket list, fare alerts, favorite moments and past reviews.",
    "For a parent, it also reads and checks off the children's packing and day pack items, and nothing else about a child.",
    "It can check things off and add or change the items named on the approval screen. It cannot delete anything except taking an add-on packing template off a trip when you ask.",
    "Before a change saves, you are shown what it will change and asked to confirm. When an assistant cannot show that prompt itself, Alyeska asks it to put the question to you and saves only when it returns your yes; Alyeska cannot tell whether that yes came from you or from the assistant.",
    "It never receives health or allergy details; confirmation, ID, member, policy or microchip numbers; typed notes; or anything from another household.",
    "A secondary traveler or a child cannot use it to change the household's trips.",
  ],
};
...
export const PRIVACY_SECTIONS = [
  ...
  },
  {
    ...
    points: [
      ...
      "Conversations with Alyeska: 90 days, then deleted.",
      ...
      "The waitlist: until you ask us to take you off it.",
  ...
  },
  {
    ...
    points: [
      "Turn off Alyeska's AI, which stops anything being sent to the AI provider.",
  ...
  },
  {
    id: "children",
    heading: "Children",
    body: "Independent sign-in requires you to be 18 or older. A parent may add a child as a traveler so plans and packing fit the family. A parent can also open a temporary, read-only view of that child’s assigned non-draft trips and own packing. The child does not sign in, use Alyeska's AI, send messages, edit lists, upload files, share location, or register for push notifications. This view uses a necessary access cookie and no screen-use analytics or session replay. Parent authorization and security records are scheduled for removal after 30 days; essential hosting and security logs may also be retained under the policies described here. Parent passkey public keys remain until the parent account is deleted. Revoking a login does not itself delete existing traveler information. Parents can contact us for access, correction, or deletion.",
  },
  {
    // For people who have no account at all, so it cannot lean on anything a
    // tester read at the gate. What the form stores is supabase/migrations/
    // 20261016_waitlist.sql and 20261017_waitlist_names.sql; the throttle is makeThrottle in lib/home/waitlist.js.
    id: "waitlist",
    // Linked to from the form itself, as /privacy#waitlist.
    anchor: true,
    heading: "If you joined the waitlist",
    points: [
      "What we keep: your first and last name, your email address, your household size if you chose one, whether you said you organize trips for a group or an organization, and the date you joined.",
      "What it is for: inviting you into the beta and, if you said you organize trips for others, inviting you to try group trips when they open. Nothing else, and it is never sold or shared.",
      "Who can see it: only the people running the beta. Nobody can read the list through the app, signed in or not.",
      "Your IP address is used to stop repeated tries from one place. It stays in the server's memory and is never saved with your entry.",
      "Your invitation goes out through the email service listed above, which sees your address and the message.",
    ],
    note: `To be taken off the list, write to ${SUPPORT_EMAIL}. You don't need an account.`,
  },
  {
    id: "transfers",
