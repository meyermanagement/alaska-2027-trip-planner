...
export const CORE_TOOLS = [
  ...
  "add_preference",
  // A person the family names for the first time should land in the file
  // wherever they were named, not only inside the interview.
  "add_person",
  // A moment somebody tells is worth keeping wherever they tell it.
  "add_favorite_moment",
...
const BY_FOCUS = {
  ...
  // add_day_pack_item belongs to the day screens and the packing screen, because
  // the bag is opened at the top of a day and listed on the packing page, and
  // nowhere else asks "what do we carry".
  itinerary: ["update_review", "find_tips", "add_day_pack_item"],
  ...
  today: ["update_review", "find_tips", "add_day_pack_item"],
  packing: [
    "clear_packing_list",
    "tidy_packing_list",
    "start_packing_list",
  ...
  tasks: ["find_tips"],
  // The money tab. The two most-asked things here are "put this figure on that"
  // and "where could we save", and only the first needs a tool the core set does
  // not already carry: an itinerary item's estimate goes on with update_itinerary_item,
  // which is core, so what this adds is the money that is not on any day.
  budget: [
    "add_trip_cost",
    "update_trip_cost",
    "delete_trip_cost",
  ...
  // The insurance tab. Everything asked here is either a question the context
  // already answers or a policy being written down from a document in front of
  // them, so what this adds is the writing.
  insurance: ["add_policy", "update_policy", "attach_policy", "find_tips"],
  notes: ["find_tips"],
  [TEMPLATES_FOCUS]: [
  ...
  // Settings is about the person. add_preference is already in the core set,
  // which is most of what gets said here; what this adds is the record their own
  // details live on, because a birthday or a carrier mentioned on the screen
  // that is otherwise all about them has nowhere else to go.
  [SETTINGS_FOCUS]: [
    "set_person_details",
    // A rule about themselves -- an allergy, a knee, a language -- is said on the
    // screen that is otherwise all about them, and it is not a preference.
    "record_household_fact",
  ],
  // The Bucket list screen. The four writes here are the whole point of the screen,
  // and none of them is reachable from anywhere else: a place wanted with no trip
  // for it, and the airports a fare for it would leave from.
  [SOMEDAY_FOCUS]: [
    "save_someday_place",
    "retire_someday_place",
    "save_home_airport",
    "remove_home_airport",
    // The two decisions a fare on this screen ends in. Offered here because this
    // is where the cards are, and a family who has just read a verdict is one
    // sentence away from making the call in the chat instead.
    "dismiss_deal",
    "put_deal_on_trip",
    // "would that be worth doing in November" is the question this screen asks,
    // and it is a question about a place rather than about a trip.
    "find_tips",
  ],
  [REWARDS_FOCUS]: [
    "add_rewards_program",
...
const RESCUES = [
  {
    // Money said from a screen that is not the Budget tab. "What is this going to
    // cost", "can we afford it", "put $400 on the flights" are all asked from the
    // overview and from the itinerary, and the cost tools live on one tab.
    test: /\b(budget|afford|cost|costs|costing|spend|spent|spending|\$\d|price|prices|expensive|cheaper)\b/i,
    tools: ["add_trip_cost", "update_trip_cost"],
  },
  {
    // Insurance said from a screen that is not the Insurance tab. "are we
    // covered", "what if we cancel", "who do I call" get asked from the overview
    // and from a day, and the policy tools live on one tab.
    test: /\b(insurance|insured|policy|policies|coverage|covered|deductible|evacuation|claim|claims|cancel for any reason|cfar)\b/i,
    tools: ["add_policy", "update_policy", "attach_policy"],
  },
  {
  ...
  // Which airports the household leaves from, said from anywhere. This comes up
  // on the Family page under the home address, in the middle of planning a trip,
  // and while looking at a fare -- and the drive to a further airport is the fact
  // only they have, so the words for it are matched too.
  {
    test: /\b(airports?|fly (?:out )?of|flying out of|fly from|flies from|fly into|drive to the airport|home airport|nonstop|non-?stop|origin)\b/i,
    tools: ["save_home_airport", "remove_home_airport"],
  },
  // Wanting to go somewhere with no trip for it. Nobody asks for this by the
  // screen's name, whatever the screen happens to be called: they say they have
  // always wanted to see Japan, or that Banff is on the list. So the shape of the
  // wish is matched rather than the name of the screen.
  {
    test: /\b(someday|some day|bucket ?list|wish ?list|wishlist|one ?day|always wanted|have wanted|would love to (?:go|see|visit)|dream (?:trip|destination)|on (?:the|our|my) list|places we want|want to (?:go|see|visit))\b/i,
    tools: ["save_someday_place", "retire_someday_place"],
  },
  // A fare that has already been pasted in, being decided about anywhere in the
  // app. Nothing here can invent a price -- both tools only mark a fare already
  // on file -- so the rescue is safe from any screen.
  {
    test: /\b(fare|fares|airfare|deal|deals|mistake fare|price alert|that flight price|book by|not for us|take it)\b/i,
    tools: ["dismiss_deal", "put_deal_on_trip"],
  },
  {
  ...
  // Facts and rules rather than tastes, said from wherever they come up. An
  // allergy, a diagnosis, a knee, a child's age, what somebody cannot eat: these
  // are constraints on every trip and filing one as a preference buries it under
  // the things that are only opinions.
  {
    test: /\b(allerg\w*|anaphyla\w*|epipen|asthma\w*|diabet\w*|celiac|coeliac|gluten|lactose|kosher|halal|vegan|vegetarian|pescatarian|peanut|shellfish|medication|meds|prescription|insulin|cpap|seizure|epileps\w*|autis\w*|adhd|sensory|wheelchair|walker|cane|crutch\w*|knee|hip|back|bad (?:knee|hip|back|ankle)|surgery|pregnan\w*|nurs(?:e|ing)|motion ?sick\w*|seasick\w*|vertigo|afraid of|scared of|phobi\w*|claustrophob\w*|acrophob\w*|can'?t (?:eat|walk|climb|swim|fly|do)|cannot (?:eat|walk|climb|swim|fly)|years? old|months? old|toddler|infant|baby|newborn|car ?seat|booster|nap ?time|bedtime)\b/i,
    tools: ["record_household_fact"],
  },
  // Being asked to conduct the interview from wherever they are. "Ask me some
  // questions", "get to know Veda", "what do you still need to know" all mean the
  // same thing, and the slot bookkeeping is what makes the next one different
  // from the last.
  {
    test: /\b(get to know|getting to know|ask me|ask her|ask him|ask them|quiz|questionnaire|interview|what (?:do|don'?t) you (?:still )?(?:know|need)|this or that|travel (?:file|profile)|fill in (?:my|her|his|their))\b/i,
    tools: ["set_slot_status", "record_household_fact", "update_preference"],
  },
  ...
  // What goes on somebody rather than in the case, asked from wherever they are
  // standing. "What should we take on the boat", "what do I wear Thursday",
  // "what goes in the backpack" are all the same question, and none of them says
  // the words the bag is called.
  {
    test: /\b(day ?packs?|daypacks?|backpacks?|rucksack|tote|carry|carrying|take with|bring with|on us|on me|wear|wearing|dress for)\b/i,
    tools: ["add_day_pack_item"],
  },
...
export function toolNamesForRequest({
  ...
  // The interview screen. Its focus carries a person's id rather than a screen
  // name, so it cannot be a key in the table above.
  if (interviewTravelerId(focus)) {
    wanted.add("record_household_fact");
    wanted.add("set_slot_status");
    wanted.add("update_preference");
    // The rest of the travel file, because an interview that can only write a
    // preference leaves the file as empty as it found it. A language, a piece of
    // equipment, the airline somebody is loyal to, whether the horse travels:
    // all of that comes up in these answers and all of it belongs somewhere the
    // rest of the app reads.
    wanted.add("set_person_details");
    // A first-login interview has to be able to add the household it is
    // interviewing. Without add_person, "there are four of us and a dog" would
    // be saved as one household fact and the file would keep one person in it.
    wanted.add("add_person");
    // Favorite moments are the second thing the interview opens with, and Aly
    // needs the tool wherever she is asked about them.
    wanted.add("add_favorite_moment");
    wanted.add("delete_favorite_moment");
    wanted.add("add_pet");
    wanted.add("update_pet");
    wanted.add("set_pet_trip");
    wanted.add("add_rewards_program");
    wanted.add("update_rewards_program");
    // Not the follow-up buttons. An interview turn already ends in a question --
    // its own next one -- and the general instruction to offer two to four more
    // is what put "Shall we check the plans for Portugal Spring 2027?" under a
    // question about mornings, and on one turn put the call itself on screen in
    // place of any question at all. Three buttons leading away from the
    // interview is the one thing this screen cannot afford to offer.
    wanted.delete("offer_followups");
    // Nor a shortlist of places. Nobody asked for anywhere to go.
    wanted.delete("show_places");
  }

  // What the wording adds, kept apart from what the screen already gives.
  const worded = new Set();
  const said = typeof message === "string" ? message : "";
  if (said) {
    for (const rescue of RESCUES) {
      if (rescue.test.test(said)) {
        for (const name of rescue.tools) if (!wanted.has(name)) worded.add(name);
      }
    }
  }

  if (said && petNameRescue(said, petNames)) {
    for (const name of PET_TOOLS) if (!wanted.has(name)) worded.add(name);
  }

  // The screen's set first, in declaration order, then whatever the wording
  // added, in declaration order after it. The screen's set is the same for every
  // question asked on that screen, so the tool list starts the same way from one
  // question to the next and the vendor's cache can reuse it; a tool the wording
  // pulled in used to land in the middle, which made every tool after it -- and
  // everything the request carried after the tools -- a fresh read.
  return [
    ...ALL.filter((t) => wanted.has(t.name)),
    ...ALL.filter((t) => worded.has(t.name)),
  ].map((t) => t.name);
}
