 ...
 *
 * No session means no person: the nightly run and the inbound mail webhook are
 * machines acting for a household whose consent was recorded when a member of it
 * walked the screens, and they have no cookies to read. Those routes are already
 * behind a shared secret, and they carry on. The same is true when there is no
 * request scope to read cookies from at all, which is what a script or a test
 * looks like from in here.
 */
async function refuseWithoutConsent() {
  let supabase = null;
  let me = null;
  let aiAllowed = null;
  try {
    ...
    const [{ createClient }, { whoIs }, consent] = await Promise.all([
      import("@/lib/supabase/server"),
      import("@/lib/supabase/who"),
      import("@/lib/beta/consent"),
    ]);
  ...
  const allowed = await aiAllowed(supabase, me.id);
  if (allowed) return;
