...
// The providers the consent screens name, and therefore the only ones an
// account's text may reach, each for the work it was disclosed for.
//
// lib/beta/agreement.js names OpenAI for Ask Aly's answers and Google for web
// lookups and every other feature, and the stored consent row keeps ai_provider
// alongside the answer. LLM_PROVIDERS was built before that screen existed and
// would happily put Anthropic at the front of the chain -- a processor nobody
// disclosed, added by an environment variable. So the chain is decided here
// rather than trusted. Adding a provider is a code change that has to move
// AI_PROVIDERS, the privacy section and the version constants with it.
export const DISCLOSED_PROVIDERS = ["openai", "gemini"];
// Everything that is not a plain Ask Aly answer.
export const DISCLOSED_PROVIDER = "gemini";

// The Ask Aly turns disclosed as going to OpenAI. The finishing turns name
// themselves chat.* too; they follow whichever model gave the answer.
const ASSISTANT_FEATURE = /^chat\./;

// Which vendor answers plain Ask Aly turns. "openai" unless ASSISTANT_PROVIDER
// says gemini, which is the way back without a deploy. Anything else is ignored.
export function assistantProvider() {
  const said = String(process.env.ASSISTANT_PROVIDER || "").trim().toLowerCase();
 ...
 *
 * - A request pinned to a model goes to the vendor that owns it, so a finishing
 *   turn follows the model that answered.
 * - A grounded request goes to Gemini: web lookups are disclosed as Google's.
 * - A plain Ask Aly turn goes to OpenAI, with Gemini behind it if OpenAI is
 *   down or over its rate limit. Both are disclosed for that work.
 * - Everything else goes to Gemini.
 */
export function chainFor({ feature = null, grounded = false, models = [] } = {}) {
  const pinned = Array.isArray(models) ? models.filter(Boolean) : [];
  if (pinned.length) return [openai.owns(pinned[0]) ? "openai" : "gemini"];
  if (grounded) return ["gemini"];
  if (ASSISTANT_FEATURE.test(String(feature || "")) && assistantProvider() === "openai")
...
export function providerNames() {
  ...
  const allowed = asked.filter((name) => DISCLOSED_PROVIDERS.includes(name));
...
export function undisclosedProviders() {
  ...
  return asked.filter((name) => name && !DISCLOSED_PROVIDERS.includes(name));
}
 ...
 */
/**
 * The consent check, done once for a whole request and shared by every generate
 * call in it. Returns the same thing refuseWithoutConsent resolves to, as a
 * promise, so a route can start it early and pass it along. The refusal is not
 * softened: a rejected promise refuses the first call that awaits it, exactly
 * as the per-call check did, and the route's own error handling sees the same
 * ModelError.
 */
export function consentOnce(consentFor = null) {
  return refuseWithoutConsent(consentFor);
}
...
export async function generate({
  ...
  // question is worth deliberating over. May also be one level per vendor,
  // { openai: "high", gemini: "low" }, for a turn that should think harder on
  // one vendor than the other.
  thinking = null,
  ...
  avoid = [],
  // The opposite of avoid: exactly these model ids, in this order, and no
  // ladder beyond them. For a finishing turn that exists to add words or cards
  // to an answer already in hand, walking to a slower model when the first one
  // says nothing is the wrong trade: one model, one attempt, a warmer
  // temperature, and the answer already in hand stands if that comes back
  // empty. Ignored when empty.
  models = [],
  // How many attempts each of those models gets. Null keeps the provider's own
  // count; 1 is what the finishing turns want.
  attempts = null,
  ...
  feature = null,
  // Machine-readable results must retain record IDs. Chat cleanup deliberately
  // removes IDs from prose, which corrupts structured search/selection results.
  // This is an internal caller option, never a user-controlled request field.
  responseFormat = "text",
  // A consent already established for this request, from consentOnce(). A chat
  // turn asks the model up to four times, and each ask used to load three
  // modules, open a Supabase client and look the person up again to reach the
  // same answer. Checked once, the answer is handed to every call in the turn.
  consent = null,
  // Which of the declared tools this call may use, for a turn that has to send
  // the same tool list as the one before it so the cached prefix still matches.
  // Null leaves every tool callable. Honored by the Gemini adapter.
  allowedTools = null,
}) {
  const who = await (consent
    ? // A promise or a resolved value both work; a rejected one refuses here,
      // the same way a fresh check would.
      Promise.resolve(consent)
    : refuseWithoutConsent(consentFor));
  const request = {
    system,
    messages,
    tools,
    ...
    responseFormat,
    allowedTools,
    // Which part of the app is asking, so the adapter can label its request
    // fingerprint. Not sent to any vendor.
    feature,
  };
  try {
    const result = await runChain(chainFor(request), PROVIDERS, request);
    await recordSpend(who, feature, result.usage, result.provider);
    return result;
  } catch (err) {
 ...
 */
async function recordSpend(who, feature, usage, provider = null) {
  ...
  try {
    ...
    await recordUsage(who.supabase, {
      userId: who.userId,
      feature,
      provider: provider || DISCLOSED_PROVIDER,
      calls: usage,
    });
  } catch {
...
// One vendor's level from a per-vendor setting; a plain level goes to all.
export function thinkingFor(thinking, provider) {
  if (!thinking || typeof thinking !== "object") return thinking ?? null;
  return thinking[provider] ?? null;
}
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    try {
      const result = await provider.generate({
        ...request,
        thinking: thinkingFor(request.thinking, chain[i]),
      ...
      const lifted = request.responseFormat === "json" ? result : liftSpokenCalls(result);
      return {
        ...lifted,
