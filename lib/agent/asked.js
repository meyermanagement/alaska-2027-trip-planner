 ...
 */
export function saidNothing(text) {
  const said = String(text == null ? "" : text).trim();
  if (!said) return true;
  // A reply that is really a tool call, typed out. One interview turn arrived as
  // `offer_followups("What should we look at next?", ...)` and nothing else: the
  // model meant to call the tool, wrote the call as prose instead, and the app
  // put it on screen as the question. It is not an answer, and it has to be
  // treated as the silence it is so the turn gets asked again.
  return /^[a-z][a-z0-9_]*\s*\(/i.test(said) && said.endsWith(")");
}

/**
 * Both the silent version and the thin version.
 */
...
// Words that ask for something on the trip to be written, moved or taken off.
// Deliberately generous in the other direction from ASKING: a match keeps the
// full second attempt, with every change tool, and being wrong here costs one
// model call while missing it would leave a request to change the trip answered
// in words alone.
const CHANGING =
  /\b(add|adding|put|move|moving|change|changing|update|updating|remove|removing|delete|drop|cancel|rename|reschedule|schedule|book|mark|set|swap|switch|replace|pack|save|keep|note|record|remember|invite|attach|clear|create|make|start|plan|log|rate|review|fix|correct)\b/i;

/**
 * Could that message be asking for the trip to change?
 *
 * Used where an empty first answer is handled: a question can be answered in
 * words on the finishing turn, which cannot propose anything, but a request to
 * add, move or remove something needs the change tools and gets the full retry.
 */
export function asksForChange(said) {
  return CHANGING.test(String(said == null ? "" : said));
}

/**
 * Is a full second attempt worth paying for when the first turn came back empty?
 *
 * Only where the finishing turn could not stand in for it: a message that may be
 * asking for a change (the finish cannot propose one), an interview turn (its
 * answer is usually something to save), or a message with no question in it at
 * all (the finish would be told to answer a question nobody asked).
 */
export function retryWhenEmpty({ said, interviewing = false }) {
  if (interviewing) return true;
  if (!asksSomething(said)) return true;
  return asksForChange(said);
}
