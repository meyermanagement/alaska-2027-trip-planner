import { tidyAnswer } from "./tidy.js";
/**
...
export function parseRich(text) {
  // Answers kept before the cleanup existed are tidied on the way to the screen.
  const said = tidyAnswer(String(text == null ? "" : text)).replace(/\r\n?/g, "\n");
  if (!said.trim()) return [];
  const blocks = [];
  let para = [];
