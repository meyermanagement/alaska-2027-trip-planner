...
function rewardsLines(rewards, travelerNameById, known) {
  ...
  if (!rewards.length) {
    lines.push(
      "(nothing saved yet — if the user mentions a program, a balance or a card they carry, offer to add it to the Rewards tab)",
    );
    return lines;
  }
  const cards = [];
