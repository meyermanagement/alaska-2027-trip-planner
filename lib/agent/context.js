  ...
  for (const t of ordered) {
    ...
    if (onTrip.length) {
      ...
      for (const p of onTrip) {
        ...
        // question. The two figures that are not coverage -- the premium, which
        // is what the household paid, and the deductible, which reads as a
        // figure but is priced per person -- are held back with the numbers.
        ...
        const withheld = [];
        if (p.deductible !== null && p.deductible !== undefined)
          withheld.push("deductible");
