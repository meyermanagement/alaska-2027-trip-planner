...
export function buildContext({
  ...
  // the finished trips are already summed up above, a draft nobody asked about
  // is an idea rather than a plan, and a packing list on a trip that is not open
  // is the biggest thing in the record and the least often asked about. Those
  // are printed as counts unless the question names the trip or is about
  // packing -- then the full lines come back, because the question needs them.
  const named = (t) => mentionsTrip(message, t.name);
  const aboutPacking =
    focus === "packing" || /pack|bring|suitcase|bag|luggage|wear|clothes|forgot/i.test(message || "");
  for (const t of ordered) {
    const focused = t.id === focusTripId;
    const past = isPast(t);
    // Header and counts only: a past trip, or a draft that is not open and not
    // named. The rows still register below; only the printing is skipped.
    const terse = !focused && !named(t) && (past || isDraft(t));
    // Packing as per-category counts: any trip that is not open, unless the
    // question is about packing or names the trip.
    const tersePacking = !focused && !terse && !aboutPacking && !named(t);
    ...
    if (onTrip.length) {
      ...
      for (const p of onTrip) {
        ...
        if (claimAsked) {
          if (p.claims_phone) bits.push(`claims ${p.claims_phone}`);
          if (p.claims_url) bits.push(`claims online ${p.claims_url}`);
        } else if (p.claims_phone || p.claims_url) {
          bits.push(
            "claims contact on file, not sent unless asked about a claim",
          );
        }
