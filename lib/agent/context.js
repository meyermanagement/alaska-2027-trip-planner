...
function preferenceLines(preferences, travelerNameById) {
  const lines = ["", "HOW THIS FAMILY LIKES TO TRAVEL:"];
  ...
  for (const p of preferences.slice(0, 60)) {
    ...
    lines.push(`- ${topic}${short(p.body, 220)}${who ? ` (${who})` : ""}`);
  }
  return lines;
}
...
- The family's travel preferences are in the context below. Use them when you suggest hotels, transport, restaurants or anything else, and say plainly when something goes against one. They are edited on the Reviews tab; you cannot change them.
...
- The app also has a "Reviews" tab (hotels, excursions and restaurants from past trips with the family's own ratings, plus their general travel preferences) and a "People" tab (passports, licenses, Known Traveler and Global Entry numbers). You cannot edit either one — point the user at the tab if they ask. You do have their travel preferences below: lean on them whenever you suggest anything, say when a suggestion goes against one, and if the user tells you a new standing preference, suggest they save it on the Reviews tab.
