...
export function buildContext({
  ...
  for (const t of ordered) {
    ...
    if (onTrip.length) {
      ...
      lines.push(
        "INSURANCE: none attached to this trip, though the family owns policies. An annual plan may still cover it -- say so as a question, not as a fact.",
      );
    }
...
export const FOCUS_LABELS = {
  ...
  notes: "the Notes",
  insurance:
    "the Insurance tab — the travel policies covering this trip, what they pay for, and who they name",
};
...
const FOCUS_HINTS = {
  ...
  insurance: {
    ask: '"Are we covered?", "what does it cover?" and "what if we cancel?" mean the policies in the INSURANCE section of this trip. Answer with what the policy says it pays for, the limits, and any DATE GAP or NOT NAMED line, all of which are already worked out for you -- never calculate a limit, never infer a peril the policy does not list, and never tell them whether a claim would be paid. "Who do I call?" means the emergency line first, then the claims line -- and the claims line is only in your context when the question is about a claim, so if it is absent say it is on the policy card in the app rather than saying there is none.  A figure marked "on file but not sent to you" is recorded in the app and deliberately withheld from you: say where to find it, never that it is missing. If a figure is not on file, say it is not on file and offer to read the document.',
