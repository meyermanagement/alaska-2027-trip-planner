...
function somedayLines(someday = [], travelerNameById = new Map()) {
  ...
  const lines = ["SOMEDAY LIST (places wanted, no trip yet):"];
  for (const row of rows) {
    const bits = [];
...
export function buildContext({
  ...
  // the moment a someday place becomes useful is never a moment somebody opened
...
/**
 * Opened from the Someday screen: the places wanted with no trip for them yet,
...
// On the Someday screen the subject is a place with no trip for it, and the two
...
function somedayFocusSection(focus) {
  ...
  return `WHERE THE USER IS RIGHT NOW:
They are on the Someday screen, looking at the places they want to go with no trip for any of them yet, and at the airports the household flies from. Resolve anything vague against that:
- A place named here goes on the someday list with save_someday_place. It is NOT a trip: do not call create_trip unless they say they are going, or name real dates.
