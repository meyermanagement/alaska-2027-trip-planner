 ...
 * are the family's own record of their cards, prefilled from a catalogue when
 * they added them, so they are a good guide and not a guarantee.
 */
function rewardsLines(rewards, travelerNameById, known) {
  const lines = [
    "",
    "POINTS, MILES AND CARDS THEY HAVE (from the Rewards tab — balances are typed in by hand, not read from the accounts, so treat them as roughly right and say so if a plan hangs on one):",
  ];
  if (!rewards.length) {
    lines.push(
      "(nothing saved yet — if the user mentions a programme, a balance or a card they carry, offer to add it to the Rewards tab)",
    );
    return lines;
  }
  const cards = [];
  ...
  lines.push(...cards);
  lines.push(
    "How to use this: when you suggest a hotel, a flight, a car or a cruise, check whether a balance above could pay for it and say what it would cost in points if you can reason it out, and always name which of their cards earns most on that kind of spending and how much it would earn. Never invent a balance, a redemption rate or an earning rule that is not written above.",
  );
  return lines;
}
...
/** Opened from the Rewards tab, which spans every trip and belongs to none. */
...
// On the Rewards tab a bare number is almost always a balance, and a bare brand
// name is almost always a programme they want added to the list.
function rewardsFocusSection(focus) {
  if (focus !== REWARDS_FOCUS) return "";
  return `WHERE THE USER IS RIGHT NOW:
They are on the Rewards tab, looking at the points, miles and credit cards listed below. Resolve anything vague against that:
- A bare number, or "I have 40k with them", is a points balance for one of those programmes. Update it rather than asking what it is for.
- A bare brand or card name means add that programme. Work out yourself whether it is an airline, a hotel, a cruise line, a car rental club or a credit card, and for a card fill in what it earns and where its points go.
...
- "Which card?" means which of their own cards earns most on the spending they just described.
...
${newTripSection(focus)}${rewardsFocusSection(focus)}${focusSection(focus, focusTripName)}${conversationsSection(extras)}THE FAMILY'S TRIPS:
${contextText}`;
}
