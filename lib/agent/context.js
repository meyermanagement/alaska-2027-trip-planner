...
export function buildContext({
  ...
  if (loose.length) {
    ...
    for (const p of loose) {
      known.insurance_policies.set(p.id, policyLine(p) || "a policy");
      const names = (insuredByPolicy.get(p.id) || [])
        .map((id) => travelerNameById.get(id))
        .filter(Boolean);
      lines.push(
        `- id=${p.id} | ${policyLine(p) || "no provider named"} | ${kindLabel(
          p.kind,
        )} | ${
  ...
  for (const t of ordered) {
    ...
    if (onTrip.length) {
      ...
      for (const p of onTrip) {
        ...
        const bits = [
          `id=${p.id}`,
          policyLine(p) || "no provider named",
          kindLabel(p.kind),
          p.coverage_start || p.coverage_end
            ? `covers ${p.coverage_start || "?"} to ${p.coverage_end || "?"}`
            : "no coverage dates on file",
