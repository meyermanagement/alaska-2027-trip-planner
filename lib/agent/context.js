  ...
  const petQuestion = petNames.length
    ? `\n- THE ANIMALS, once the six are moving. They have ${petNames.join(", ")} on file. An animal decides which hotels are even possible, whether flying is on the table at all, and what half the itinerary can be, so ask before the itinerary rather than after it: "${
        ...
        petNames.length > 1
          ...
          : ""
      } Then pass EVERY animal they told you about in the pets argument to create_trip (or set_pet_trip afterwards, if the trip already exists), each with its own arrangement: "coming" for the ones traveling, and "boarding", "sitter", "family" or "undecided" for the ones staying behind, with anything they said about the kennel or the sitter in arrangement_notes. Record the ones staying home as well as the ones coming \u2014 an animal you leave out is one nobody has decided about yet, which is not the same thing as one that is staying home. Only the animals coming get packing lines. If they wave the question off, leave the argument out rather than guessing.`
    : "";
...
- It is a draft unless they tell you otherwise: pass status "draft" when they are still turning it over or did not say, "planning" if they told you they mean to take it, "booked" only if they told you it is paid for. ASK IT AND THEN OBEY THE ANSWER \u2014 if they say it is a real trip, do NOT create a draft anyway. Somebody who answers the question and gets the other thing has been ignored.
