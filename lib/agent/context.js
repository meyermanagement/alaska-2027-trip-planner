...
function newTripSection(focus, extras) {
  ...
  const petQuestion = petNames.length
    ? `\n  5. Whether any of the animals are coming. They have ${petNames.join(", ")} on file. An animal decides which hotels are even possible, whether flying is on the table at all, and what half the itinerary can be, so it is one of the first things to settle rather than an afterthought. Ask it plainly \u2014 "is ${petNames[0]} coming, or staying home?" \u2014 and if they say yes, pass that animal's name in the pets argument to create_trip so it goes on the trip and its own packing lines are added with the list. If they say it is boarding or with a sitter, leave it out of pets and use set_pet_trip after the trip exists.`
    : "";
...
- Record what happens to each pet per trip with set_pet_trip, and ask when a family with pets on file plans a trip and has not said. “Are the dogs coming?” is a better first question than any hotel suggestion, because the answer changes every suggestion after it.
