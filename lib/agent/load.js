...
/**
 * The same reads, handed back as rows rather than as a prompt.
 *
...
export async function readEverything(supabase, userId) {
  const [
    ...
    supabase
      ...
      .order("sort_order", { ascending: true }),
    // What the family carries on each day, which is not what is in the case. Read
    // on every turn because Aly's first job here is not to suggest a water bottle
    // that is already in the bag: without this she was asked what else to carry
    // and answered with half of what was already on the list.
    supabase
      .from("day_pack_items")
      .select("id, trip_id, item_date, item, why, assignee, is_packed, source")
      .order("item_date", { ascending: true, nullsFirst: true })
      .order("sort_order", { ascending: true }),
    // Advice already filed onto a day. It shows up in the same bag on the screen,
    // so a suggestion that repeats it is a duplicate the family can see and Aly
    // cannot.
    supabase
      .from("pro_tips")
      .select("id, trip_id, for_date, title")
      .eq("scope", "daypack")
      .eq("status", "active")
      .order("for_date", { ascending: true }),
    // What happens when the trip goes wrong. Read on every turn rather than only
    // when somebody asks about insurance, because the useful moment for a policy
    // is never the moment it is discussed: it is the answer to "is the cruise
    // refundable if Veda breaks an arm", asked while planning, and the answer to
    // "who do I call" asked from a clinic in Ketchikan. A limit Aly has to be
    // asked twice for is a limit nobody looks up.
    supabase
    ...
    // Where a flight would leave from, and how long the drive to each one costs.
    // Read on every turn because it is the fact that makes a fare answerable:
    // without it, "is $340 to Seattle good" has no origin in it.
    supabase
      .from("home_airports")
      .select("code, name, city, region, drive_minutes, is_primary")
      .order("is_primary", { ascending: false })
      .order("drive_minutes", { ascending: true, nullsFirst: false }),
    // The wish list, with the months, the length and the fare that would make
    // each place worth doing. The trips that do not exist yet -- and the only
    // thing in the app that lets Aly recognize an opportunity rather than answer
    // a question about a trip already being planned.
    supabase
      .from("someday_places")
      .select(
        "id, place, region, lat, lon, why, months, nights, fare_ceiling, traveler_ids, watch, status, trip_id",
      )
      .order("created_at", { ascending: true }),
    // Fares the family pasted in and has not decided about. Read on every turn
    // because the useful moment for one is never the moment it was pasted: it is
    // "should we do Portugal in May", asked three days later, and the answer to
    // that is worse without the $412 sitting in front of her.
    supabase
      .from("flight_deals")
      .select("*")
      .eq("status", "open")
  ...
  return {
    ...
    dayPack: dayPack.data || [],
    dayTips: dayTips.data || [],
    policies: policies.data || [],
    policyTrips: policyTrips.data || [],
    policyTravelers: policyTravelers.data || [],
