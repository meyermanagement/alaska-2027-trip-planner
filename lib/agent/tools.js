...
export const SHARED_TOOL_DECLARATIONS = [
  ...
  },
  {
    name: "dismiss_deal",
    description:
      "Record that the family is not taking a fare they pasted in. Use it when they say a fare is not for them, and put their own reason in the note -- the refusal is what stops the same fare coming back at them next week. The fare stays on file so they can ask again.",
    parameters: {
      type: "object",
      properties: {
        id: {
          type: "string",
          description:
            "The id of the fare, taken from the FARES THE FAMILY PASTED IN lines.",
        },
        reason: {
          type: "string",
          description:
  ...
  },
  {
    ...
    description:
      "Mark a pasted fare as the one they are taking, against the trip it belongs to. Only call it when they have decided; it records the decision and does not book anything. If no trip is named and none is obvious from the fare, ask which one rather than guessing.",
    parameters: {
      type: "object",
      properties: {
        id: {
  ...
  switch (name) {
    ...
    case "add_policy": {
      ...
      const patch = prune({
        ...
        premium: cleanDecimal(args.premium, 200000),
