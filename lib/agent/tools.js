...
export const TOOL_DECLARATIONS = [
  ...
  },
  {
    ...
    description:
      "Take off a trip's packing list every item that belongs to somebody who is not on that trip, in one step. Use this when the user says somebody came off the trip but their things are still on the list, or when the list is out of step with who is going. Anything already packed or with a note stays behind. Never call delete_packing_item once per row to do this.",
    parameters: {
      type: "object",
      properties: { ...tripField },
      required: [],
...
export const TRIP_TOOL_DECLARATIONS = [
  ...
  },
  {
    ...
    description:
      "Record what is true of one person rather than of a trip: their cell phone provider and device, the equipment they travel with, anything else about getting around, and the languages they speak. Call this whenever the user tells you any of it — 'Steph is on AT&T', 'Veda still needs the stroller', 'I speak some Spanish'. These are what let advice be specific, so it is worth recording the moment it is said. Send only the parts you were actually told; anything left out keeps the value it already has. Passports, licenses and Known Traveler numbers are NOT here — those live on the People tab and only the user can edit them.",
    parameters: {
      type: "object",
      properties: {
        whose: {
