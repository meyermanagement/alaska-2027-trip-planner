...
import { matchKnownCaseId } from "@/lib/daypack/link";
import {
  addDays,
  formatFullDay,
  formatRange,
...
import { draftPackingWords } from "../packing/draft";
import { topicPatch } from "../preferences/topics";
import { SLOT_IDS, SLOT_STATUSES, slotLabel } from "../travelers/slots";
import {
  MOBILITY_AID_VALUES,
  aidPhrase,
  cleanAids,
...
import { COST_CATEGORIES, money } from "../budget/budget";
import { airportByCode, airportPlace } from "@/lib/airports";
import { driveMinutesFrom, driveSaid } from "@/lib/airports/drive";
import { monthsSaid, parseMonths } from "@/lib/someday/months";
import {
  COVERS,
  POLICY_KINDS,
  coverLabel,
...
export const BUDGET_CATEGORY_KEYS = COST_CATEGORIES;
...
export const TRIP_STATUSES = [
  ...
  "draft",
  // A real trip: decided on, whether or not anything has been paid for yet.
  //
  // There used to be a "booked" status beside this one, and it earned nothing. It
  // changed no grouping, drew no badge and sorted nothing differently; all it did
  // was give Aly a question to ask and get wrong, and forbid approximate dates on
  // the trips that carried it. Whether a thing is paid for is a fact about that
  // flight or that hotel, and the itinerary already records it per item -- which
  // is where somebody actually looks for it.
  "planning",
  "active",
  "complete",
  "archived",
...
const itineraryFields = {
  ...
  location: {
    type: "string",
    description:
      'Where it is, written so somebody could find it and so the map can pin it. The place\'s own name, then where it actually sits: the town and the state or country for an ordinary address, and on a resort, a park, a ship or a campus the property and the part of it -- "Via Napoli Ristorante e Pizzeria, Italy pavilion, EPCOT, Walt Disney World", "Narcoossee\'s, Disney\'s Grand Floridian Resort, Walt Disney World", "Palo, deck 12 aft, Disney Dream", "Old Faithful Inn, Yellowstone National Park, Wyoming". Never guess the municipality a property\'s mail goes to: it geocodes to a pin in the wrong place and states as fact something the family never gave you. Name the property, add the state or country, and stop.',
  },
  confirmation_number: {
    type: "string",
  ...
  notes: { type: "string" },
  // Money lives on the thing it is spent on. An estimate is welcome on anything
  // and is expected while a trip is being planned: it is what the Budget tab
  // adds up, and a trip whose items carry no figures has a budget screen that
  // can tell the family nothing.
  cost_estimate: {
    type: "number",
    description:
      "What this is likely to cost in US dollars, for everybody going — not per person. A working estimate is wanted, not a quote: put a realistic number on flights, stays, tours and dinners as you add them, and say in your reply that it is an estimate. Multiply out a nightly rate over the nights of the stay. Never state an estimate as a price you have checked.",
  },
  cost_actual: {
    type: "number",
    description:
      "What it actually came to, in US dollars. Only when the user tells you a real figure they have paid or been quoted. Never estimate into this field.",
  },
  cost_note: {
    type: "string",
    description:
      "Where the figure comes from or what it covers: '6 nights at about $310', 'two adults and a child', 'per person, four of us'.",
  },
};
...
export const TOOL_DECLARATIONS = [
  ...
  // What leaves the room with the family on one morning, which is not what is in
  // the case. This tool exists because the day pack shipped without one: Aly was
  // asked what else to carry on a Wednesday, named a dozen things, and had
  // nowhere to put any of them but the suitcase list, undated.
  {
    name: "add_day_pack_item",
    description:
      "Add one thing to a day's pack \u2014 what the family carries on them on that day, which is not the same list as the suitcase. Call this once per distinct item, and call it for EVERY item you recommend in your reply rather than a selection of them. Use it whenever the question is about what to carry, wear, bring or take on a particular day. When the thing is already on the trip's packing list, pass its id as from_packing_id so the line says it comes out of the case instead of reading as a second object. When it is not on the list, the app puts it there itself \u2014 never call add_packing_item for the same thing as well, that writes it twice.",
    parameters: {
      type: "object",
      properties: {
        item: { type: "string" },
        item_date: {
          type: "string",
          description:
            "YYYY-MM-DD, the day it is carried on. Leave it out only for the every-day set \u2014 sunscreen, the water bottle, an inhaler \u2014 which is shown on every day of the trip.",
        },
        assignee: {
          type: "string",
          description: "Who carries it. Use 'Shared' for family items.",
        },
        why: {
          type: "string",
          description:
            "One short clause saying why it is on that day: 'showers from 2pm, open deck boat', 'photo ID checked at the gate'. This is what makes a day pack readable rather than a second inventory, so write one.",
        },
        from_packing_id: {
          type: "string",
          description:
            "The id of the packing item this is the same object as, taken exactly from the PACKING context for this trip. Set it whenever the thing is already in the case.",
        },
        ...tripField,
      },
      required: ["item"],
  ...
  },
  {
    name: "add_trip_cost",
    description:
      "Put money on a trip that is not an event on any day: groceries, gas, checked bags, parking, souvenirs, travel insurance, the pet sitter, the kennel. Anything already on the itinerary takes its cost with update_itinerary_item instead — never add a cost here for a flight, a hotel or a dinner that is on the schedule, or the trip is counted twice. cost_estimate is what you or they think it will come to; cost_actual is only what they have already paid and told you.",
    parameters: {
      type: "object",
      properties: {
        label: {
          type: "string",
          description:
            'What the money is for, in a few words. "Groceries for the week", "checked bags", "boarding for Storm".',
        },
        category: {
          type: "string",
          enum: COST_CATEGORIES,
          description:
            "Which part of the trip it belongs to, so it lands in the right group on the Budget tab.",
        },
        ...
        cost_note: {
          type: "string",
          description:
            "Where the figure came from or what it covers, in a short phrase. Worth filling in on an estimate so nobody mistakes it for a price.",
        },
        ...tripField,
      },
      required: ["label"],
  ...
  {
    name: "update_trip_cost",
    description:
      "Change one of those costs: its wording, its group, its estimate, or the final figure once they have paid.",
    parameters: {
      type: "object",
      properties: {
        id: { type: "string" },
  ...
  {
    name: "delete_trip_cost",
    description:
      "Take one of those costs off the trip. Only for a cost that should not be there at all — a figure that turned out to be wrong is an update, not a deletion.",
    parameters: {
      type: "object",
      properties: { id: { type: "string" } },
      required: ["id"],
...
export const SHARED_TOOL_DECLARATIONS = [
  {
    name: "add_policy",
    description:
      "Keep a travel insurance policy the family has bought: who wrote it, what it pays for, its limits, and the numbers to call. Only from what they tell you or from a document they have had read -- never invent a limit, never guess a policy number, and never record everyday health or car insurance here. If a trip is open, the policy is attached to it; an annual plan can be attached to the others afterwards. Leave out anything they have not said rather than putting a plausible figure in it.",
    parameters: {
      type: "object",
      properties: {
        provider: {
          type: "string",
          description: "Who wrote the policy, e.g. 'Allianz', 'Travel Guard'.",
        },
        plan_name: { type: "string" },
        policy_number: { type: "string" },
        kind: {
          type: "string",
          enum: POLICY_KINDS.map((k) => k.value),
          description:
            "'trip' for a policy bought for one journey, 'annual' for a plan that covers every trip in its year.",
        },
        coverage_start: { type: "string", description: "YYYY-MM-DD." },
        coverage_end: { type: "string", description: "YYYY-MM-DD." },
        covers: {
          type: "array",
          items: { type: "string", enum: COVERS.map((c) => c.value) },
          description:
            "Only the perils the policy actually names. Anything else it covers goes in notes.",
        },
        emergency_phone: { type: "string" },
        claims_phone: { type: "string" },
        claims_url: { type: "string" },
        // No premium. Aly cannot read it back -- see lib/agent/context.js -- and a
        // figure she can write but never recall reads as amnesia about money. It
        // reaches the record through the inbox parser and the policy card, which
        // are the two places it belongs. The deductible is different: it decides
        // whether a claim is worth filing, so she can both save it and discuss it.
        deductible: { type: "number" },
        medical_limit: { type: "number" },
        evacuation_limit: { type: "number" },
        notes: {
          type: "string",
          description:
            "Exclusions, waivers, what a rider covers -- in the policy's own terms, not your reading of them.",
        },
      },
      required: ["provider"],
    },
  },
  {
    name: "update_policy",
    description:
      "Change a policy already on file: a limit they have looked up, a claims number, the dates, what it covers. Only fields they have told you.",
    parameters: {
      type: "object",
      properties: {
        id: { type: "string" },
        ...
        plan_name: { type: "string" },
        policy_number: { type: "string" },
        kind: { type: "string", enum: POLICY_KINDS.map((k) => k.value) },
  ...
  },
  {
    name: "attach_policy",
    description:
      "Say that a policy already on file covers the trip that is open. What an annual plan is for: bought once, attached to each trip as it comes. Never use it to claim a policy covers a trip when its dates do not reach -- the trip's INSURANCE lines already say whether they do.",
    parameters: {
      type: "object",
      properties: { id: { type: "string" }, ...tripField },
      required: ["id"],
    },
  },
  {
  ...
  },
  {
    name: "show_places",
    description:
      "Show a shortlist of real places as cards the family can look at: restaurants, hotels, or things to do. Call this whenever you recommend somewhere, INSTEAD OF listing the places in your reply - the card carries a photo, the address, a link to the place, a link to the map, and a button to put it on the itinerary. Write the words that go above the cards in reply, inside this call, rather than as separate text: the answer and its cards arrive together, and a shortlist with nothing said about it is not an answer. This saves nothing and changes nothing; the family taps a card to add it. Only real places you are confident exist. Asked for more after a shortlist you already showed: leave out everything already on screen, and set tradeoff on every place so the family can see what each new one gives up against the first set. Asked for the highest regarded places regardless of whether they suit this family: choose on reputation alone, set misfit on every place instead of tradeoff, and leave group off. A place to stay must carry nightly and nightly_basis -- an average for the family's own dates, because a hotel with no price is not a recommendation they can turn down -- and must name one of their own rewards programs or cards in program when it gets them something there.",
    parameters: {
      type: "object",
      properties: {
        places: {
          ...
          items: {
            ...
            properties: {
              ...
              kind: {
                type: "string",
                enum: ["eat", "stay", "do", "car"],
                description:
                  "eat for a restaurant, bar or cafe; stay for a hotel, resort, aparthotel, villa or a whole-house rental; do for an activity, tour, museum or sight; car for a car to rent -- a company's own car class at that airport, or one particular car on Turo.",
              },
              via: {
                type: "string",
                enum: ["airbnb", "vrbo", "turo"],
                description:
                  'ONLY when the suggestion is booked through one of these rather than with the place itself: a whole-house or apartment rental on Airbnb or Vrbo, or a car lent by its owner on Turo. Leave it out for a hotel, a resort, a restaurant, an activity, and for a car from a rental company. Setting it changes what the card says -- it prints "On Airbnb", links to the listing or to that site\'s search for the area, and drops the map pin and the photograph lookup, because a listing title is not a place on a map. It also removes any hotel or rental-car program from the card, since none of them reach a platform booking; a credit card can still earn on it.',
              },
              area: {
                type: "string",
                description:
                  'Where it is, accurately, because this is what finds the place on the map and fetches its photograph. Town, district or island, plus the country if it is not obvious. When it sits inside a property rather than in a town -- a theme park, a resort, a ship, a campus -- name the property and the part of it instead, such as "Italy pavilion, EPCOT, Walt Disney World", "deck 12 aft, Disney Dream" or "Old Faithful area, Yellowstone National Park", and never the municipality around the property, which is a guess that pins the card somewhere the family is not. On a platform listing this is what the site\'s own search link is built from, so name the town or neighborhood they would actually search.',
              },
              why: {
                type: "string",
                description:
              ...
              tradeoff: {
                type: "string",
                description:
                  "ONLY when this is a further suggestion after a shortlist you already showed, and then on every place in the new set: one sentence saying plainly what this one gives up against the ones shown first -- further out, longer drive, no view, worse for the ages on the roster, dearer for what you get, harder to book, closed the day they are there. It is the reason it was not in the first set, so it has to be a drawback and not a compliment in a different jacket. Leave it out on a first shortlist, where there is nothing to compare against. If the first set was genuinely the best available, say so in your reply and show fewer places, or none, rather than padding the list.",
              },
              misfit: {
                type: "string",
                description:
                  "ONLY when the family has asked for the highest regarded places regardless of whether they suit them, and then on every place in that set: one sentence saying which of their own preferences, constraints or circumstances this one cuts against, naming the preference so they can see which rule they would be setting aside -- the drive they said they did not want, the formality they avoid, the age on the roster it is wrong for, the budget it sits above, the crowds they said they dislike. Where it genuinely suits them despite not having been chosen for them, say that plainly instead: an invented doubt is worse than none, because it makes a good suggestion look risky. Never set this alongside tradeoff; they answer different questions and two grey boxes on one card is one too many.",
              },
              group: {
                type: "string",
              ...
              nightly: {
                type: "string",
                description:
                  'REQUIRED for kind=stay and kind=car. The average rate for one night -- or for one day, on a car -- on THIS FAMILY\'S OWN DATES, in the local currency with its symbol: a single figure or a tight range, such as "EUR 640" or "$480-560". Not the cheapest night of the year and not the rack rate. On a rental or a Turo car, make it what they would actually pay per day with the cleaning fee, the service fee, the young-driver or airport fee and the tax spread across the stay, and say in nightly_basis that it is the all-in figure -- a nightly rate that doubles at checkout is the single most misleading number a card can carry. Leave it out only if you truly have no idea what it costs, which should be rare.',
              },
              nightly_basis: {
                type: "string",
                description:
              ...
              program: {
                type: "string",
                description:
                  "The exact brand of one of the family's OWN rewards programs or credit cards that gets them something here, copied as it is written in their rewards list. Expected on kind=stay, where their hotel programs and their cards' own hotel channels both count; worth setting on kind=eat or kind=do too when a card they carry has a credit or a benefit there. Leave it out when none of them applies -- and then say so in the words above the cards rather than staying silent about it. NEVER name a program they are not enrolled in -- the app checks this against their rows and throws away a perk it cannot find, so an invented one costs you the whole line.",
              },
              perk: {
                type: "string",
                description:
        ...
        reply: {
          type: "string",
          description:
            "The words printed above these cards: the answer to what they asked, which the cards illustrate. Which one you would book and why, what would change your mind, and how they differ from each other -- the walkable one against the taxi ride, the loud one against the quiet one, the one that needs a table weeks out. A few sentences, or two short paragraphs. Not the names read out again, and not a line telling them to tap a card: both are already on screen.",
        },
      },
    ...
    parameters: {
      ...
      properties: {
        ...
        reply: {
          type: "string",
          description:
            "Your answer to what they just asked, in full: the words these buttons go under. Write it here, inside this call, rather than as separate text, so the answer and its next questions arrive together -- buttons with no answer above them are not an answer. When show_places in this same reply already carries your answer, leave this empty rather than saying it twice.",
        },
      },
      required: ["questions", "reply"],
    },
  },
  {
    name: "add_preference",
    ...
    parameters: {
      ...
      properties: {
        ...
        whose: {
          type: "string",
          description:
            "Who the preference belongs to. One traveler's name when it is only about that person; SEVERAL NAMES -- 'Mark and Steph' -- when it belongs to some of them but not all, which is used on any trip one of them is on and left off the ones none of them are. Use 'Shared', or leave this out, ONLY when it is genuinely true of everybody. Naming the people matters: something saved as Shared applies to every trip forever, so 'we want a suite so Veda has her own bed' filed as Shared says a child is in the room on a trip Veda is not on, and quietly rules out places that would have suited the two of them perfectly.",
        },
        slot: {
          type: "string",
          description:
            "The ledger slot this answers, when the context named one to pursue and this row is the answer to it: pace, mornings, doing_or_seeing, weather, crowds, food, evenings, comfort, downtime. Leave it out when the preference came up on its own rather than out of a question you asked.",
        },
        source: {
          type: "string",
          description:
            'How you came to know it, and this matters more than anything else on the row. "said" when they told you in words and the body is close to their own -- the default. "derived" when you worked it out from a choice they made and they never said it outright, which the app shows differently so nobody mistakes your reasoning for their words. "noticed" when you spotted it across their trips and nobody has confirmed it. Never mark a guess as said.',
        },
        reason: {
          type: "string",
          description:
            'Why, in their own words, verbatim, when they gave a reason. This is the most valuable thing you can save -- "because the light is better and we get it to ourselves" answers questions nobody has asked yet -- so keep their phrasing and do not tidy it. Leave it out if they gave no reason; never write one on their behalf.',
        },
      },
      required: ["body"],
    },
  ...
  {
    name: "record_household_fact",
    description:
      'Save something about a person that is not a matter of taste: an allergy, a medical or mobility limit, how much walking is realistic, a language they read, a child\'s age. These are the constraints under every recommendation, so they are kept apart from preferences and are never traded off against a nicer restaurant. Ask for these plainly and in full -- never infer one from a picture or a choice, and never guess at a limit. An opinion is not a fact: "she would rather not walk far" is a preference, "she cannot manage more than about a mile" belongs here.',
    parameters: {
      type: "object",
      properties: {
        body: {
          type: "string",
          description:
            "The fact itself, in their words where you have them, specific enough to act on: 'Shellfish allergy, carries an EpiPen', 'About a mile on the flat is comfortable, stairs are hard', 'Reads Spanish well enough for a menu'.",
        },
        kind: {
          type: "string",
          description:
            'Which of three. "rule" for something that must be obeyed and can never be traded away, like an allergy or a medication schedule. "capability" for what a body can do today: distance, stairs, heat, altitude, a boat. "fact" for anything else worth never asking twice, like ages and languages. When it sits between rule and capability, choose rule.',
        },
        whose: {
          type: "string",
          description:
            "The one person it is about. Leave it out only when it is true of the household rather than of a person.",
        },
        slot: {
          type: "string",
          description:
            "The ledger slot this answers, when there was one: limits or languages.",
        },
        source: {
          type: "string",
          description:
            'Almost always "said", because these are asked rather than deduced. Use "derived" only when you worked it out and it still needs confirming.',
        },
      },
      required: ["body", "kind"],
    },
  },
  {
    ...
    description:
      "Retire a question nobody is going to answer. Call it the moment somebody waves one away, says it does not divide them, or changes the subject off it twice -- with their reason in the note, because a no with a reason is worth more than a blank. Only ever the question you actually asked them: skipping one they were never asked writes down an answer they never gave. You cannot mark a question as asked or as settled; the app records what you asked from your own reply, and saving a preference or a fact against a slot is what settles it. This changes no advice by itself; it is the bookkeeping that lets an interview end.",
    parameters: {
      type: "object",
      properties: {
        slot: {
          type: "string",
          description:
            "The slot id from the context: pace, mornings, doing_or_seeing, weather, crowds, food, evenings, comfort, downtime, limits, languages.",
        },
        status: {
          type: "string",
          description:
            'One of: "skipped" when they declined, said the question does not divide them, or changed the subject off it twice; "open" to put a skipped one back because they have started answering it after all.',
        },
        whose: {
          type: "string",
          description:
            "The one person it is about. Leave it out for a household slot.",
        },
        note: {
          type: "string",
          description:
            "Why it was skipped, in their words: 'says it depends entirely on the trip'.",
        },
      },
    ...
    parameters: {
      ...
      properties: {
        ...
        whose: {
          type: "string",
          description:
            "A traveler's name to make it about one person, several names -- 'Mark and Steph' -- to make it about some of them, or 'Shared' to make it about the whole family. Whatever is given replaces the owners it has now.",
        },
      },
      required: ["id"],
    },
  ...
  {
    name: "save_home_airport",
    description:
      "Save an airport the household flies from, or change the drive time or home base on one they already have. Call this when they name an airport they use -- 'we fly out of St. Louis', 'Kansas City is about four hours', 'we would drive to Chicago for a good fare'. The airport's name, city and position come from its three-letter code, so the code is all you need. A drive time is worth asking for once and never worth guessing: it is the whole reason a further airport is ever worth considering. Saving a code they already have changes that row rather than adding a second one.",
    parameters: {
      type: "object",
      properties: {
        code: {
          type: "string",
          description:
            "The airport's three-letter IATA code, uppercase: STL, MCI, ORD. If they named a city rather than a code, use the code of the airport that city means. If more than one airport could be meant -- a family saying 'Chicago' could mean ORD or MDW -- ask which rather than picking.",
        },
        drive_time: {
          type: "string",
          description:
            "How long the drive there takes, in their own words: '25 minutes', '4 hours', '1:30', '2 hr 15'. Save it only when they have said it. Never estimate it from the distance -- the app knows where the runways are and nothing about the roads, and a drive time they did not say is the one number here that would be quietly wrong.",
        },
        home_base: {
          type: "boolean",
          description:
            "True when this is the airport a fare should be priced from by default -- the one they fly from unless there is a reason not to. Only one airport can hold it, so setting it here moves it off whichever airport had it. Leave it out unless they have said which one that is.",
        },
      },
      required: ["code"],
    },
  ...
  {
    name: "remove_home_airport",
    description:
      "Take an airport off the list the household flies from. Only when they say they do not use it -- 'we would never actually drive to Chicago'. Removing the home base leaves them with none until another is set, so say so if it comes to that.",
    parameters: {
      type: "object",
      properties: {
        code: {
          type: "string",
          description:
            "The three-letter code of the airport to remove, as it appears on their list of home airports.",
        },
      },
      required: ["code"],
    },
  ...
  {
    name: "save_someday_place",
    description:
      "Put a place on the bucket list, or fill in more about one already on it. Call this whenever they say they want to go somewhere with no trip for it yet -- 'we have always wanted to see Japan', 'Banff one day', 'I would go back to Charleston in a heartbeat'. A place name alone is worth saving; the months, the nights and the airfare that would make it worth doing are what turn a name into something a fare can be checked against, so ask for those over the next few turns rather than all at once. Naming a place already on the list edits that row, so send only what is new. This is not a trip: use create_trip when they are actually going.",
    parameters: {
      type: "object",
      properties: {
        place: {
          type: "string",
          description:
            "Where they want to go, as they say it: 'Kyoto', 'Banff', 'the Amalfi Coast'. Match the wording already on their list when the place is on it.",
        },
        region: {
          type: "string",
          description:
            "The country or state it is in, if they said or it is unambiguous: 'Japan', 'Alberta', 'South Carolina'. Optional.",
        },
        why: {
          type: "string",
          description:
            "What they want to do there, in their own words, and why it appeals. A sentence about the thing they are picturing, not a category -- 'walk the temple district in blossom season, which Steph has wanted since college' is the useful version and 'culture' is not. Optional but worth having: it is what a day gets planned around later, and what makes the place recognizable when a fare turns up two years later.",
        },
        months: {
          type: "array",
          items: { type: "integer" },
          description:
            "The months they could actually go, as numbers 1 to 12. Only when they have said -- school holidays, a season, 'not in summer'. Leave it out for any month; an empty list means any month and is a real answer.",
        },
        nights: {
          type: "integer",
          description:
            "How many nights the trip wants to be. Only when they have said a length, even loosely -- 'a week' is 7.",
        },
        fare_ceiling: {
          type: "number",
          description:
            "The airfare per person that would make this worth booking, in dollars. Per person, not for the household: 'I would go if flights were under six hundred each' is 600. Only when they name a figure -- this number is what a fare gets judged against, so an invented one would make every verdict wrong.",
        },
        who: {
          type: "array",
          items: { type: "string" },
          description:
            "The people it is for, by name, when it is not the whole household -- a trip the two adults would take without the children. Leave it out for everybody.",
        },
        watch: {
          type: "boolean",
          description:
            "False when they want the place on the list but do not want to hear about fares for it. Defaults to true; leave it out unless they say so.",
        },
      },
      required: ["place"],
    },
  ...
  {
    name: "retire_someday_place",
    description:
      "Take a place off the bucket list because they no longer want it, or have been and are done with it. It is kept as retired rather than deleted, so they can put it back. If they have booked it, this is the wrong tool -- the trip itself is the record, and the list is tidied from the Bucket list & fares screen.",
    parameters: {
      type: "object",
      properties: {
        place: {
          type: "string",
          description:
            "The place to retire, worded as it appears on their bucket list.",
        },
      },
      required: ["place"],
    },
  },
  {
    name: "dismiss_deal",
    description:
      "Record that the family is not taking a fare that came in. Use it when they say a fare is not for them, and put their own reason in the note -- the refusal is what stops the same fare coming back at them next week. The fare stays on file so they can ask again.",
    parameters: {
      type: "object",
      properties: {
        id: {
          type: "string",
          description:
            "The id of the fare, taken from the FARES FORWARDED IN lines.",
        },
        reason: {
          type: "string",
          description:
            "Why not, in their words and a few of them. 'Wrong week for us', 'too early to book'.",
        },
      },
      required: ["id"],
    },
  ...
  {
    name: "put_deal_on_trip",
    description:
      "Mark a forwarded fare as the one they are taking, against the trip it belongs to. Only call it when they have decided; it records the decision and does not book anything. If no trip is named and none is obvious from the fare, ask which one rather than guessing.",
    parameters: {
      type: "object",
      properties: {
        id: {
        ...
        trip: {
          type: "string",
          description: "The trip it goes on, named as the family names it.",
        },
      },
      required: ["id", "trip"],
    },
...
export const TRIP_TOOL_DECLARATIONS = [
    ...
    parameters: {
      ...
      properties: {
        ...
        preferred_budget: {
          type: "number",
          description:
            "What the family would like the whole trip to cost, in US dollars, when they tell you a figure or a range (use the middle of a range). This is a preference and never a limit: nothing in the app refuses anything for going over it, and you must never tell them they cannot afford something. It exists so the Budget tab can compare the trip against it and so you can say where a concession would come from. Ask for it early when a trip is being planned and they have not said.",
        },
        travelers: {
          type: "array",
          items: { type: "string" },
  ...
  },
  {
    name: "add_person",
    description:
      "Add another person to this family. Call this the first time somebody who is not on the list yet is named as a member of the household — 'my husband Mark', 'our daughter Veda', 'the two of us and my mom Linda'. Add each person as a separate call. Nothing else has to be known: a name is enough, and their birthday, gender, phone, equipment, languages and a paragraph about them all belong on set_person_details once they are said. Only for people. An animal is add_pet.",
    parameters: {
      type: "object",
      properties: {
        name: {
          type: "string",
          description:
            "Their name as the user said it. Keep the capitalization the user used.",
        },
        relationship: {
          type: "string",
          description:
            "How they are related to the person adding them, in the user's own words: 'wife', 'son', 'my mom', 'partner'. Optional. Not stored on its own row — it is only used to describe the save.",
        },
      },
      required: ["name"],
    },
  ...
  {
    name: "add_favorite_moment",
    description:
      "Record one favorite moment from a past trip in this person's own words. Call this on the first-login interview when the user shares a moment — 'a slow dinner on a terrace in Rome', 'the morning we found tide pools in Maine', 'reading on the beach in Hawaii until the sun went down'. One call per moment; save two or three per person if they mention that many. Keep the user's own phrasing — do not summarize or categorize. This is a paragraph of memory, not a preference; the preference-shaped read of it happens automatically.",
    parameters: {
      type: "object",
      properties: {
        whose: {
          type: "string",
          description:
            "The person the moment is theirs, exactly as their name appears on the family's people list.",
        },
        body: {
          type: "string",
          description:
            "The moment in the user's own words. A sentence or two, not a caption. Keep verbs and adjectives the user used; do not tidy them into a category.",
        },
      },
      required: ["whose", "body"],
    },
  ...
  {
    name: "delete_favorite_moment",
    description:
      "Remove one favorite moment. Call this only when the user says a moment is wrong or is no longer theirs, using the id of the row on the person's file.",
    parameters: {
      type: "object",
      properties: {
        id: { type: "string" },
...
const TABLE_FOR_TOOL = {
  ...
  add_day_pack_item: "day_pack_items",
  add_task: "predeparture_tasks",
  update_task: "predeparture_tasks",
  delete_task: "predeparture_tasks",
  add_note: "trip_notes",
  add_trip_cost: "trip_costs",
  update_trip_cost: "trip_costs",
  delete_trip_cost: "trip_costs",
  add_policy: "insurance_policies",
  update_policy: "insurance_policies",
  attach_policy: "insurance_policies",
  add_preference: "travel_preferences",
  record_household_fact: "household_facts",
  // Bookkeeping rather than content, but it writes a row, so the validator has
  // to know it: an action missing from this map is refused as "Unknown action".
  set_slot_status: "traveler_slots",
  ...
  add_person: "travelers",
  add_favorite_moment: "favorite_moments",
  delete_favorite_moment: "favorite_moments",
  ...
  // Both of these find their row by what the user called it -- an airport code, a
  // place name -- rather than by an id, because neither list is shown to the model
  // with ids on it. The apply step does the matching against the family's own
  // rows, which is also what makes them safe to call twice.
  save_home_airport: "home_airports",
  remove_home_airport: "home_airports",
  save_someday_place: "someday_places",
  retire_someday_place: "someday_places",
  // These two do go by id, because the fares are the one family-level list that
  // reaches the model with ids on it -- there is no name to match a fare by.
  dismiss_deal: "flight_deals",
  put_deal_on_trip: "flight_deals",
};
...
export const EDIT_TOOLS = new Set([
  "retire_lesson",
  "draw_trip_cover",
  // Attaching names a policy by id and writes a join row rather than a patch,
  // so it needs the id checked like an edit even though the word is not "update".
  "attach_policy",
  // Both change a fare that is already on file. Nothing here inserts one: a fare
  // gets in by being pasted and read, never by being described in a conversation,
  // so there is no path by which Aly can invent a price.
  "dismiss_deal",
  "put_deal_on_trip",
]);
...
export const FAMILY_TABLES = new Set([
  ...
  "lessons",
  "pets",
  // Constraints on a person rather than a trip: they outlive every trip, so they
  // are family-level for the same reason a preference is.
  "household_facts",
  // Where the household flies from and where it wants to go. Both outlive every
  // trip -- that is the point of them -- and both are matched by name rather than
  // by id, so the apply route handles them ahead of this set's generic insert.
  "home_airports",
  "someday_places",
  // Fares the family pasted in. Family-level for the same reason: a fare is a
  // question about the household before it is attached to any one trip.
  "flight_deals",
]);
...
export const TRIP_SCOPED_TABLES = new Set([
  ...
  "trip_templates",
  // Money that is not an event on a day still belongs to exactly one trip.
  "trip_costs",
  // What is carried on one day of one trip. Trip-scoped for the same reason the
  // packing list is, and the day within the trip is a column rather than a scope.
  "day_pack_items",
]);
 ...
 *
 * A number-typed argument does not reliably arrive as a number. A model asked
 * for a budget after somebody said "around $6,000 all in" will hand back
 * "$6,000", "6,000" or "6k" often enough that treating those as nonsense is a
 * bug rather than strictness: Number("$6,000") is NaN, the field is dropped, the
 * patch comes out empty and the family is told no valid changes were given for
 * their trip. So the dollar sign, the commas and a trailing k are read the way a
 * person means them, and only genuine nonsense is dropped.
 */
function cleanDecimal(value, max) {
  ...
  if (typeof value === "number") {
    ...
    n = thousands ? Number(thousands[1]) * 1000 : Number(text);
  }
  if (!Number.isFinite(n) || n < 0 || n > max) return undefined;
...
// Whose a preference is: nobody, one person, or several. Nobody means the whole
// family, which is what "Shared" and anything unrecognized come to, and the
// column stores it as an empty list.
//
// Several matters more than it sounds. "We want a suite so Veda has her own bed"
// used to be saved as the family's, because one owner was all there was, and on a
// trip Veda is not on it then read as a child in the room and quietly ruled out
// every adults-only hotel. So "Mark and Steph" is now a thing Aly can say.
//
// `args.whose` is what the model sends -- one name, several names, or a list.
// `args.traveler_ids` is what comes back when the apply route revalidates an
// action the family has already approved, and `args.traveler_id` is the same
// from before there was a list.
function prefOwners(args, travelerNames, travelerIds) {
  ...
  const fromIds = (raw) => {
    ...
    for (const value of list) {
      const text = String(value || "");
      if (!UUID_RE.test(text) || !allowed.has(text)) continue;
      if (!out.includes(text)) out.push(text);
    }
    return out;
  };
  if (args.traveler_ids !== undefined) return fromIds(args.traveler_ids);
  if (args.traveler_id !== undefined) return fromIds(args.traveler_id);
  if (args.whose === undefined || args.whose === null) return undefined;
  // One field, several names. Aly writes them the way anybody would.
  const said = Array.isArray(args.whose)
    ? args.whose
    : String(args.whose).split(/\s*(?:,|&|\band\b|\+)\s*/i);
  const out = [];
  for (const part of said) {
    const name = matchAssignee(part, travelerNames);
    if (!name || name === "Shared") continue;
    const id = travelerIds.get(name);
    if (id && !out.includes(id)) out.push(id);
  }
  return out;
}

// The same question where only one answer can be right: a person's own phone and
// languages, their email, a loyalty account in their name. A pair holding one
// airline number between them is not a thing, so these stay single.
function prefTraveler(args, travelerNames, travelerIds) {
  const owners = prefOwners(args, travelerNames, travelerIds);
  if (owners === undefined) return undefined;
  return owners.length === 1 ? owners[0] : null;
}

/** The three ways a row about a person can have come to be known. */
const PREF_SOURCES = ["said", "derived", "noticed"];

/** Rule, capability or plain fact -- see the household_facts migration. */
const FACT_KINDS = ["rule", "capability", "fact"];

/** A traveler's name from their id, for the sentence on the approval card. */
function nameForTraveler(id, travelerIds) {
  for (const [n, tid] of travelerIds.entries()) if (tid === id) return n;
  return "one traveler";
}

/** The two columns that spell it, so anything still reading the old one agrees. */
function ownerPatch(owners) {
  if (owners === undefined) return {};
  return {
    traveler_ids: owners,
    traveler_id: owners.length === 1 ? owners[0] : null,
  };
}
...
/**
 * What status a trip being created in this same batch is being created with,
 * by name.
 *
 * The trip does not exist yet, so nothing can be looked up: the only place its
 * status lives is the create_trip call sitting beside the one being checked.
 * Without this, a packing list proposed alongside a brand-new draft looks like
 * a packing list for a trip whose status is unknown, and the refusal happens
 * later -- after the family has approved it.
 */
export function pendingTripStatuses(calls) {
  const out = new Map();
  for (const call of calls || []) {
    const tool = call?.name || call?.tool;
    if (tool !== "create_trip") continue;
    const name = cleanText(call?.args?.name ?? call?.patch?.name, 120);
    const status = cleanText(call?.args?.status ?? call?.patch?.status, 20);
    if (name && !out.has(name)) out.set(name, status || null);
  }
  return out;
}
...
export function validateAction(call, ctx) {
  const {
    ...
    pendingTrips = [],
    pendingTemplates = [],
    // Statuses for those same about-to-exist trips, keyed by name.
    pendingTripStatuses = new Map(),
  ...
  switch (name) {
    ...
    case "update_trip": {
      ...
      const patch = prune({
        ...
        // A preference, and one the app never enforces. Capped high enough that a
        // real trip cannot hit the ceiling and low enough that a fat-fingered
        // figure does not become a million-dollar budget.
        // Read under any of the names it gets called by. The tool declares
        // preferred_budget, the column is budget_target, and the conversation
        // calls it the budget -- a model that picks the wrong one of the three
        // should not produce an empty patch and a refusal.
        budget_target: cleanDecimal(
          args.preferred_budget ?? args.budget_target ?? args.budget,
          2000000,
        ),
      });
      ...
      if (Object.keys(patch).length === 0) {
        // Say which arguments came in and could not be used. A bare "no valid
        // changes" is a dead end: the model cannot tell whether it named the
        // field wrongly or wrote a value the app refused, so it either gives up
        // or tries the same call again.
        const offered = Object.keys(args || {}).filter(
          (k) => k !== "trip" && k !== "trip_id" && k !== "id",
        );
        return {
          error: offered.length
            ? `Nothing in that change could be used for the trip. You sent ${offered.join(", ")}. Check the field names against the tool and that each value is a plain figure or a sentence, then try once more.`
            : "No valid changes were given for that trip.",
        };
      }
      return {
        action: {
    ...
    case "update_itinerary_item": {
      const patch = prune({
        ...
        cost_estimate: cleanDecimal(args.cost_estimate, 2000000),
        cost_actual: cleanDecimal(args.cost_actual, 2000000),
        cost_note: cleanText(args.cost_note, 200),
      });
          ...
          action: {
            ...
            summary: `Add "${patch.title}" to the itinerary on ${formatFullDay(patch.item_date) || patch.item_date}${
              patch.start_time
                ...
                : ""
            }${on}${statusNote(patch.status)}${costNote(patch)}`,
          },
        };
      }
      if (Object.keys(patch).length === 0)
    ...
    case "start_packing_list": {
      ...
      // Nothing packs for a draft, and this is where that has to be said. The
      // rule was already enforced where the list is actually written, which
      // meant a model that offered it anyway -- on the trip builder, where the
      // trip being created is usually a draft -- produced a card the family
      // could tap and an error where the list should have been. Refusing here
      // means the card is never drawn, and `tell` carries the reason up so the
      // reply says why instead of quietly losing a promise it just made.
      const status = newTripName
        ? pendingTripStatuses.get(newTripName)
        : known.tripStatus?.get(tripId);
      if (status === "draft") {
        return {
          error: draftPackingWords(newTripName || known.trips?.get(tripId)),
          tell: true,
        };
      }
      return {
    ...
    // A day's bag rather than the case. Kept apart from add_packing_item on
    // purpose: the two ticks make different claims, and a line that says which
    // suitcase row it came out of is the only way "carry the rain shell" reads as
    // the shell that was packed rather than a second one.
    case "add_day_pack_item": {
      const item = cleanText(args.item, 200);
      if (!item) return { error: "Something carried needs a name." };
      const when = cleanDate(args.item_date);
      // The link to the case, and it has to be a row on this same trip. A model
      // reaching for an id it half-remembers would otherwise tie a Curaçao towel
      // to an Alaska morning, and the family would never see it.
      let fromCase = cleanText(args.from_packing_id, 40);
      if (fromCase && !UUID_RE.test(fromCase)) fromCase = null;
      if (fromCase && !known.packing_items?.has(fromCase)) fromCase = null;
      if (fromCase && tripId && known.rowTrip?.get(fromCase) !== tripId)
        fromCase = null;
      // And if she did not name one, look it up by name rather than trusting her
      // to have looked. The family's rule is that nothing is carried without also
      // being on the trip list, so this is not an optimisation -- it decides
      // whether the card says "out of the case" or "and onto the packing list",
      // which is the difference between remembering something and buying it.
      if (!fromCase) fromCase = matchKnownCaseId(known, { tripId, item });
      const patch = prune({
        item,
        item_date: when || undefined,
        assignee: matchAssignee(args.assignee, travelerNames) || "Shared",
        why: cleanText(args.why, 300),
        from_packing_id: fromCase || undefined,
        // Hers, not theirs, so a family clearing her guesses never loses a line
        // they wrote themselves.
        source: "aly",
      });
      Object.assign(patch, tripRef(tripId, newTripName));
      const day = when ? formatFullDay(when) : null;
      return {
        action: {
          tool: name,
          table,
          patch,
          ...pendingOn,
          summary: `Carry ${patch.item} ${
            day ? `on ${day}` : "every day"
          } \u2014 ${patch.assignee}${
            fromCase ? ", out of the case" : ", and onto the packing list"
          }${on}`,
        },
      };
    }
    ...
    case "delete_packing_item": {
      // Off the list means off the trip, which means off the days somebody was
      // carrying it on. The database does that; the card has to say it, because
      // "remove the rain shell" and "remove the rain shell and take it out of
      // Wednesday's bag" are different things to agree to.
      const carriedOn = known.dayPackFor?.get(id) || [];
      const alsoDays = carriedOn
        .map((day) => (day === "every day" ? "every day" : formatFullDay(day)))
      ...
      return {
        action: {
          ...
          summary: `Remove ${label("an item")} from the packing list${on}${
            alsoDays.length
              ? `, and out of the day pack for ${alsoDays.join(", ")}`
              : ""
          }`,
        },
      };
    ...
    // Travel insurance. Everything here is transcription: the model is putting
    // down what the family said, or what a document reader already extracted, and
    // the one rule it must not break is inventing a figure -- a medical limit
    // nobody has read is worse than an empty field, because an empty field gets
    // looked up and a wrong one gets relied on.
    case "add_policy": {
      const provider = cleanText(args.provider, 140);
      if (!provider) {
        return { error: "Say who the policy is with and I will keep it." };
      }
      const covers = normalizeCovers(args.covers);
      const patch = prune({
        provider,
        plan_name: cleanText(args.plan_name, 140),
        policy_number: cleanText(args.policy_number, 80),
        kind: POLICY_KINDS.some((k) => k.value === args.kind)
          ? args.kind
          : undefined,
        coverage_start: cleanDate(args.coverage_start),
        coverage_end: cleanDate(args.coverage_end),
        covers: covers.length ? covers : undefined,
        emergency_phone: cleanText(args.emergency_phone, 60),
        claims_phone: cleanText(args.claims_phone, 60),
        claims_url: cleanText(args.claims_url, 300),
        deductible: cleanDecimal(args.deductible, 200000),
        medical_limit: cleanDecimal(args.medical_limit, 100000000),
        evacuation_limit: cleanDecimal(args.evacuation_limit, 100000000),
        notes: cleanText(args.notes, 1000),
      });
      if (
        patch.coverage_start &&
        patch.coverage_end &&
        patch.coverage_end < patch.coverage_start
      ) {
        return { error: "That policy would end before it starts." };
      }
      // Which trip it lands on, when one is open. A policy needs no trip -- an
      // annual plan bought in January belongs to the family until a trip exists
      // for it -- so a missing trip is not a refusal here, unlike a packing item.
      const attach = focusTripId || null;
      return {
        action: {
          tool: name,
          table,
          patch,
          ...(attach ? { attachTrip: attach } : {}),
          summary: `Keep the ${provider} policy${
            covers.length
              ? ` (${covers.map(coverLabel).slice(0, 3).join(", ")})`
              : ""
          }${attach && known.trips?.get(attach) ? ` on ${known.trips.get(attach)}` : ""}`,
        },
      };
    }
    ...
    case "update_policy": {
      const covers = normalizeCovers(args.covers);
      const patch = prune({
        provider: cleanText(args.provider, 140),
        plan_name: cleanText(args.plan_name, 140),
        policy_number: cleanText(args.policy_number, 80),
        kind: POLICY_KINDS.some((k) => k.value === args.kind)
          ? args.kind
        ...
        coverage_start: cleanDate(args.coverage_start),
        coverage_end: cleanDate(args.coverage_end),
        covers: Array.isArray(args.covers) ? covers : undefined,
        emergency_phone: cleanText(args.emergency_phone, 60),
        claims_phone: cleanText(args.claims_phone, 60),
        claims_url: cleanText(args.claims_url, 300),
        deductible: cleanDecimal(args.deductible, 200000),
      ...
      if (!Object.keys(patch).length) {
        return { error: "Nothing to change on that policy." };
      }
      return {
        action: {
          tool: name,
          ...
          id,
          patch,
          summary: `Change ${clip(label("that policy"), 60)}: ${describePatch(patch)}`,
        },
      };
    }
    ...
    case "attach_policy": {
      ...
      if (!attach) {
        return {
          error:
            "Open the trip you want that policy on and I will attach it there.",
        };
      }
      return {
        action: {
          ...
          attachTrip: attach,
          patch: {},
          summary: `Say ${clip(label("that policy"), 60)} covers ${
            known.trips?.get(attach) || "this trip"
          }`,
        },
      };
    }
    ...
    case "add_trip_cost": {
      const label = cleanText(args.label, 140);
      if (!label) return { error: "A cost needs to say what it is for." };
      const patch = prune({
        label,
        category: COST_CATEGORIES.includes(args.category)
          ? args.category
          : undefined,
        cost_estimate: cleanDecimal(args.cost_estimate, 2000000),
        cost_actual: cleanDecimal(args.cost_actual, 2000000),
      ...
      if (
        patch.cost_estimate === undefined &&
        patch.cost_actual === undefined
      ) {
        return {
          error: `I have nothing to put against “${clip(label, 60)}”. Tell me roughly what it comes to and I will add it as an estimate.`,
        };
      }
      return {
        action: {
          ...
          patch,
          ...pendingOn,
          summary: `Add “${clip(label, 60)}” to the budget${on}${costNote(patch)}`,
        },
      };
    }
    ...
    case "update_trip_cost": {
      const known_label =
        (known.trip_costs || new Map()).get(id) || "that cost";
      const patch = prune({
        label: cleanText(args.label, 140),
        category: COST_CATEGORIES.includes(args.category)
          ? args.category
          : undefined,
        cost_estimate: cleanDecimal(args.cost_estimate, 2000000),
      ...
      if (!Object.keys(patch).length)
        return { error: "Nothing to change on that cost." };
      return {
        action: {
          tool: name,
          table,
          id,
          patch,
          summary: `Change “${clip(known_label, 60)}”${on}: ${describePatch(patch)}`,
        },
      };
    }

    case "delete_trip_cost": {
      const gone = (known.trip_costs || new Map()).get(id) || "that cost";
      return {
        action: {
          tool: name,
          table,
          id,
          destructive: true,
          summary: `Remove “${clip(gone, 60)}” from the budget${on}`,
        },
      };
    }
    ...
    case "set_person_details": {
      ...
      // Their provider, folded onto one spelling per provider. A name we do not
      // know is kept as she heard it, so a small carrier is not renamed to the
      // nearest big one.
      if (args.phone_carrier !== undefined) {
        const value = normalizeCarrier(cleanText(args.phone_carrier, 60));
        patch.phone_carrier = value;
        said.push(value ? `on ${value}` : "no carrier");
      }
    ...
    case "update_preference": {
      ...
      const patch = prune({
        ...
        // Where the row came from, which is the difference between a constraint
        // and a guess. Anything the model sends that is not one of the three
        // words falls back to said only on an add, because a row with no source
        // at all reads as though somebody stated it.
        source: cleanEnum(args.source, PREF_SOURCES),
        slot: cleanEnum(args.slot, SLOT_IDS),
        // Their own words for why, kept verbatim. 600 rather than 1000: a reason
        // longer than this is an answer to a different question.
        reason: cleanText(args.reason, 600),
      });
      Object.assign(patch, ownerPatch(owners));
      if (askedTopics) {
      ...
      const whoseName =
        owners === undefined
          ...
          : owners.length === 0
            ? "the whole family"
            : owners.map(nameForId).join(" and ");

      if (name === "add_preference") {
        if (!patch.body)
          return { error: "A travel preference needs something to say." };
        if (!patch.source) patch.source = "said";
        return {
          action: {
            tool: name,
            table,
            patch,
            // The card says out loud when this is her inference rather than
            // their words, because a family approving “we would rather move
            // than look” should know whether they ever said it.
            summary: `${
              patch.source === "said" ? "Save" : "Save what you worked out as"
            } a travel preference${topicsSaid ? ` under ${topicsSaid}` : ""}${
              whoseName && whoseName !== "the whole family"
                ? ` for ${whoseName}`
                : ""
            }: “${clip(patch.body, 90)}”${
              patch.reason ? ` — because “${clip(patch.reason, 70)}”` : ""
            }`,
          },
        };
      }
    ...
    case "record_household_fact": {
      const owners = prefOwners(args, travelerNames, travelerIds);
      const patch = prune({
        body: cleanText(args.body, 600),
        kind: cleanEnum(args.kind, FACT_KINDS),
        slot: cleanEnum(args.slot, SLOT_IDS),
        source: cleanEnum(args.source, PREF_SOURCES),
        traveler_id:
          owners === undefined
      ...
      if (!patch.body) return { error: "That fact needs something to say." };
      // Deliberately not defaulted. A row that must be obeyed and a row that is
      // merely true are treated differently by every reader of this table, and
      // guessing which one she meant is how an allergy becomes a footnote.
      if (!patch.kind)
        return {
          error:
            "Say whether that is a rule, a capability or a plain fact, and I will keep it.",
        };
      if (!patch.source) patch.source = "said";
      const whose =
        owners === undefined || owners.length !== 1
          ? "the household"
          : nameForTraveler(owners[0], travelerIds);
      const heading =
        patch.kind === "rule"
          ? "Keep as a rule"
          : patch.kind === "capability"
            ? "Keep as what is possible"
            : "Keep as a fact";
      return {
        action: {
          tool: name,
          table,
          patch,
          summary: `${heading} for ${whose}: “${clip(patch.body, 90)}”`,
        },
      };
    }

    case "set_slot_status": {
      const slot = cleanEnum(args.slot, SLOT_IDS);
      // Settled is not hers to declare. A slot counts as answered because a row
      // exists that answers it, so letting her assert it would let the ledger
      // drift away from the data the family can actually see and delete.
      // Neither settled nor asking is hers to declare. A slot counts as answered
      // because a row exists that answers it, and it counts as asked because the
      // app watched her ask it -- the alternative was a reply with no words in it,
      // the question having gone into this argument instead of to the person.
      const status = cleanEnum(
        args.status,
        SLOT_STATUSES.filter((s) => s !== "settled" && s !== "asking"),
      );
      if (!slot)
        return { error: "I did not recognize which question that was about." };
      if (!status)
        return {
          error:
            "I could not tell what to do with that question, so I left it.",
        };
      const owners = prefOwners(args, travelerNames, travelerIds);
      const travelerId =
        owners === undefined ? null : owners.length === 1 ? owners[0] : null;
      const whose = travelerId
        ? nameForTraveler(travelerId, travelerIds)
        : "the household";
      const note = cleanText(args.note, 300);
      const patch = prune({
        slot,
      ...
      return {
        action: {
          ...
          summary:
            status === "skipped"
              ? `Stop asking ${whose} about ${slotLabel(slot).toLowerCase()}${
                  note ? ` — ${clip(note, 70)}` : ""
                }`
              : `Put ${slotLabel(
                  slot,
                ).toLowerCase()} back on the list for ${whose}`,
        },
      };
    }

    case "add_favorite_moment": {
      const travelerId = prefTraveler(args, travelerNames, travelerIds);
      let who = null;
      for (const [n, tid] of travelerIds.entries())
        if (tid === travelerId) who = n;
      if (!travelerId || !who || who.toLowerCase() === "shared") {
        return {
          error: `Tell me whose moment that is. The people on this family are ${travelerNames
            .filter((n) => n.toLowerCase() !== "shared")
            .join(", ")}.`,
        };
      }
      const body = cleanText(args.body, 1200);
      if (!body) {
        return {
          error:
            "A favorite moment needs the moment in the user's own words. A phrase like 'a slow dinner on the terrace in Rome' is enough.",
        };
      }
      return {
        action: {
          tool: name,
          table,
          patch: { traveler_id: travelerId, body },
          summary: `Save a favorite moment for ${who}: ${clip(body, 80)}`,
        },
      };
    }

    case "delete_favorite_moment": {
      const id = String(args.id || "").trim();
      if (!id) return { error: "Which moment? I need the row's id." };
      return {
        action: {
          tool: name,
          table,
          id,
          patch: {},
          summary: "Remove a favorite moment",
        },
      };
    }

    case "add_person": {
      const clean = cleanText(args.name, 60);
      if (!clean) return { error: "A person needs a name." };
      // Somebody who is already on the family's list is not a new person. The
      // model gets shown who is on file above; still worth defending against a
      // repeat here because a duplicate row would silently split the file
      // between two people with the same name.
      const already = (travelerNames || []).find(
        (n) => n && n.toLowerCase() === clean.toLowerCase(),
      );
      if (already) {
        return {
          error: `${already} is already on this family's list. Nothing more to add — use set_person_details to say more about them.`,
          tell: true,
        };
      }
      if (clean.toLowerCase() === "shared") {
        return {
          error:
            "Shared is not a person — it is where the family's shared belongings live.",
          tell: true,
        };
      }
      const rel = cleanText(args.relationship, 60);
      return {
        action: {
          tool: name,
          table,
          patch: { name: clean, is_person: true },
          summary: rel
            ? `Add ${clean} (${rel}) to the family`
            : `Add ${clean} to the family`,
        },
      };
    }
    ...
    case "update_pet": {
      ...
      if (name === "add_pet") {
        ...
        // An animal already on file is not a new animal. An interview turn about
        // who looks after the horse produced add_pet("Storm", dog) while Storm
        // the horse was sitting in the same context, which would have given the
        // family two Storms and made one of them the wrong species.
        const already = [...(known.pets || new Map()).entries()].find(
          ([, petName]) =>
            String(petName || "")
              .trim()
              .toLowerCase() === patch.name.trim().toLowerCase(),
        );
        if (already) {
          return {
            error: `${already[1]} is already one of the family's animals. Use update_pet to change what is known about them, and set_pet_trip to say whether they are travelling.`,
            tell: true,
          };
        }
    ...
    case "save_home_airport": {
      ...
      if (!airport) {
        return {
          error: `I do not have an airport with the code ${
            clip(code, 12) || "you gave"
          }, so I did not save it. The list covers the United States and Canada -- give me the three-letter code, or the city, and I will find it.`,
        };
      }

      // A drive time is only ever the family's own. Reading it here rather than
      // letting the column take whatever arrived means "about two hours" becomes
      // 120 and "quite a way" becomes a question, instead of a null nobody
      // notices until a fare from four hours away looks like a bargain.
      let drive;
      if (args.drive_time !== undefined && args.drive_time !== null) {
        const minutes = driveMinutesFrom(String(args.drive_time));
        if (minutes === null) {
          return {
            error: `I could not read "${clip(
              String(args.drive_time),
              40,
            )}" as a drive time, so I have not saved ${code}. Say it as minutes or hours -- "25 minutes", "4 hours", "2 hr 15" -- or leave it out and I will save the airport without one.`,
          };
        }
        if (minutes > 1440) {
          return {
            error: `${driveSaid(
              minutes,
            )} is longer than a day's drive, so I have not saved that as the drive to ${code}.`,
          };
        }
        drive = minutes;
      }

      const primary = cleanBool(args.home_base);
      const patch = prune({
        code,
        name: cleanText(airport.name, 120),
        city: cleanText(airport.city, 80),
        region: cleanText(airport.region, 40),
        lat: airport.lat,
        lon: airport.lon,
        drive_minutes: drive,
        is_primary: primary,
      ...
      const bits = [];
      if (drive !== undefined) bits.push(`${driveSaid(drive)} away`);
      if (primary === true) bits.push("the airport fares are priced from");
      return {
        action: {
          tool: name,
          table,
          patch,
          summary: `Fly from ${code}, ${airportPlace(airport)}${
    ...
    case "remove_home_airport": {
      const code = String(args.code || "")
        .trim()
        .toUpperCase();
      if (!/^[A-Z]{3}$/.test(code)) {
        return {
          error: `I need the three-letter code of the airport to take off the list, and "${clip(
            String(args.code || ""),
            20,
          )}" is not one.`,
        };
      ...
      return {
        action: {
          ...
          patch: { code },
          destructive: true,
          summary: `Stop pricing fares from ${code}${
            airport ? `, ${airportPlace(airport)}` : ""
          }`,
        },
      };
    }

    case "save_someday_place": {
      const place = cleanText(args.place, 120);
      if (!place) {
        return {
          error:
            "I need the name of the place to put it on the bucket list, and none came through.",
        };
      }

      const months =
        args.months === undefined ? undefined : parseMonths(args.months);
      const owners = prefOwners(
        { whose: args.who, traveler_ids: args.traveler_ids },
        travelerNames,
        travelerIds,
      );
      ...
      const patch = prune({
        place,
        region: cleanText(args.region, 80),
        why: cleanText(args.why, 600),
        months,
        nights: cleanCount(args.nights, 365),
        // Per person, and the number every verdict on a fare is measured
        // against, so a cent is as precise as it gets and a thousand-dollar
        // ceiling is the top of what this is for.
        fare_ceiling: cleanDecimal(args.fare_ceiling, 100000),
        traveler_ids: owners,
        watch: cleanBool(args.watch),
      });

      const bits = [];
      if (months && months.length && months.length < 12)
        bits.push(`in ${monthsSaid(months)}`);
      if (patch.nights !== undefined)
        bits.push(`${patch.nights} night${patch.nights === 1 ? "" : "s"}`);
      if (patch.fare_ceiling !== undefined)
        bits.push(
          `worth doing under $${Number(patch.fare_ceiling)} each in airfare`,
        );
      if (owners && owners.length)
        bits.push(
          `for ${owners
            .map((id) => nameForTraveler(id, travelerIds))
            .join(" and ")}`,
        );
      if (patch.watch === false) bits.push("without watching for fares");
      ...
      return {
        action: {
          ...
          table,
          patch,
          summary: `Put ${place} on the bucket list${
    ...
    case "retire_someday_place": {
      const place = cleanText(args.place, 120);
      if (!place) {
        return {
          error:
            "I need the name of the place to take off the bucket list, and none came through.",
        };
      }
      return {
        action: {
          tool: name,
          table,
          patch: { place, status: "retired" },
          destructive: true,
          summary: `Take ${place} off the bucket list`,
        },
      };
    }
    ...
    case "dismiss_deal": {
      ...
      return {
        action: {
          ...
          table,
          id,
          patch: prune({ status: "dismissed", dismissed_reason: reason }),
          summary: `Turn down ${label("that fare")}${reason ? ` — ${reason}` : ""}`,
        },
      };
    }

    case "put_deal_on_trip": {
      // The trip is matched against the ones she was shown rather than created,
      // because a fare landing on a trip that does not exist yet is two
      // decisions dressed as one.
      const said = cleanText(args.trip, 120);
      const wanted = String(said || "")
        .trim()
        .toLowerCase();
      let onTrip = null;
      for (const [tripKey, tripName] of known.trips || new Map()) {
        if (
          String(tripName || "")
            .trim()
            .toLowerCase() === wanted
      ...
      if (!onTrip) {
        return {
          error: said
            ? `I could not find a trip called ${said}, so I did not put that fare on anything.`
            : "Say which trip that fare is for and I will put it on.",
        };
      }
      return {
        action: {
          ...
          table,
          id,
          patch: { status: "taken", trip_id: onTrip },
          summary: `Put ${label("that fare")} on ${known.trips.get(onTrip)}`,
        },
      };
    }
...
const FIELD_LABELS = {
  ...
  budget_target: "preferred budget",
  cost_estimate: "estimated cost",
  cost_actual: "final cost",
  cost_note: "what the figure covers",
};

// The columns that hold dollars, so the line the family approves says $2,400
// rather than 2400 -- which reads like a quantity, and on a screen full of
// counts and quantities that is exactly how it was being read.
const MONEY_FIELD = new Set(["budget_target", "cost_estimate", "cost_actual"]);
...
/**
 * The money on a line, for the card the family approves.
 *
 * An estimate is said as an estimate. A figure offered without that word is a
 * figure the family will take for a price somebody checked, and Aly's are
 * usually neither checked nor checkable.
 */
function costNote(patch) {
  if (patch.cost_actual !== undefined && patch.cost_actual !== null)
    return `, ${money(patch.cost_actual)}`;
  if (patch.cost_estimate !== undefined && patch.cost_estimate !== null)
    return `, about ${money(patch.cost_estimate)} estimated`;
  return "";
}
...
function describePatch(patch) {
  return Object.entries(patch)
    ...
    .map(([k, v]) => {
      ...
      if (MONEY_FIELD.has(k)) return `${label} → ${money(v) || v}`;
      return `${label} → ${v}`;
    })
    .join(", ");
}
