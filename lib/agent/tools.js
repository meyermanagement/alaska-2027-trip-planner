...
export const SHARED_TOOL_DECLARATIONS = [
  ...
  },
  {
    ...
    description:
      'Mark where a question stands, so the app knows what has been covered and never hands you the same thing again. Call it with "asking" in the same reply as the question you just put, and with "skipped" the moment somebody waves one away -- with their reason in the note, because a no with a reason is worth more than a blank. Do not use "settled": saving a preference or a fact against the slot settles it on its own. This changes no advice by itself; it is the bookkeeping that lets an interview end.',
    parameters: {
      type: "object",
      properties: {
        slot: {
        ...
        status: {
          type: "string",
          description:
            'One of: "asking" when you have just put the question and are waiting; "skipped" when they declined, said the question does not divide them, or changed the subject twice; "open" to put it back.',
        },
        whose: {
          type: "string",
          description:
        ...
        last_question: {
          type: "string",
          description:
            "The question exactly as you asked it, when the status is asking, so a later conversation does not ask it again in the same words.",
        },
        note: {
          type: "string",
          description:
  ...
  switch (name) {
    ...
    case "set_slot_status": {
      ...
      const status = cleanEnum(
        args.status,
        SLOT_STATUSES.filter((s) => s !== "settled"),
      );
      if (!slot)
      ...
      const patch = prune({
        ...
        last_question: cleanText(args.last_question, 300),
        note,
      });
      return {
        action: {
          ...
          summary:
            status === "skipped"
              ...
              : status === "asking"
                ? `Note that ${whose} has been asked about ${slotLabel(
                    slot,
                  ).toLowerCase()}`
                : `Put ${slotLabel(
                    slot,
