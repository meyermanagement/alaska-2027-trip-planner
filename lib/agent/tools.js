    ...
    case "update_template_item": {
      ...
      if (name === "add_template_item") {
        ...
        if (templateId === null) {
          return {
            error:
              "I could not tell which standing packing list you meant, so I did not add that.",
          };
        }
        if (templateId === undefined) {
          return {
            error:
              "There is no standing packing list to add that to yet. Name the list and I will add it.",
          };
        }
        // An unassigned item belongs to the whole family.
        if (!patch.assignee) patch.assignee = "Shared";
