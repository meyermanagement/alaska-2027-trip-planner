...
import { SPANNING_CATEGORIES } from "../format";
...
const itineraryFields = {
  ...
  end_date: {
    type: "string",
    description:
      "Only for lodging and cruise items that run over more than one day: the check-out date, or the last day of the cruise, in YYYY-MM-DD. Must be after item_date. Never set this on a flight, meal, tour or note, and leave it out for a single-night stay that has no check-out date yet. Prefer one item with a date range over a separate 'check out' item.",
  },
  start_time: {
    type: "string",
    description:
    ...
    case "update_itinerary_item": {
      ...
      if (patch.end_date) {
        ...
        if (patch.item_date && patch.end_date <= patch.item_date)
          return {
            error: `The check-out date has to fall after ${patch.item_date}.`,
          };
      }
