...
export const SHARED_TOOL_DECLARATIONS = [
  ...
  },
  {
    ...
    description:
      'Put a travel rewards programme on the Rewards tab: an airline, a hotel chain, a cruise line past-guest club, a car rental club, or a credit card the family carries. Call this when the user says they belong to something or carry a card ("we have Marriott Bonvoy", "I put everything on the Sapphire Reserve"). For a credit card, always fill in earn_rules and program_name so the app can work out which card a booking should go on. Never invent a points balance or a membership number.',
    parameters: {
      type: "object",
      properties: {
        brand: {
        ...
        program_name: {
          type: "string",
          description:
            "For a credit card, the currency its points land in: 'Chase Ultimate Rewards', 'Delta SkyMiles', 'Marriott Bonvoy points'. Leave out for a programme that is its own currency.",
        },
        currency_label: {
          type: "string",
          description:
            "What the programme calls its points, for reading back: 'points', 'miles', 'Rapid Rewards points'.",
        },
        points_balance: {
          type: "integer",
          description:
        ...
        status_tier: {
          type: "string",
          description:
            "Their elite tier if they mention one, in the programme's own words: 'Gold Elite', 'Explorist', '4-Star Mariner'.",
        },
        member_number: {
          type: "string",
  ...
  },
  {
    ...
    description:
      "Change a saved rewards programme — most often a new points balance the user just told you. Use the exact id from the Rewards section of the context, and include only what changes. When you set a balance, also set points_checked_on to today so the app can say how fresh it is.",
    parameters: {
      type: "object",
      properties: {
        id: { type: "string" },
  ...
  },
  {
    ...
    description:
      "Take a rewards programme off the list — a card they closed or an account they no longer use. Use the exact id from the context.",
    parameters: {
      type: "object",
      properties: { id: { type: "string" } },
      required: ["id"],
  ...
  switch (name) {
    ...
    case "update_rewards_program": {
      ...
      if (name === "add_rewards_program") {
        if (!patch.brand) return { error: "A rewards programme needs a name." };
        if (!patch.kind)
          return {
            error: `I could not tell what kind of programme ${patch.brand} is, so I did not add it.`,
          };
        return {
          action: {
            tool: name,
            table,
            patch,
            summary: `Add ${patch.brand} to Rewards${balanceBit}${earnBit}`,
          },
        };
      }
      ...
      if (Object.keys(patch).length === 0)
        return {
          error: "No valid changes were given for that rewards programme.",
        };
      const bits = [];
      ...
      return {
        action: {
          ...
          id,
          patch,
          summary: `Update ${label("that rewards programme")}: ${bits.join(", ")}`,
        },
      };
    }
    ...
    case "delete_rewards_program":
      return {
        action: {
          ...
          id,
          destructive: true,
          summary: `Remove ${label("that rewards programme")} from Rewards`,
        },
      };
