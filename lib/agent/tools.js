...
export const SHARED_TOOL_DECLARATIONS = [
    ...
    parameters: {
      ...
      properties: {
        ...
        copy_from_list: {
          type: "string",
          description:
            "Fill the new list from another standing list, by its exact name. Use this when the user wants part of an existing list split out into its own.",
        },
        only_categories: {
          type: "string",
          description:
  ...
  },
  {
    ...
    description:
      "Add one item to a STANDING packing list — the lists every future trip is built from, not any one trip's list. Use this when the user is talking about what the family always takes, or is on the Packing lists screen. Call once per distinct item. This changes nothing on trips that already exist.",
    parameters: {
      type: "object",
      properties: {
        item: { type: "string" },
        ...
        list: {
          type: "string",
          description:
            "Which standing list, by its exact name from the PACKING LISTS context. Leave it out for the base list that every trip starts from. Put gear specific to one kind of trip on the matching add-on list instead of the base.",
        },
      },
      required: ["item"],
    },
  },
  {
    ...
    description:
      "Change an item on a standing packing list: rename it, move it to another person, or change its category or quantity. Use the exact id from the PACKING LISTS context.",
    parameters: {
      type: "object",
      properties: {
        id: { type: "string" },
  ...
  },
  {
    ...
    description:
      "Take an item off a standing packing list so it stops appearing on future trips. Use the exact id from the PACKING LISTS context.",
    parameters: {
      type: "object",
      properties: { id: { type: "string" } },
      required: ["id"],
    ...
    parameters: {
      ...
      properties: {
        ...
        travelers: {
          ...
          items: { type: "string" },
          description:
            "Who is actually going, by name, when it is not the whole family. Leave it out when everyone is going. This trims the packing list to those people and whatever is shared, so nobody packs for someone who stayed home.",
        },
        copy_base_packing: {
          type: "boolean",
          description:
...
// Rows that belong to a standing packing list rather than to a trip. A write
 ...
 */
// Which standing packing list an item goes on. Nothing named means the base
...
export function validateAction(call, ctx) {
  ...
  if (needsId) {
    ...
    if (!pool.has(id)) {
      if (TEMPLATE_TABLES.has(table)) {
        return {
          error:
            "I could not find that on any of your standing packing lists, so I did not change it.",
        };
      }
      return {
        error:
  ...
  switch (name) {
    ...
    // The standing lists, not any trip's list. Everything here is deliberately
    ...
    case "create_template": {
      ...
      if (!wanted) return { error: "A standing packing list needs a name." };
      const templates = known.packing_templates || new Map();
      for (const [, t] of templates.entries()) {
        if (String(t?.name || "").toLowerCase() === wanted.toLowerCase()) {
          return {
            error: `There is already a standing list called “${clip(wanted, 60)}”, so I did not start another. I can add to that one instead.`,
          };
        }
      }
      ...
      if (listId || tripId) {
        ...
        if (listId) {
          if (!templates.has(listId)) {
            return {
              error:
                "That standing list is no longer there to copy from, so I did not start a new one.",
            };
          }
          patch.copy_from_template_id = listId;
          source = ` from ${templates.get(listId)?.name || "another standing list"}`;
        } else {
          if (!known.trips?.has(tripId)) {
            return {
              error:
                "That trip is no longer there to copy its packing list, so I did not start a new standing list.",
            };
          }
        ...
        if (!sourceId) {
          return {
            error: `I could not find a standing list called “${clip(fromList, 60)}” to copy from, so I did not start a new one.`,
          };
        }
        patch.copy_from_template_id = sourceId;
        source = ` from ${templates.get(sourceId)?.name || "another standing list"}`;
      } else if (fromTrip) {
        ...
        if (resolved && typeof resolved === "object") {
          return {
            error: `“${clip(resolved.pending, 60)}” is not saved yet and has no packing list to copy, so I did not start a new standing list from it.`,
          };
        }
        if (!resolved) {
          return {
            error: here
              ...
              : `I could not find a trip called “${clip(fromTrip, 60)}” to copy its packing list, so I did not start a new standing list.`,
          };
        }
      ...
      return {
        action: {
          ...
          patch,
          summary: source
            ? `Start a standing packing list “${clip(wanted, 60)}”${source}${only}`
            : `Start an empty standing packing list “${clip(wanted, 60)}”`,
        },
      };
    }
    ...
    case "update_template_item": {
      ...
      const listName = (tid) =>
        known.packing_templates?.get(tid)?.name || "the standing list";
      ...
      if (name === "add_template_item") {
        ...
        if (templateId === null) {
          const have = Array.from(known.packing_templates?.values() || [])
            ...
            .filter(Boolean);
          return {
            error: `There is no standing packing list called “${clip(
        ...
        if (templateId === undefined) {
          return {
            error:
              "There are no standing packing lists yet. Start one with create_template and I will put that on it.",
          };
        }
        // An unassigned item belongs to the whole family.
        if (!patch.assignee) patch.assignee = "Shared";
      ...
      return {
        action: {
          ...
          summary: `Stop always packing ${
            ...
            row?.template_id
              ? ` — ${known.packing_templates?.get(row.template_id)?.name || "standing list"}`
              : ""
          }`,
        },
      };
      ...
      if (name === "add_rewards_program") {
        ...
        return {
          action: {
            ...
            table,
            patch,
            summary: `Add ${patch.brand} to Travel programs${balanceBit}${earnBit}${creditBit}`,
          },
        };
      }
    ...
    case "delete_rewards_program":
      return {
        action: {
          ...
          id,
          destructive: true,
          summary: `Remove ${label("that travel program")} from Travel programs`,
        },
      };
