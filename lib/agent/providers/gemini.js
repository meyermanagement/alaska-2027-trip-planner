...
//   gemini-3.7-flash       not broken: busy. Measured again 2026-08-31 against
//                          this project's own key it returns Google's own
//                          sentence, "This model is currently experiencing high
//                          demand ... please try again later" — and the refusal
//                          log for 08-27 shows it returning 429 on a spent
//                          free-tier daily allowance. The hundred seconds of
//                          silence and the empty 502 recorded on 08-27, which is
//                          what got it taken out, are what being over capacity
//                          looks like when the request is left waiting for it. It
//                          was first in this list and retried three times before
//                          anything else was tried, which is exactly why every
//                          look was timing out. Ask /model-check about it: when it
//                          answers, put it back — first, or anywhere — with
//                          GEMINI_MODELS.
...
export const SHELVED_MODELS = ["gemini-3.7-flash", "gemini-3.1-flash-lite"];
  ...
  outer: for (let modelIndex = 0; modelIndex < models.length; modelIndex++) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      try {
        ...
        skipped(
          504,
          `No reply within ${Math.min(ATTEMPT_CAP_MS, remaining)}ms (${
 ...
 * not in the ladder: gemini-3.7-flash was taken out on 27 August after a hundred
 * seconds of silence and an empty 502, and the refusal log for that same morning
 * also shows it returning 429 on an allowance called
 * generate_content_free_tier_requests -- which is a spent daily allowance rather
 * than a broken model, and a spent daily allowance comes back tomorrow.
 *
