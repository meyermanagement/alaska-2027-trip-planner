...
import { logPrefix } from "../fingerprint";
...
// Fastest first, then the newest, then the model we ran on for months, then the
// cheap small one. Order is measured rather than assumed: see the notes below.
//
// Deliberately absent:
...
//
// gemini-3.7-flash is back at the head of the ladder as of 2026-09-03, having
// been out since 27 August.
...
//
// gemini-3.8-flash joined it on 2026-09-03, and sits second rather than first
// despite being the newest name Google lists. Measured in the same minute as 3.7,
// twice over: the same three tool-calling probes took 1.7, 2.1 and 5.6 seconds
// against 3.7's 1.2, 1.2 and 1.5, and the same grounded search took 5.8 and 12.1
// seconds against 3.2 and 6.4. It is not wrong -- every probe produced the right
// call with the right trip id, and it refused to invent an id for a row that does
// not exist, naming the two rows it could actually see. It is just slower today,
// and the first rung is the one the family waits on. Second is where being newer
// and correct is worth something: it is the fallback that gets asked when 3.7 is
// busy, in place of a model two versions older.
//
// Moved to first on 2026-09-25. The model check's 2026-09-25 run, on made-up
// data, had 3.8 level with or ahead of 3.7 on quality in all six request types
// (ask 100% against 67%) and faster in five of them (fares the exception). 3.7
// stays second. Launch pricing on 3.8 ends 2026-12-31; revisit then.
...
export const SHELVED_MODELS = ["gemini-3.1-flash-lite"];
...
// How free the model is to call the tools it has been shown.
//
// AUTO is the ordinary turn. An explicit list becomes VALIDATED, which still
// lets the model answer in words but limits any call to the names given --
// ANY would force a call, and a finishing turn is usually owed words. An empty
// list becomes NONE: no calls at all, the declarations left where they were.
// Names that were never declared are dropped, since the API refuses them.
export function callingConfig(tools, allowedTools) {
  if (!Array.isArray(allowedTools)) return { mode: "AUTO" };
  const declared = new Set((tools || []).map((t) => t?.name).filter(Boolean));
  const names = [...new Set(allowedTools)].filter((n) => declared.has(n));
  return names.length
    ? { mode: "VALIDATED", allowedFunctionNames: names }
    : { mode: "NONE" };
}
...
export function buildRequest({
  ...
  grounded = false,
  thinking = null,
  // Which of the declared tools this turn may actually call. Null leaves the
  // model free to call any of them. An array narrows the calls without taking
  // the declarations out of the request, which is the whole point: the tool
  // list sits at the very front of the prompt, so a follow-up turn that sent a
  // shorter list shared no prefix with the turn before it and missed the cache
  // on every token. The declarations stay the same; only the permission moves.
  allowedTools = null,
}) {
  const body = {
    systemInstruction: { parts: [{ text: system }] },
    // A message may carry notes: extra parts after its text, in the same turn.
    // The finishing turn hands its instructions over this way, so everything
    // before them -- tools, system, history, the question -- is byte for byte
    // the request that just answered.
  ...
  if (tools?.length) {
    ...
    body.toolConfig = { functionCallingConfig: callingConfig(tools, allowedTools) };
  }
...
export async function generate({
  ...
  avoid = [],
  // Exactly these models and no ladder beyond them, when set. See lib/agent/llm.js.
  models: only = [],
  // Attempts per model; the usual three unless a caller asks for fewer.
  attempts: attemptCap = null,
  // Which declared tools may be called. See buildRequest.
  allowedTools = null,
  // Which part of the app is asking. Only used to label the prefix log line.
  feature = null,
}) {
  ...
  const refusals = [];
  // What each attempt cost, in tokens, as Google reported it. Kept the same way
  // refusals are and for the same reason: by the time anybody asks what the month
  // went on, the response that could have answered is long gone.
  //
  // One entry per call that came back with a body, whether or not that body was
  // any use. A 200 with nothing in it is billed like an answer and then asked
  // again on the next model, so it has to be a row of its own or the doubling is
  // invisible.
  const spend = [];
  // How many calls this request has made, counting across models. Zero is the
  // first one asked; anything above it was paid for after something else failed.
  let asked = 0;
  ...
  // A caller that names its models gets those, in that order, and nothing
  // after them: a finishing turn pinned to the model that just answered has an
  // answer to fall back on and no reason to walk down to a slower one. Only
  // names on the configured ladder count, so a stale pin cannot send account
  // text to a model nobody chose.
  const pinned = (Array.isArray(only) ? only : []).filter((m) =>
    all.includes(m),
  );
  // Passing over every model is not an option: a thin answer from the same model
  // beats no answer at all.
  const models = pinned.length ? pinned : rest.length ? rest : all;
  const tries = Math.max(1, Math.min(3, Number(attemptCap) || 3));
  ...
  outer: for (let modelIndex = 0; modelIndex < models.length; modelIndex++) {
    ...
    for (let attempt = 0; attempt < tries; attempt++) {
      ...
      asked++;
      // One line per call, matching one model_usage row per call: where this
      // request's prefix would stop matching the one before it.
      logPrefix(body, { feature, model, attempt: asked - 1 });
      try {
      ...
      if (res.ok) {
        ...
        const bill = (ok) => {
          spend.push({
            model,
            grounded: grounded && !searchDropped,
            searches: (parsed.queries || []).length,
            attempt: asked - 1,
            status: res.status,
...
export function parseResponse(json) {
  ...
  return {
    ...
    usage: usageFrom(json?.usageMetadata),
  };
}

/**
 * What the call cost, as Google counted it.
 *
 * Every response carries usageMetadata and the app threw it away for months,
 * which is how "which feature is the bill" became a question to be reasoned about
 * from the shape of the code rather than looked up. The counts are kept apart
 * rather than summed because they are not priced alike: input is cheaper than
 * output, a cached prefix is cheaper than input, and thinking is billed as
 * output even though nobody ever reads it. Summing them here would throw away
 * the only information that makes the total explainable.
 *
 * Names vary by model version, so each is read from whichever field is present.
 * Missing counts stay null rather than becoming zero: a model that reported
 * nothing is a different fact from a call that used nothing.
 */
export function usageFrom(meta) {
  const count = (value) => (Number.isFinite(value) ? value : null);
  const parts = Array.isArray(meta?.promptTokensDetails)
    ? meta.promptTokensDetails
    : [];
  const modality = (name) =>
    count(parts.find((p) => p?.modality === name)?.tokenCount);
  return {
    promptTokens: count(meta?.promptTokenCount),
    candidatesTokens: count(meta?.candidatesTokenCount),
    // What it spent deliberating. Billed as output, never shown to anybody, and
    // the reason thinking is set to "low" nearly everywhere in this app.
    thoughtsTokens: count(meta?.thoughtsTokenCount),
    cachedTokens: count(meta?.cachedContentTokenCount),
    // Tokens Google's own tools used on our behalf, where it reports them.
    toolTokens: count(meta?.toolUsePromptTokenCount) ?? modality("TOOL"),
    totalTokens: count(meta?.totalTokenCount),
  };
}
