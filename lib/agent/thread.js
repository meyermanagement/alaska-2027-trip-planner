...
// Fifteen exchanges at the least: enough that a question about something said
// half an hour ago still has it in front of her.
export const CONTEXT_MESSAGES = 30;
// How far the replayed window's first message moves at a time. The history
// sits after the record in the prompt, and the vendor's cache only reuses a
// prefix that reads the same as the last request's. A window that slid by one
// exchange per question dropped the oldest message every time a conversation
// passed the limit, so the whole history missed the cache on every follow-up.
// Moving the start ten messages at once keeps it still for five questions
// in a row, at the price of replaying up to nine extra messages.
export const REPLAY_STEP = 10;

/**
 * How many of the newest messages to replay when a conversation holds `total`.
 * Never fewer than `min`; the start index is a multiple of `step` past the
 * first `min`, so it only moves when the count crosses a step.
 */
export function replayCount(total, min = CONTEXT_MESSAGES, step = REPLAY_STEP) {
  const n = Math.max(0, Math.floor(Number(total) || 0));
  if (n <= min) return n;
  return min + ((n - min) % step);
}
...
export async function loadThread(
  ...
  // Stepped: read enough for the widest window and the total, then keep the
  // count replayCount allows. One query either way.
  const { data: read, error, count } = await supabase
    .from("chat_messages")
    .select(
      "id, role, kind, body, sources, places, followups, created_at",
      stepped ? { count: "exact" } : undefined,
    )
    .eq("conversation_id", conversationId)
    .order("created_at", { ascending: false })
    .order("id", { ascending: false })
    .limit(stepped ? limit + REPLAY_STEP - 1 : limit);
  if (error) return { messages: [], error };
  const data =
    stepped && Number.isFinite(count)
      ? (read || []).slice(0, replayCount(count, limit))
      : (read || []).slice(0, limit);
...
/**
 * Whether pressing Ask Aly here should pick up the last thread or start a clean one.
 *
 * Picking up where you left off is right when the two questions are about the same
 * thing, and a trip is what makes them about the same thing. Without a trip it was
 * wrong in a way that was easy to miss: every conversation with no trip on it --
 * the All Trips screen, the packing templates, the Wallet, the rewards page,
 * planning something that does not exist yet -- counted as the same conversation,
 * so a request for template item suggestions was filed into whatever general
 * thread happened to be newest, arriving mid-sentence after a question about a
 * completely different subject.
 *
 * So: with a trip, continue; without one, start fresh. The old thread is not lost,
 * it is on the conversation list where it can be chosen deliberately.
 */
export function resumesLastThread(focus, tripId) {
  if (FRESH_EACH_TIME.has(focus)) return false;
...
export async function ensureConversation(
  ...
  userId,
  {
    ...
    resume = true,
    // The row, when the caller has already read it for its trip.
    known = null,
  } = {},
) {
  if (conversationId && known?.id === conversationId) {
    return { id: known.id, created: false, error: null };
  }
  if (conversationId) {
  ...
  // on. Continue what they were last saying about this trip, when there is a trip. The drawer resolves
  ...
  // than showing it.
  if (resume && resumesLastThread(focus, tripId)) {
...
/**
 * Hand a trip-less conversation to the trip it just built.
 *
 * The trip builder and the trip log both start a conversation about a trip that
 * does not exist yet, so the row is filed with no trip on it. Once the trip is
 * created, that is no longer true -- and leaving it untrue was its own bug:
 * pressing Ask Aly on the new draft looked for the last conversation about THAT
 * trip, found none, and started a second thread, so the answers that built the
 * trip sat in a thread the trip could not reach.
 *
 * Only ever fills in a blank. A conversation already about a trip is somebody
 * asking about that trip and is left exactly where it is, which is also what
 * keeps a batch that creates a second trip from stealing the first one's thread.
 */
export async function adoptConversationTrip(
  supabase,
  { conversationId = null, tripId = null } = {},
) {
  if (!conversationId || !tripId) return { adopted: false };
  const { data } = await supabase
    .from("chat_conversations")
    ...
    .select("id")
    .maybeSingle();
  if (!data?.id) return { adopted: false };
  // The lines already written in it belong to the trip too -- what was asked
  // while the trip was being worked out is the best record the trip has.
  await supabase
    .from("chat_messages")
    .update({ trip_id: tripId })
    .eq("conversation_id", conversationId)
