 ...
 * see each other's.
 */
export async function latestConversation(supabase, { tripId = null } = {}) {
  let query = supabase
    .from("chat_conversations")
    .select("id, title, trip_id, focus, updated_at")
...
export async function ensureConversation(
  ...
  if (resume && !FRESH_EACH_TIME.has(focus)) {
    const { conversation } = await latestConversation(supabase, { tripId });
    if (conversation?.id) {
