...
export function shouldLookUp(message = "", history = []) {
  ...
  if ((asksToSave(said) || INSTRUCTS.test(said)) && !ASKS.test(said))
    return false;
  return true;
}
...
// stays a question.
const INSTRUCTS =
  /^(?:please\s+|can you\s+|could you\s+)?(move|moves|shift|rename|delete|remove|empty|clear|erase|wipe|mark|tick|untick|uncheck|swap|reorder|renumber|duplicate|copy|assign|unassign|archive|split|merge|sort|group|combine|rearrange)\b/i;
