...
const SUBJECTS =
  /\b(dinner|lunch|breakfast|brunch|supper|dine|dining|eat|eating|restaurants?|food|meals?|cuisine|bars?|cafes?|coffee|hotels?|resorts?|lodges?|inns?|stay|staying|lodging|accommodations?|airbnb|vrbo|turo|rental cars?|car rentals?|rental homes?|rentals?|vacation homes?|villas?|condos?|activit(?:y|ies)|things? to do|excursions?|tours?|attractions?|museums?|hikes?|hiking|sights?|sightseeing|shows?|day trips?|worth doing|worth seeing|to do|to see|to eat|to visit|to explore)\b/i;

// The shape of asking rather than telling.
const ASKS =
  /\b(recommend|recommends|recommended|recommendations?|suggest|suggests|suggestions?|ideas?|options?|shortlist|choices|what should|where should|which should|what would|where would|what can|where can|what do you|where do you|any good|anywhere good|somewhere good|anything good|best|top|favou?rites?|worth|what is there|what's there|whats there|what else|advice|thoughts on|opinions? on|help us (?:find|pick|choose)|show (?:me|us)|find (?:me|us)|are there any|what(?:'s| is| are)? on|which (?:\w+ ){0,2}(?:should|would|do you|is best|are best))\b/i;
 ...
 *
 * Open by default, and it took three rounds of the wrong answer to learn why. A
 * list of phrases that count as asking for ideas will never be complete: "try
 * again" is not on it, and neither was "more for this trip", and each miss looks
 * to the family exactly like the web being broken. Google only charges for a
 * search the model actually runs, and the model does not run one to answer "who
 * is on this trip", so leaving the door open costs almost nothing and closing it
 * costs the answer.
 *
 * Shut only for instructions - "add dinner at Orso on Thursday", "empty the
 * Disney list". Those are decided already, they need the app rather than the web,
 * and they should not wait on one.
 */
export function shouldLookUp(message = "", history = []) {
  if (wantsIdeas(message)) return true;
  if (isFollowUp(message) && recentlyAskedForIdeas(history)) return true;
  const said = String(message || "").trim();
  if (!said) return false;
  // An instruction that is only an instruction. "Add somewhere good for dinner on
  // Thursday" is both, and the web helps with the somewhere good part, so the test
  // is whether anything is being asked - not whether a restaurant is mentioned.
  if (
    (asksToSave(said) || INSTRUCTS.test(said)) &&
    !ASKS.test(said) &&
    !VAGUE.test(said)
  )
    return false;
...
// The other imperatives. Rearranging what is already there needs the app, not the
// web. Anchored at the start so "what should we do about changing the flights"
// stays a question.
// What can sit in front of an instruction without changing it into a question.
// "Now update this trip with the items from the new cruise template" searched the
// web -- an instruction to copy rows between two of the family's own lists, sent
// to Google. Two separate misses: "update" was in neither this list nor SAVES,
// and "Now" alone would have defeated the anchor even if it had been.
const LEAD_IN =
  /^(?:(?:ok(?:ay)?|right|so|now|then|next|also|and|please|could you|can you|would you|will you|go ahead and|i want you to|i'd like you to)[\s,]+)*/i;

const INSTRUCTS = new RegExp(
  LEAD_IN.source +
    "(move|moves|shift|rename|delete|remove|empty|clear|erase|wipe|mark|tick|untick|uncheck|swap|reorder|renumber|duplicate|copy|assign|unassign|archive|split|merge|sort|group|combine|rearrange|" +
    // Changing what is already in the app. "Update", of all words, was missing.
    "update|updates|change|changes|edit|edits|fix|correct|set|reset|replace|rewrite|amend|adjust|apply|push|sync|fill|populate|pull|import|refresh|tidy|propagate|generate|rebuild|build|start|make)\\b",
  "i",
);
...
// An instruction that has not decided the important part yet. "Change the hotel
// to something nicer" is an instruction by its verb and a question by everything
// after it: the family has not named the hotel, so the web is the only place the
// answer can come from. Widening the instruction verbs above made this matter --
// "change" and "update" cover far more ground than "reorder" ever did.
const VAGUE =
  /\b(some(?:where|thing|one)|any(?:where|thing)|nicer|cheaper|closer|nearer|quieter|fancier|posher|better|different|alternative|good|best)\b/i;
