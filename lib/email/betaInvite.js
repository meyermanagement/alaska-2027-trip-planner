 ...
 *
 * The closing block asks for the two different things a tester can give, in the
 * order they will happen: the flag for the moment something breaks, and the
 * survey for how the whole thing is going. The survey is the harder of the two
 * to find -- a row inside a drawer that is shut, rather than a mark that floats
 * in a corner -- so it gets a picture of the open drawer with its row ringed
 * rather than another sentence about where to tap.
 */
...
export function betaInviteEmail({ name, email, code, siteUrl, note }) {
  ...
  const text = [
    `Hi ${first}, I’m Aly.`,
    "",
    `${alyDescriptorList()}… let’s just say ${ALY_PAYOFF}`,
    "",
    ...
    "",
    "The survey is the longer conversation. A page of questions about how the app is working for you: what you tried, what you would pay, what is missing. It saves as you go, so you can answer two questions now and come back after your next trip. Find it in the menu, under More, on the row called Beta Survey.",
    "",
    "— Aly, at Alyeska",
  ]
...
${alyDescriptorList()}… let’s just say <strong style="font-weight:700; color:${INK};">${ALY_PAYOFF}</strong>
</p>
...
<!-- The survey. A flag press is one thing gone wrong; this is the whole
     picture, and it is the harder of the two to find, because it is a row
     inside a closed drawer rather than a mark that floats in a corner. So the
     picture here is the drawer open, with the row it means ringed: naming a
     menu asks somebody to go hunting, while a picture of the menu they are
     about to open lets them recognize it. Cropped from the real menu on a
     phone, a PNG for the same reason the flag and the wordmark are, and served
     at twice its printed width so it stays sharp on a retina screen. -->
<div style="font-family:${SANS}; font-size:15px; line-height:1.6; color:${INK}; padding:18px 0 0;"><strong style="font-weight:700;">The survey is the longer conversation.</strong> A page of questions about how the app is working for you: what you tried, what you would pay, what is missing. It saves as you go, so you can answer two questions now and come back after your next trip. Find it in the menu, under More.</div>
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:12px 0 0;">
<tr>
<td align="center" style="background:${SAND}; border:1px solid ${SAND_DEEP}; border-radius:12px; padding:10px;">
<img src="${escapeHtml(siteUrl)}/beta-survey-menu.png" width="414" height="357" alt="The menu, with the More drawer open and the row named Beta Survey ringed in bright teal" style="display:block; width:100%; max-width:414px; height:auto; border:0; outline:none; text-decoration:none; border-radius:8px;">
</td>
</tr>
</table>
</td>
