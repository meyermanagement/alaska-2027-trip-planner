/**
 * One way out for outbound email, with two possible pipes behind it.
 *
 * Which pipe is used is decided entirely by environment variables, so the
 * transport can be swapped without touching a line of feature code:
 *
 *   Gmail    GMAIL_USER + GMAIL_APP_PASSWORD   (sends as that Gmail address)
 *   Resend   RESEND_API_KEY + EMAIL_FROM       (needs a domain you own)
 *
 * Set EMAIL_TRANSPORT to "gmail" or "resend" to force one when both are
 * present. With neither configured, sending fails loudly with a message the UI
 * can show, rather than pretending it worked.
 */

const GMAIL = "gmail";
const RESEND = "resend";

/**
 * Household mail leaves from our own domain, or it does not leave.
 *
 * Both pipes were wired and either could win depending on which variables
 * happened to be set, and what production actually did was send every tester's
 * invitation and every morning reminder from a personal Gmail account while the
 * privacy policy named a different processor. That is a disclosure problem before
 * it is a matter of taste: the person is told who handles their email, and it has
 * to be true.
 *
 * The rule that is worth enforcing is not "which pipe" -- Google Workspace and
 * Resend are both reasonable answers -- it is whose name is on the envelope. Mail
 * about somebody's household goes out from an address on this app's own domain,
 * checked here rather than trusted to whoever set the variables. Anything else is
 * refused in production, and the refusal says what to fix.
 */
export const SENDING_DOMAIN = "alyeska.app";

export function isProduction() {
  return (
    (process.env.VERCEL_ENV || process.env.NODE_ENV || "").toLowerCase() ===
    "production"
  );
}

export function emailTransport() {
  const forced = (process.env.EMAIL_TRANSPORT || "").trim().toLowerCase();
  if (forced === GMAIL || forced === RESEND) return forced;
  if (process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD) return GMAIL;
  if (process.env.RESEND_API_KEY) return RESEND;
  return null;
}

/** The bare address mail would go out from, without the display name. */
export function sendingAddress() {
  const address =
    process.env.EMAIL_FROM ||
    (emailTransport() === GMAIL ? process.env.GMAIL_USER : null);
  return (address || "").trim().toLowerCase() || null;
}

export function onOwnDomain(address = sendingAddress()) {
  return Boolean(address && address.endsWith(`@${SENDING_DOMAIN}`));
}

/**
 * Why nothing can be sent, in a sentence somebody can act on. Null when it can.
 */
export function transportProblem() {
  if (!emailTransport() || !sendingAddress()) {
    return "Email sending is not set up on the server yet, so nothing was sent.";
  }
  if (isProduction() && !onOwnDomain()) {
    return `Household email goes out from an address on ${SENDING_DOMAIN}, and this server is configured to send as ${sendingAddress()}. Nothing was sent. Set EMAIL_FROM to the ${SENDING_DOMAIN} mailbox mail should come from.`;
  }
  return null;
}

export function emailFrom() {
  const label = process.env.EMAIL_FROM_NAME || "Alyeska";
  const address =
    process.env.EMAIL_FROM ||
    (emailTransport() === GMAIL ? process.env.GMAIL_USER : null);
  if (!address) return null;
  return `${label} <${address}>`;
}

/**
 * @param {{
 *   to: string,
 *   subject: string,
 *   html?: string,
 *   text?: string,
 *   replyTo?: string,
 *   attachments?: Array<{ filename: string, contentType?: string, data: Buffer }>,
 * }} params
 * @returns {Promise<{ok: true} | {ok: false, error: string}>}
 *
 * `attachments` are simple `{filename, contentType, data}` records; each
 * data is a Node Buffer. Kept transport-agnostic here so the caller does not
 * have to know whether Resend or Gmail SMTP is on the other end.
 */
export async function sendEmail({
  to,
  subject,
  html,
  text,
  replyTo,
  attachments,
}) {
  const transport = emailTransport();
  const from = emailFrom();

  const problem = transportProblem();
  if (problem) {
    return { ok: false, error: problem };
  }

  try {
    if (transport === RESEND) {
      const res = await fetch("https://api.resend.com/emails", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${process.env.RESEND_API_KEY}`,
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          from,
          to: [to],
          subject,
          html,
          text,
          ...(replyTo ? { reply_to: replyTo } : {}),
          ...(attachments && attachments.length
            ? {
                // Resend takes base64 content and infers the mime type from
                // the filename; we pass content_type when we already know
                // it, so a screenshot stays image/png in the recipient's
                // client rather than octet-stream.
                attachments: attachments.map((a) => ({
                  filename: a.filename,
                  content: a.data.toString("base64"),
                  ...(a.contentType ? { content_type: a.contentType } : {}),
                })),
              }
            : {}),
        }),
      });
      if (!res.ok) {
        const body = await res.text().catch(() => "");
        return { ok: false, error: resendMessage(res.status, body) };
      }
      return { ok: true };
    }

    // Gmail over SMTP. Imported here rather than at the top so a Resend-only
    // deployment never has to load it.
    const nodemailer = (await import("nodemailer")).default;
    const mailer = nodemailer.createTransport({
      host: "smtp.gmail.com",
      port: 587,
      secure: false,
      auth: {
        user: process.env.GMAIL_USER,
        // Google displays app passwords in four spaced blocks; tolerate a paste.
        pass: (process.env.GMAIL_APP_PASSWORD || "").replace(/\s+/g, ""),
      },
    });
    await mailer.sendMail({
      from,
      to,
      subject,
      html,
      text,
      replyTo,
      ...(attachments && attachments.length
        ? {
            attachments: attachments.map((a) => ({
              filename: a.filename,
              content: a.data,
              contentType: a.contentType,
            })),
          }
        : {}),
    });
    return { ok: true };
  } catch (err) {
    return { ok: false, error: friendly(err) };
  }
}

function resendMessage(status, body) {
  if (status === 403 && /testing emails/i.test(body)) {
    return "Resend will only deliver to your own account address until a domain you own is verified. Verify a domain, or switch to the Gmail sender.";
  }
  if (status === 401 || status === 403) {
    return "The email provider rejected the API key.";
  }
  if (status === 429) {
    return "The email provider is rate limiting us. Wait a minute and try again.";
  }
  return `The email provider returned an error (${status}).`;
}

function friendly(err) {
  const message = String(err?.message || err || "");
  if (
    /Invalid login|Username and Password not accepted|BadCredentials/i.test(
      message,
    )
  ) {
    return "Gmail rejected the app password. It has to be a 16-character app password from an account with 2-Step Verification turned on, not the account password.";
  }
  if (/Daily user sending limit|limit exceeded/i.test(message)) {
    return "Gmail's daily sending limit was hit. Try again tomorrow.";
  }
  return message || "The email could not be sent.";
}
