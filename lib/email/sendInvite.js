...
export async function sendTravelerInvite({
  ...
  const { data: traveler } = await supabase
    .from("travelers")
    .select("id, name, email, family_id, user_id, date_of_birth")
    .eq("id", travelerId)
    .maybeSingle();
  ...
  if (isMinorTraveler(traveler)) {
    return { ok: false, error: "Minors do not sign in separately. Open their trip view from Family & pets → Child access.", status: 403 };
  }
  if (!traveler.email) {
    return {
      ok: false,
