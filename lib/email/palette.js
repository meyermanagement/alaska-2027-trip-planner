 ...
 * the app has one email skin, named here, and it is Daybreak Aurora — the light
 * version of the design that the web now defaults to Midnight Aurora for.
 *
 * A light-mode message survives an inbox better than a dark-mode one. Most
 * clients render mail on a white or near-white ground and the ones that offer a
 * dark mode invert light templates for the reader; a template painted dark to
 * begin with cannot be inverted back, and lands as a black card floating on
 * white with a mismatched border around it. So the mail stays in Daybreak
 * whichever skin the web is set to, and both versions are the same design in
 * the same colors seen at different times of day.
 *
...
// Geist will not load in an inbox. This is the same fallback stack the site
// declares, so the shapes stay close to the real thing.
//
// DISPLAY and SANS now resolve to one stack, because the app has one face. It was
// an editorial serif for names and headings over this sans for everything else,
// and the serif went when the wordmark became letterspaced capitals over a
// hairline. The two names are kept apart so a heading in a message is still
// declared as a heading, and so a second face is one line here rather than a
// rewrite of three templates.
export const SANS =
