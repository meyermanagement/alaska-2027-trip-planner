...
import { pushRemindersTo } from "@/lib/push/reminders";
import { remindersDueToday } from "@/lib/tasks/dueToday";
import { todayISO } from "@/lib/reminders";

/**
 * The morning run: work out who owes what today and tomorrow, email them once,
 * and ring their phone about the parts with a date on them.
 *
...
export async function sendDueTodayReminders({
  ...
  const readEverything = () =>
    Promise.all([
      ...
      supabase
        .from("travelers")
        .select(
          "id, name, email, family_id, is_person, wants_reminders, user_id, date_of_birth",
        )
        .eq("is_person", true),
      supabase.from("trip_travelers").select("trip_id, traveler_id"),
    ]);

  let [{ data: tasks, error: taskError }, { data: travelers }, { data: crew }] =
    await readEverything();

  // One second chance at the read the whole morning depends on.
  //
  // Twice in September the scheduled run was refused at 12:00:28 with "JWT issued
  // at future" -- the database declining a token whose issued-at time was ahead of
  // its own clock -- and once with a gateway timeout. All three considered nobody
  // and emailed nobody. None of them were anything wrong with the household, the
  // work, or the mailer; they were a couple of seconds of bad luck between two
  // machines, and they cost a whole day's reminders each time.
  //
  // So the read is asked again, once, after a pause. If the second attempt fails
  // too the run still gives up and says why, because a database that is genuinely
  // unreachable should be reported rather than hammered.
  if (taskError) {
    ...
    if (second[0].error) {
      return {
        ok: false,
        error: `${taskError.message} (asked twice)`,
        status: 500,
      };
    }
    tasks = second[0].data;
    travelers = second[1].data;
    crew = second[2].data;
  }
  ...
  const notifiable = await whoAgreedToBeInterrupted(supabase, travelers || []);
  ...
  // Every household this run actually looked at, whether or not anything was due
  // in it. This is the set the run record is written from, and the reason it is
  // built from the people rather than from the work: a household where nothing is
  // due still had its morning run, and the app has to be able to say so. Taking
  // the household from the work instead is how a quiet morning becomes
  // indistinguishable from a morning that never happened.
  const tally = new Map();
  const seen = (familyId) => {
    if (!familyId) return null;
    if (!tally.has(familyId))
      tally.set(familyId, { familyId, considered: 0, sent: [], failed: [] });
    return tally.get(familyId);
  };
  for (const person of travelers || []) {
    if (onlyTravelerId && person.id !== onlyTravelerId) continue;
    if (onlyFamilyId && person.family_id !== onlyFamilyId) continue;
    seen(person.family_id);
  }
  for (const batch of batches) {
    const row = seen(batch.person.family_id);
    if (row) row.considered++;
  }

  const sent = [];
  const failed = [];
  // Counted rather than named, the same as `declined`: this is evidence the phone
  // was tried, not a record of what anybody was told.
  const pushed = { tried: 0, delivered: 0 };
  ...
  for (const batch of batches) {
    ...
    if (result.ok) {
      ...
      const row = seen(person.family_id);
      if (row) row.sent.push(sent[sent.length - 1]);
    } else {
      failed.push({ to: person.email, error: result.error });
      const row = seen(person.family_id);
      if (row) row.failed.push(failed[failed.length - 1]);
    ...
    // And the phone, whatever the email did. An email that failed is the case
    // where the notification is the only thing that arrives, so this is not
    // hung off the send succeeding. It cannot break the run either: a push
    // service having a bad morning must not stop the next person's email.
    if (pushConfigured()) {
      try {
        const rang = await pushRemindersTo({
          supabase,
          travelerId: person.id,
          siteUrl,
          items,
        });
        pushed.tried += rang.tried;
        pushed.delivered += rang.delivered;
      } catch {
        // Deliberately silent. The email is the record; this was the nudge.
      }
    }
  }
  ...
  return {
    ...
    // One entry per household, so the run can be written down household by
    // household instead of stamped with whichever one the database handed back
    // first.
    households: [...tally.values()],
    pushed,
...
async function whoAgreedToBeInterrupted(supabase, travelers) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    on.set(
      row.user_id,
      consentIsCurrent(row) && features.notifications === true,
    );
  }
