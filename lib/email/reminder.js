...
/**
 * "1 thing", "3 things" — the count is the whole subject line's job.
 */
function subjectFor(items, tripNames) {
  const n = items.length;
  const thing = plural(n, "thing", "things");
  if (tripNames.length === 1) {
    return `${n} ${thing} to do today for ${tripNames[0]}`;
  }
  return `${n} ${thing} to do today before we travel`;
}
 ...
 * @param {Array} input.items    {title, detail, tripName, tripSlug, priority, exact, note, assignee}
 */
export function reminderEmail({
  ...
  const text = [
    ...
    ...items.map((item) => {
      const bits = [
        ...
        item.exact ? "  Due today" : `  ${item.note || "Due about now"}`,
  ...
  const rows = items
    .map((item, i) => {
      const flag = item.exact
        ? `<span style="font-family:${SANS}; font-size:11px; font-weight:700; letter-spacing:0.06em; text-transform:uppercase; color:${AMBER};">Due today</span>`
        : `<span style="font-family:${SANS}; font-size:11px; font-weight:700; letter-spacing:0.06em; text-transform:uppercase; color:${INK_SOFT};">${escapeHtml(item.note || "Due about now")}</span>`;
      const high =
        item.priority === "high"
          ? `<span style="font-family:${SANS}; font-size:11px; font-weight:700; letter-spacing:0.06em; text-transform:uppercase; color:${ROSE};"> &middot; High priority</span>`
          : "";
...
${detail}${trip}
  ...
  const lede =
    items.length === 1
      ? `One thing has your name on it today${tripNames.length === 1 ? ` for ${escapeHtml(tripNames[0])}` : ""}.`
      : `${items.length} things have your name on them today${tripNames.length === 1 ? ` for ${escapeHtml(tripNames[0])}` : ""}.`;
...
<div style="font-family:${SANS}; font-size:14px; line-height:1.55; color:${INK};">Ticking something off in the app is enough &mdash; there is nothing to reply to here, and anything you finish will not be mentioned again tomorrow.</div>
</td>
</tr>
</table>
