...
export const ISSUE_COLUMNS =
  "id, ref, created_at, email, kind, body, path, trip_id, skin, viewport, user_agent, build, trail, shots, status, route, maybe_fixed, seen_count, last_at, detail, anonymized_at";
...
export async function readIssues(admin, rows, { build = "" } = {}) {
  ...
  return rows.map((one) => ({
    ...
    email: one.email,
    // Said rather than left as an absent address. A report with nothing naming
    // anybody could be an old row or it could be one whose reporter deleted
    // their account and had everything about them taken out; those are different
    // things and the desk should not have to guess which it is looking at.
    anonymized: Boolean(one.anonymized_at),
    kind: one.kind,
    body: one.body,
    path: one.path,
