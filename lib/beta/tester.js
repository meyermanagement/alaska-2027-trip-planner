...
export async function isTesterAccount(supabase, me) {
  ...
  // Establish membership with the caller's session, then read only those
  // households. Secondary traveler visibility is not the household roster.
  const { data: memberships, error: membershipError } = await supabase
    .from("family_members").select("family_id").eq("user_id", me.id);
  const familyIds = (memberships || []).map(row => row.family_id);
  if (membershipError || !familyIds.length) return false;
  const { data: mates, error: matesError } = await admin
    .from("family_members").select("user_id").in("family_id", familyIds);
  if (matesError) return false;
  const ids = Array.from(
    new Set((mates || []).map((one) => one.user_id).filter(Boolean)),
  );
  if (!ids.length) return false;
  if (ids.some(id => isAdminUser({ id }))) return true;
