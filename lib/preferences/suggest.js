...
import { dedupe, topicsOf } from "./topics";
...
{"suggestions":[{"topics":["…"],"body":"…","whose":"…","because":"…"}]}

  topics   one to three short labels, sentence case, reusing their existing topics where one fits. Give two when the preference genuinely spans two \u2014 a hotel spa is about where they stay and about what they do.
  ...
  if (preferences.length) {
    ...
    for (const pref of preferences) {
      ...
      const owner =
        ...
        SHARED_LABEL;
      out.push(
        `- [${topicsOf(pref).join(", ") || "no topic"}, ${owner}] ${clip(pref.body, 300)}`,
      );
    }
  } else {
...
export function acceptSuggestions({
  ...
  for (const candidate of Array.isArray(candidates) ? candidates : []) {
    ...
    // Either shape: a model that answers with one topic instead of a list is not
    // wrong about the preference, and refusing the suggestion over the shape of
    // the field would throw away the answer to save the schema.
    const proposed = Array.isArray(candidate?.topics)
      ? candidate.topics
      : candidate?.topics || candidate?.topic
        ? [candidate.topics || candidate.topic]
        : [];

    const topics = dedupe(proposed.map((t) => clip(t, 60))).slice(0, 3);

    suggestions.push({
      topics,
      // Mirrored, the same way the table mirrors it: a draft is handed straight
      // to a card that may be older than this change, and one field going quietly
      // undefined is how a suggestion arrives filed under nothing.
      topic: topics[0] || "",
      body,
