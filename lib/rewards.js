// Everything the app agrees on about rewards programmes: what kinds there are,
// how a points balance is worth talking about, and how an earning rule reads.
//
// A rewards row is deliberately loose. A hotel programme has a balance and a
...
export const REWARD_KINDS = [
  ...
  },
  {
    ...
    blurb: "Programmes that feed miles into the others.",
  },
  {
    key: "other",
 ...
 */
export function normaliseRules(value) {
...
export function bestCardFor(rows, spend) {
  ...
  for (const row of rows || []) {
    ...
    for (const rule of normaliseRules(row.earn_rules)) {
