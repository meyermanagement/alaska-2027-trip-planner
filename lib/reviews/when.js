 ...
 * - A stay is over at checkout, not at check-in. Reviewing the hotel on the first
 *   morning is possible in principle, but the end date is the honest answer to
 *   "has this passed" and it is the date the family will look for.
...
export function hasHappened(item, { today, nowHM = null } = {}) {
  ...
  // A stay: one row standing for several nights. Over when the last night is.
  if (item.end_date && item.end_date > start) return item.end_date < today;
