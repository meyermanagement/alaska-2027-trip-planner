import { groupHistory } from "../history/periods";
...
/** Relative date bands are the default; trip and kind remain available. */
...
/**
 * One chronological default works as a household's history grows. Readers
 * can still regroup by trip or kind without losing search or ratings.
 */
...
export function groupPlaces({
  ...
  if (by === "date") return groupHistory(items, item => item.item_date || item.trip?.end_date, today);
  if (by === "trip") {
...
export function openByDefault({
  ...
  if ((by !== "trip" && by !== "date") || narrowed) return sections.map((s) => s.key);
...
export function browsePlaces({
  ...
  const sections = groupPlaces({ items: ordered, by, trips, kinds, today });
  // Date bands remain chronological; the chosen order applies inside each.
  if (by === "date") sections.forEach(section => { section.items = sortPlaces(section.items, sort); });
