...
/**
 * Where an all-day event stops, in the exclusive form every calendar wants:
 * the item's own end date when it spans days, otherwise the day after.
 */
 ...
 *
 * A stay that carries a check-out date is always written as one multi-day
 * all-day event, which is how a hotel belongs in a calendar — a single band
 * across the week rather than an hour on the first afternoon. The check-in time
 * is not lost; it moves into the description, where it is still there to read.
 */
export function eventFromItem(item, trip) {
  ...
  const spans = Boolean(item.end_date && item.end_date > item.item_date);
  const bits = [
    trip?.name ? `Trip: ${trip.name}` : null,
    spans && time ? `Check in from ${formatTime(time)}` : null,
  ...
  return {
    ...
    date: item.item_date,
    // Calendars treat the end of an all-day event as exclusive, so a stay that
    // checks out on the 8th ends on the 8th and the last night booked is the
    // 7th — exactly what the check-out date already means.
    endDate: spans ? item.end_date : null,
    startTime: spans ? null : time,
    endTime: spans || !time ? null : addMinutes(time, DEFAULT_MINUTES),
...
function vevent(event, now) {
  ...
  if (event.startTime) {
    ...
    lines.push(`DTEND;VALUE=DATE:${compactDate(allDayEnd(event))}`);
  }
...
export function googleUrl(event) {
  const dates = event.startTime
    ...
    : `${compactDate(event.date)}/${compactDate(allDayEnd(event))}`;
...
export function outlookUrl(event, flavour = "live") {
  ...
  if (event.startTime) {
    ...
    params.set("enddt", allDayEnd(event));
