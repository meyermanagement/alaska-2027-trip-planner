...
// A date-only end date leaves the offer alive through that whole household day:
// an offer ending today is still an offer today. The date is the issuer's own,
// read off the page the terms came from, so unlike an inferred fare deadline it
// can be acted on without a guess.
export function offerEnded(offer, today) {
  const ends = offer?.offer_ends_on ? String(offer.offer_ends_on).slice(0, 10) : "";
  return Boolean(ends) && Boolean(today) && ends < String(today);
}

/** Offers whose end date has passed, so the ledger stops calling them open. */
export function staleOffers(ledger = [], today) {
  return (ledger || [])
    .filter((row) => row?.status === "open" && offerEnded(row, today))
    .map((row) => row.id);
}

/**
 * The ledger as the four lists a screen shows, worked out against today rather
 * than waiting for a pass to write the status down.
 *
 * This is the offers half of what fareListsForToday does for fares, and for the
 * same reason: the watch runs in the evening, so between a deadline going by and
 * the next pass the Wallet would otherwise present a dead offer as a live one.
 * An offer already turned down whose date has since gone by is filed as expired
 * too -- the refusal is no longer the interesting fact about it, and there is
 * nothing left to reconsider.
 *
 * @param {Array} offers card_offers rows
 * @param {string} today YYYY-MM-DD in the household's zone
 */
export function offerListsForToday(offers = [], today) {
  const lists = { open: [], declined: [], expired: [], taken: [] };
  for (const row of offers || []) {
    if (!row) continue;
    if (row.status === "taken") {
      lists.taken.push(row);
      continue;
    }
    if (row.status === "expired") {
      lists.expired.push(row);
      continue;
    }
    if (offerEnded(row, today)) {
      lists.expired.push({ ...row, status: "expired" });
      continue;
    }
    if (row.status === "declined") lists.declined.push(row);
    else if (row.status === "open") lists.open.push(row);
  }
  return lists;
}

/**
 * Retire the offers whose end date has gone by, and put the sentences written
 * about them away with them.
 *
 * Recheck household, status and date at write time, the way retireFares does: a
 * pass can be reading a ledger while somebody extends an offer or takes the card.
 * The household is named rather than inferred, because this is called from the
 * service-role watch as well as from a signed-in request.
 *
 * The tip is the other half. A tip left active would keep arguing for an offer
 * that no longer exists, and 'expired' keeps it out of the live list and out of
 * the cleared record while leaving it in the table, so the next look does not
 * write the same sentence again.
 *
 * @returns {Promise<{retired: Array, error: object|null}>}
 */
export async function retireOffers(supabase, familyId, ids, today) {
  if (!supabase || !familyId || !ids?.length) return { retired: [], error: null };
  const { data, error } = await supabase
    ...
    .eq("status", "open")
    .in("id", ids)
    .lt("offer_ends_on", today)
    .select("id, tip_id");
  if (error) return { retired: [], error };

  const tips = (data || []).map((row) => row.tip_id).filter(Boolean);
  if (tips.length) {
    // Not fatal. A dead offer off the open list is the point; a sentence left on
    // the live list is a tip read twice, not a decision lost.
    await supabase
      .from("pro_tips")
  ...
  return { retired: data || [], error: null };
}
