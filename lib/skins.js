...
/**
 * Whether this browser has already been shown the full opening -- the compass
 * drawing itself, the wordmark, the tagline turning once.
 *
 * A session cookie, deliberately. The full opening is worth five and a half
 * seconds the first time somebody arrives and is worth nothing at all the
 * fourth time in ten minutes, and it was showing on every document load: a
 * sign-in redirect, a server redirect, anything that is not a client-side
 * navigation. So the first document load in a browser session gets it and every
 * load after that gets the short one -- a compass crossing a map, up for as long
 * as the page actually needs and no longer.
 *
 * No max-age, so it dies when the browser does and tomorrow's first open is an
 * arrival again. Written by the script in the document head rather than by
 * middleware, because the choice of which veil to show has to be made in the
 * first frame of HTML, before anything has hydrated.
 */
export const BOOT_COOKIE = "alyeska-opened";
