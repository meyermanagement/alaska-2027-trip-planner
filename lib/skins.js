...
export const SKINS = [
  {
    ...
    tag: "Dark with a northern-lights glow",
    blurb:
      "Deep backgrounds with teal and violet highlights. A little night-sky magic for your plans.",
  ...
  },
  {
    ...
    tag: "Light, fresh, and cool",
    blurb:
      "Pale backgrounds and teal accents. The northern-lights look, brightened up for daytime.",
  ...
  },
  {
    id: "journal",
    name: "Field Journal",
    tag: "Warm paper and forest green",
    blurb:
      "Cream backgrounds, dark text, and green accents. A travel-journal feel for your next chapter.",
  ...
  },
  {
    id: "frost",
    name: "Frostglass",
    tag: "Soft pastels and frosted glass",
    blurb:
      "Light blue, lilac, and pink with purple accents. Airy and softly colorful.",
  ...
  },
  {
    id: "sodium",
    name: "Sodium",
    tag: "Dark with a warm amber glow",
    blurb:
      "Deep brown-black backgrounds with amber and copper accents. Think city lights after sunset.",
 ...
 *
 * It was Field Journal to start, on the reasonable ground that an account which
 * had never been to Settings should see the app as it had always looked. Then
 * Daybreak Aurora, on the better ground that it was the version that survived a
 * phone held at arm's length in daylight. Midnight Aurora now, because the
 * night version is what the app is meant to be seen in: dark glass over a wash
 * of teal into violet is the whole idea of the design, and Daybreak is that
 * idea softened for daylight rather than the other way round. First in the
 * list above as well as first by default, with Daybreak Aurora directly under
 * it as the same light in the morning.
 *
 ...
 */
// Named here and nowhere else, with one exception that has to be kept in step:
// profiles.skin carries a column default, because a row can be inserted by a
// database trigger that cannot read this file. The two disagreed once --
// 'daybreak' in the column against "aurora" here -- and the symptom was every
// new account arriving in the wrong skin after a one-frame flash of the right
// one. If this line changes, change the column default with it.
export const DEFAULT_SKIN = "aurora";

/**
 * Where the browser is told which skin to paint before the app loads.
 *
 * The name carries a generation, and it is bumped whenever the column moves
 * underneath people -- twice now. This cookie is not a hint the app double-checks -- it is the only
 ...
 * was wrong both times a skin was reassigned in SQL rather than chosen: first
 * when every profile row was moved to Daybreak Aurora underneath them, and again
 * when a test account was moved back to Midnight after the column default was
 * corrected. In both cases the browser went on holding a year-long cookie naming
 * a skin nobody was set to any more, and would have kept painting it until the
 * cookie expired.
 *
 ...
 */
export const SKIN_COOKIE = "alyeska-skin-4";

/** The names it had before that, cleared on sight so they do not sit for a year. */
export const SKIN_COOKIE_STALE = [
  "alyeska-skin",
  "alyeska-skin-2",
  "alyeska-skin-3",
];
...
/**
 * Whether the next document load is an arrival -- somebody who has just signed
 * in, and is owed the full opening: the compass drawing itself and swinging on
 * to north, the wordmark, the tagline turning once.
 *
 * The full opening used to be given to the first document load in a browser
 * session, which meant a refresh could earn it, a new tab could earn it, and
 * signing in could miss it. It belongs to one moment instead. Both sign-in doors
 * are server redirects, so each one sets this on its way past, and the script in
 * the document head spends it: full opening, cookie deleted in the same breath.
 * Every other load in the app -- a refresh, a server redirect, a cold open by
 * somebody already signed in -- gets the short one, a compass crossing a map,
 * held only as long as the page actually needs.
 *
 * No max-age, so an arrival that is never collected dies with the browser rather
 * than waiting a year to play over somebody's trips. Read by a script in the
 * head rather than by middleware, because which veil to show has to be settled
 * in the first frame of HTML, before anything has hydrated.
 */
export const ARRIVE_COOKIE = "alyeska-arrive";
