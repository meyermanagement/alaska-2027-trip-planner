...
function noHealth(ToolError, ...texts) {
  if (readsAsHealth(...texts.filter(Boolean))) {
    throw new ToolError("Leave health details out; add them in the app if they matter. Nothing was saved.");
  }
}
...
function onlyPrimaryFamily(ToolError, scope, what) {
  ...
  if (fams.length > 1) throw new ToolError(`You belong to more than one household, so this has to be done in the app. Nothing was changed.`);
  return fams[0];
}
...
export function planTools(ADD, UPDATE, tripArg) {
  ...
  return [
    t("add_itinerary_item", "Add an itinerary item",
      "Adds one plan to a trip's itinerary: a flight, a stay, a dinner, an excursion. Primary travelers only. Will not add an item already on that day with the same title, or anything about health. Confirmation numbers are added in the app.",
      {
        trip: tripArg,
    ...
    t("create_trip", "Create a trip",
      "Creates a new trip in the household, with who is going. Primary travelers only. It does not build a packing list; that is started in the app. Will not create a second trip with the same name.",
      {
    ...
    t("update_trip", "Update a trip",
      "Changes a trip's name, destination, dates, status or budget. Primary travelers only. Who is going is changed in the app.",
      {
        trip: tripArg,
        name: s("A new name."),
    ...
    t("add_rewards_program", "Add a rewards program or card",
      "Adds a loyalty program or credit card to the household's Wallet, for one adult or Shared. Primary travelers only. Member and card numbers are added in the app, never here. Will not add a program that person already has.",
      {
...
export function planHandlers({ ToolError, pickTrip, tripRef, visiblePacking }) {
  return {
    ...
    async create_trip(client, scope, args) {
      ...
      return {
        summary: `Created ${name}${start ? ` (${start}${end ? ` to ${end}` : ""})` : ""} for ${who.join(", ")}.${status === "complete" ? "" : " Start its packing list in the app."}`,
    ...
    async add_rewards_program(client, scope, args) {
      ...
      if ([brand, programName, tier].some((x) => x && /\d{6,}/.test(x.replace(/[\s-]/g, "")))) {
        throw new ToolError("Member and card numbers are added in the app, not here. Nothing was added.");
      }
    ...
    async update_rewards_program(client, scope, args) {
      ...
      if (tier && /\d{6,}/.test(tier.replace(/[\s-]/g, ""))) throw new ToolError("Member numbers are added in the app, not here. Nothing was changed.");
    ...
    async add_template_item(client, scope, args) {
      ...
      if (!given(args.list)) {
        ...
        if (!template) throw new ToolError("There is no base packing list yet. Name a template, or start one in the app. Nothing was added.");
      } else {
