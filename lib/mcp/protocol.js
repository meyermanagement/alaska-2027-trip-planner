...
const INSTRUCTIONS =
  "Access to one household's saved travel in Alyeska: trips, one itinerary day at a time with booking advice, packing and day packs, reminders, adult travelers, preferences, budget, wallet balances and card offers, document and pet expiration dates, insurance coverage, deadlines, trip essentials, pro tips, nearby tips, bucket list, fare alerts, pets, favorite moments and past reviews. It can check packing items, day pack items and reminders off or back on, and a primary traveler can add a packing item, a reminder or a bucket-list place; add or change an itinerary item; create a trip or change its name, dates, destination, status or budget; add a Wallet program or update its balance, status or fee; add an item to a packing template; and put a packing-list item in a day pack. Nothing is ever deleted: to drop an itinerary item, set its status to cancelled. It cannot change preferences, expenses, notes or confirmation numbers; point the person to Ask Aly in the app for those. A template change applies to trips built afterward, not existing ones. A day pack item must already be on the trip's packing list. If more than one line matches, it changes nothing and lists them; ask the person which. Adding something already on the list changes nothing. A parent also sees, checks off and adds their children's packing and day pack items; nothing else about a child is shared. Health and allergy details, typed notes, and confirmation, ID, member, policy and microchip numbers are not shared. Some tools are for the household's primary travelers only.";
