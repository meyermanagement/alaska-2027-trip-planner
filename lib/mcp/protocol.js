...
export const SERVER_INFO = { name: "alyeska", title: "Alyeska", version: "0.1.0" };

const INSTRUCTIONS =
  "Read-only access to one household's saved trips in Alyeska: trips, one itinerary day at a time, packing progress, adult travelers, open pro tips and past reviews. Nothing here changes the trip. Children, health details, notes, bookings' confirmation numbers and costs are not shared.";
...
// database.
export async function handleMessage(message, { headers = {}, admin, getScope, log = () => {} }) {
  ...
  switch (method) {
    ...
    case "tools/call": {
      ...
      try {
        ...
        const result = await callTool(admin, scope, name, params?.arguments || {});
