...
export function writeHandlers({ ToolError, pickTrip, tripRef, tripTravelers }) {
  return {
    ...
    async add_packing_item(client, scope, args) {
      ...
      if (readsAsHealth(item, args.category)) throw new ToolError("Health items are added in Alyeska, not through an assistant. Nothing was added.");
    ...
    async add_reminder(client, scope, args) {
      ...
      if (readsAsHealth(title, detail)) throw new ToolError("Health reminders are added in Alyeska, not through an assistant. Nothing was added.");
      const assignee = resolveAssignee(ToolError, fam, await tripTravelers(client, scope, trip), args.traveler);
    ...
    async add_bucket_list_place(client, scope, args) {
      ...
      if (fams.length > 1) throw new ToolError("You belong to more than one household, so this has to be added in Alyeska. Nothing was added.");
      const fam = fams[0];
      const why = clip(args.why, 600);
      if (readsAsHealth(why)) throw new ToolError("Leave health details out of the reason; add them in Alyeska if they matter. Nothing was added.");
      let months;
      if (args.months) {
