...
//   2. The client must be approved: a hand-registered row in
//      assistant_oauth_clients with approved_for_consent, or a self-registered
//      client whose every return address belongs to one trusted assistant
//      (lib/mcp/trust.js), checked on every call.
...
import { resolveClient } from "@/lib/mcp/assistantClients";
import { surfaceFor } from "@/lib/mcp/trust";
...
export async function connectionGrant(client, { id: userId, clientId } = {}) {
  ...
  try {
    const known = await resolveClient(client, clientId);
    if (known.error) return { refused: "unavailable" };
    if (!known.client || known.client.approved_for_consent !== true) return { refused: "unknown-client" };
    ...
    return { ok: true, surface: surfaceFor(known.client) };
  } catch {
