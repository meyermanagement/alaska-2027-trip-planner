...
// a household. Exactly one tool writes: check_off_packing_item flips is_packed
// on one visible packing line, through the signed-in person's own client, so
// row-level security and the secondary check-off guard decide what it may
// touch -- the same rules the Packing screen's checkbox runs under.
//
// Children: a primary traveler (a parent) sees and can check off packing and
// day pack items assigned to their children, because those lines are the
