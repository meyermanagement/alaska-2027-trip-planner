...
// a household. Fourteen tools write, and none of them deletes. Six tick or add
// lines: check_off_packing_item here, and in writeTools.js
// check_off_day_pack_item, complete_reminder, add_packing_item, add_reminder
// and add_bucket_list_place. Eight in planTools.js plan: add and update an
// itinerary item, create and update a trip, add and update a Wallet program,
// add a packing-template item, and put a packing-list line in a day pack. Each
// runs through the signed-in person's own client, so row-level security and
// the secondary guards decide what it may touch -- the same rules the app's
// own checkboxes and forms run under. Adding and changing (everything but a
// check-off) is for primary travelers only.
//
