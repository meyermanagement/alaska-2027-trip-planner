...
async function pickTrip(admin, scope, wanted) {
  const trips = await visibleTrips(admin, scope);
...
async function tripTravelers(admin, scope, trip) {
  const links = await admin.from("trip_travelers").select("traveler_id").eq("trip_id", trip.id);
...
const handlers = {
  ...
  async get_trip(admin, scope, args) {
    const trip = await pickTrip(admin, scope, args.trip);
    const people = await tripTravelers(admin, scope, trip);
    const days = await admin.from("itinerary_items").select("item_date").eq("trip_id", trip.id);
  ...
  async get_itinerary_day(admin, scope, args) {
  ...
  async get_packing_status(admin, scope, args) {
    const trip = await pickTrip(admin, scope, args.trip);
    const fam = familyOf(scope, trip.family_id);
    const { data, error } = await admin
      .from("packing_items")
  ...
  async get_travelers(admin, scope, args) {
    const trip = await pickTrip(admin, scope, args.trip);
    const people = await tripTravelers(admin, scope, trip);
    const names = people.map((p) => p.name);
    return {
  ...
  async get_pro_tips(admin, scope, args) {
  ...
  async get_prior_reviews(admin, scope, args) {
...
export async function callTool(admin, scope, name, args = {}) {
