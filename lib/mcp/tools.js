...
import { pointToAlyeska } from "./nextSteps";
...
export async function callTool(client, scope, name, args = {}) {
  ...
  try {
    return await run(client, scope, args);
  } catch (err) {
    // A refusal names Alyeska and links to it, so the person is never left at a
    // dead end in the chat.
    if (err instanceof ToolError) throw new ToolError(pointToAlyeska(err.message));
    throw err;
  }
}
