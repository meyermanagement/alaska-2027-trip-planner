...
export async function resolveClient(supabase, clientId) {
  ...
  if (row) {
    // A hand-registered client keeps its row's name and purpose, but its
    // addresses still say which assistant it is, which picks the surface.
    const assistant = reg ? assistantForClient(reg.redirect_uris, { manual: true }) : null;
    return {
      client: { ...row, source: "registered", ...(assistant ? { assistant: assistant.key, surface: assistant.surface || "full" } : {}) },
      error: null,
    };
  }
