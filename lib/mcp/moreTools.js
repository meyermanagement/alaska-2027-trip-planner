...
//
//   - Primary travelers only, in one household. The refusal here is the first
//     wall; restrictive secondary-traveler policies on every table these touch
//     are the second (supabase/migrations/20261022_restrictive_secondary_policies.sql).
//   - Nothing is deleted. Retiring a place, turning a fare down and taking it
//     onto a trip are statuses. Taking a template off a trip removes its link
//     row (never a template or a packing line), and only when the template is
//     named in remove; that is the one delete here.
...
import { budgetStep, fareSteps, templatePushStep } from "./nextSteps";
import {
...
export function moreTools(ADD, UPDATE, tripArg) {
  ...
  return [
    ...
    t("set_trip_templates", "Choose a trip's add-on packing templates",
      "Adds add-on packing templates (such as Beach or Cruise) to a trip. Templates already chosen stay unless named in remove. Say none in remove to go back to the base list only. Primary travelers only. It does not change the packing list already on the trip.",
      {
        trip: tripArg,
        templates: s("Template names to add, comma-separated."),
        remove: s("Template names to take off the trip, comma-separated, or none to go back to the base list only. Only when the person asked."),
      },
      null, UPDATE),
    ...
    t("update_packing_item", "Update a packing-list line",
      "Changes one line on a trip's packing list: its name, quantity, who it is for, category, bag, or whether it is packed last minute. Primary travelers only. Notes are changed in Alyeska. To tick it, use check_off_packing_item.",
      {
        trip: tripArg,
    ...
    t("update_reminder", "Update a reminder",
      "Changes one reminder on a trip: its title, who it is for, due date, timing or priority. Its detail is changed in Alyeska. Primary travelers only. To mark it done, use complete_reminder.",
      {
        trip: tripArg,
    ...
    t("set_pet_plan", "Say what a pet is doing for a trip",
      "Saves one pet's arrangement for a trip: coming, boarding, a sitter, or staying with family. Coming adds the pet's packing lines; staying behind sets them aside. Primary travelers only. Taking a pet off a trip is done in Alyeska.",
      {
        trip: tripArg,
    ...
    t("add_preference", "Add a travel preference",
      "Saves one travel preference the person said, for the whole household or named adults, such as Window seats or We would rather walk than taxi. Primary travelers only. Not for children or anything about health, diet or accessibility; those are added in Alyeska.",
      {
...
export function moreHandlers({ ToolError, pickTrip, tripRef, visiblePacking }) {
  ...
  function owners(ToolError, scope, fam, said, verb) {
    ...
    for (const name of list(said)) {
      if (fam.minorNames.has(name) || [...fam.minorNames].some((n) => norm(n) === norm(name))) {
        throw new ToolError(`Preferences about a child are added in Alyeska. Nothing was ${verb}.`);
      }
  ...
  // `{ next_steps }` when a follow-up has something to offer, `{}` otherwise.
  async function steps(...pending) {
    const out = (await Promise.all(pending)).flat().filter(Boolean);
    return out.length ? { next_steps: out } : {};
  }
  ...
  return {
    async start_packing_list(client, scope, args) {
      ...
      if (!outcome.copied) {
        return { summary: `Nothing was added to ${trip.name}: the base packing list is empty or missing. Add items with add_template_item, or start one in Alyeska.`, trip: tripRef(trip), added: 0, changed: false };
      }
      return {
    ...
    async set_trip_templates(client, scope, args) {
      ...
      const saidRemove = String(args.remove || "").trim();
      if (!said && !saidRemove) throw new ToolError("Say which templates to add or remove.");
      const trip = await pickTrip(client, scope, args.trip);
      ...
      const lists = (await read(client.from("packing_templates").select("id, name, is_base").eq("family_id", fam.familyId), "templates"))
        .filter((l) => !l.is_base);
      const NONE = ["none", "base", "base only", "the base list", "no", "all"];
      const pick = (text) => {
        const out = [];
        for (const want of commas(text)) {
      ...
      if (said && NONE.includes(norm(said))) throw new ToolError("To go back to the base list only, say none in remove. Nothing was changed.");
      const addIds = said ? pick(said) : [];
      const before = (await read(client.from("trip_templates").select("template_id").eq("trip_id", trip.id), "templates")).map((r) => r.template_id);
      const clearAll = saidRemove && NONE.includes(norm(saidRemove));
      const removeIds = clearAll ? [...before] : (saidRemove ? pick(saidRemove) : []);
      const clash = addIds.filter((id) => removeIds.includes(id));
      if (clash.length && !clearAll) throw new ToolError(`${clash.map((id) => lists.find((l) => l.id === id).name).join(", ")} was named in both templates and remove. Nothing was changed.`);
      const ids = [...before.filter((id) => !removeIds.includes(id)), ...addIds.filter((id) => !before.includes(id) || removeIds.includes(id))]
        .filter((id, i, a) => a.indexOf(id) === i && lists.some((l) => l.id === id));
      ...
      // Additive by default. A join row goes only when the person named its
      // template in remove (or said none there); the rest are left alone.
      ...
      if (dropping.length) {
        ...
        if (del.error) throw new ToolError(`The templates were only partly changed${adding.length ? "; the new ones were added" : ""}. Try again, or choose them in Alyeska.`);
      }
      ...
      return {
        ...
        templates: names,
        ...(await steps(templatePushStep(client, scope, fam, { tripId: trip.id }))),
        changed: true,
      };
    },
    ...
    async add_trip_cost(client, scope, args) {
      ...
      if (touchesMinor(fam, { texts: [label, note] })) throw new ToolError("Costs that name a child are added in Alyeska. Nothing was added.");
      ...
      return {
        ...
        trip: tripRef(trip),
        cost: costOut(row),
        ...(await steps(budgetStep(client, trip))),
        changed: true,
      };
    },
    ...
    async update_trip_cost(client, scope, args) {
      ...
      if (label && touchesMinor(fam, { texts: [label] })) throw new ToolError("Costs that name a child are changed in Alyeska. Nothing was changed.");
      const patch = {
      ...
      return { summary: `Updated ${row.label} on ${trip.name}'s budget.`, trip: tripRef(trip), before: costOut(row), cost: costOut({ ...row, ...patch }), ...(await steps(budgetStep(client, trip))), changed: true };
    },
    ...
    async update_reminder(client, scope, args) {
      ...
      if (title && touchesMinor(fam, { texts: [title] })) throw new ToolError("Reminders that name a child are changed in Alyeska. Nothing was changed.");
    ...
    async add_favorite_moment(client, scope, args) {
      ...
      if ([...fam.minorNames].some((n) => norm(n) === norm(args.whose))) throw new ToolError("Favorite moments for a child are added in Alyeska. Nothing was added.");
      if (touchesMinor(fam, { texts: [body] })) throw new ToolError("Moments that name a child are added in Alyeska. Nothing was added.");
    ...
    async put_fare_on_trip(client, scope, args) {
      ...
      return { summary: `Put the ${dealLabel(deal)} fare on ${trip.name}. Nothing was booked.`, trip: tripRef(trip), fare: dealLabel(deal), next_steps: fareSteps(deal), changed: true };
    },
    ...
    async set_pet_plan(client, scope, args) {
      ...
      if (!arrangement) throw new ToolError(`arrangement must be one of ${ARRANGEMENT_KEYS.join(", ")}. Taking a pet off a trip is done in Alyeska.`);
    ...
    async add_preference(client, scope, args) {
      ...
      if (touchesMinor(fam, { texts: [body, reason] })) throw new ToolError("Preferences about a child are added in Alyeska. Nothing was added.");
    ...
    async update_preference(client, scope, args) {
      ...
      if (touchesMinor(fam, { texts: [body, reason] })) throw new ToolError("Preferences about a child are changed in Alyeska. Nothing was changed.");
