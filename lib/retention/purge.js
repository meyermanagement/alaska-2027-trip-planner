...
export const JOBS = [INBOX_ORIGINALS, CONVERSATIONS, DIAGNOSTICS];
...
/**
 * Run all three purges, record each, and never let one take the others down.
 *
 * @param {object} input
...
export async function runRetentionPurges({
  ...
  for (const job of jobs) {
    ...
    try {
      await supabase.from("retention_runs").insert(row);
    } catch {
      // The purge itself succeeded; losing the bookkeeping is a gap in the record
      // and not a reason to throw away a completed deletion. Silent on purpose,
      // and the row is still reported to the caller below.
    }
    outcomes.push(row);
  }
