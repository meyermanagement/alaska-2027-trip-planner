...
export function expectBrief({
  ...
  const fareSaid = (fares || [])
    .map((row) => {
      if (!row?.award_pricing && (row?.price == null || !Number.isFinite(Number(row.price)))) return null;
      const bits = [
  ...
  if (fareSaid.length) {
    blocks.push(
      `FARES THEY HAVE ACTUALLY BEEN SENT FOR THIS PLACE\n${fareSaid.join("\n")}\nName the number and who sent it in the cost line. It is one data point beside the range, not the range. Award fares require miles/points AND cash: never treat the fees as a cash ticket, compare them with cash budgets, or invent round-trip miles by doubling a one-way quote.`,
    );
  }
