...
//
// Beside that it scores the fit, and the scoring is the part that had to be
// honest. A model saying a place is an 84 percent match invites the one question
// it cannot answer, which is why not 91, and the first time somebody disagrees
// with the number the whole panel stops being believed. So the number here is
// arithmetic anybody can check: the model names the things it weighed, each one a
// line from the household's own record with a sentence on how this place sits
// against it, and the fraction is simply how many of them line up. The verdict is
// computed from that fraction here rather than asked for, so the words and the
// number can never disagree on screen.
//
...
/** Past this the fraction stops being something a person can read at a glance. */
export const MAX_CHECKS = 7;

/** Under two checks there is no fraction worth showing, only a lonely claim. */
const MIN_CHECKS = 2;
...
Then the fit. List the things in their record that actually bear on this place -- between three and seven of them, the ones that matter most -- and for each say whether this place lines up with it. Use only things that are written in the record below. Never invent a preference, and never list something the record does not mention just to reach seven. "unsure" is a real answer and a better one than a guess.
...
{"cost":"…","like":"…","book":"…","stop":"…","checks":[{"about":"…","match":"yes","because":"…"}]}
  ...
  checks  three to seven things from their record, each with:
...
/**
 * The checks worth showing, and nothing else.
 *
 * A check with no sentence is a label with a machine's word behind it, which is
 * the thing this feature exists to replace, so it is dropped rather than
 * repaired. Two checks about the same line of the record are one check said
 * twice, and the first survives.
 */
export function acceptChecks(candidates = []) {
  ...
  return kept.length >= MIN_CHECKS ? kept : [];
}

/**
 * The verdict, computed rather than asked for.
 *
 * The model is never asked to grade the place, because a grade and a fraction
 * that disagree on the same card is worse than either alone, and the fraction is
 * the one a person can check. A collision counts double against the place: a
 * family whose record says somebody cannot manage the walking does not want to
 * hear that four other things line up.
 */
export function verdictFrom(checks = []) {
  if (!checks.length) return null;
  const yes = checks.filter((one) => one.match === "yes").length;
  const no = checks.filter((one) => one.match === "no").length;
  const share = (yes - no) / checks.length;
  if (share >= 0.6) return "A strong fit";
  if (share >= 0.25) return "Worth a look";
  return "Probably not for you";
}

/** How many of the things we know about them line up, as a sentence. */
export function fitSaid(checks = []) {
  if (!checks.length) return "";
  const yes = checks.filter((one) => one.match === "yes").length;
  return `${yes} of ${checks.length} things you have told me line up`;
}
 ...
 *
 * @returns {{tips: Array, checks: Array, verdict: string|null, fit: string, sources: Array, model: string|null, searched: boolean}}
 */
