...
// either is a question about school, work, heat, crowds and the trips they have
// already committed to. Both halves are needed: the season answer alone is a
// travel article, and the family answer alone cannot tell you it rains.
//
...
// going, their ages, what the trips they already have are doing to the calendar,
// and the months currently ticked. It comes back with at most three windows, each
// with a sentence for why and a note when a window collides with a trip they have
// already booked.
//
...
The second half is this family. Their record is below. What they can stand, what they have said about crowds and money and heat, the hard limits on the people going, the ages of the children, and above all the months they actually travel in -- their past and upcoming trips tell you when this household is able to leave, and that is a stronger signal than anything they have typed. A window that is perfect for the place and impossible for them is the wrong answer.
...
3. Where a window collides with a trip they have already committed to, say so in "clash" rather than dropping the window. They may move it, and hiding the collision means they will not know they had to.
4. Never invent a fact about them. If nothing in the record bears on a window, give the season reason alone and do not dress it up as something you know about them.
5. Where the months they have already ticked are wrong, say so plainly in "note", and give the window you would tick instead. Where their ticks are right, say that too -- an answer that confirms what they already thought is worth having.
6. American spelling. No exclamation marks. Do not address them as "you" in the label; the label is a name for the window, not a sentence.

Reply with JSON and nothing else, in this exact shape:

{"windows":[{"label":"…","months":["Apr","May"],"because":"…","clash":"…"}],"note":"…"}
  ...
  clash    a trip of theirs this window runs into, or omit it entirely
...
export function seasonBrief({
  ...
  const tripSaid = (trips || [])
    .filter((trip) => trip?.name && trip?.start_date)
    .map((trip) => {
      const dates = trip.end_date
        ? `${trip.start_date} to ${trip.end_date}`
        : trip.start_date;
      const status = trip.status ? ` (${trip.status})` : "";
      return `${clip(trip.name, 80)}: ${dates}${status}`;
    })
    .slice(0, 20);
  if (tripSaid.length) {
    blocks.push(
      `TRIPS THEY ALREADY HAVE\n${tripSaid.join("\n")}\nTwo jobs for this block. It tells you which months this household is actually able to travel in, which is the most reliable thing in this brief. And a window that lands on one of these is a clash worth naming.`,
    );
  }
...
export function acceptWindows(candidates = []) {
  ...
  for (const raw of Array.isArray(candidates) ? candidates : []) {
    ...
    kept.push({
      ...
      because,
      clash: clip(raw?.clash, MAX_CLASH) || null,
    });
  }
...
export function reasonFor(chosen) {
  ...
  const clash = chosen.clash ? ` ${chosen.clash}` : "";
  return clip(`${said}: ${chosen.because}${clash}`, 500);
}
