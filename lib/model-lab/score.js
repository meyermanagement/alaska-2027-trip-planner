...
/**
 * Tips: the answer parses, every tip clears the app's own bar (the same
 * acceptTips the database write goes through), the planted fact is reached,
 * and nothing already on the list is suggested again.
 */
export function scoreTips({ candidates, accepted, dropped: thrown = [] }, testCase) {
  // No tips passes "nothing thrown out" and "nothing repeated" trivially.
  if (!Array.isArray(candidates) || !candidates.length) return { score: 0, max: 4, notes: ["no tips came back"] };
  const notes = [];
  let score = 0;
  const point = (ok, note) => {
    if (ok) score++;
  ...
  point(Array.isArray(candidates) && candidates.length > 0, "no tips came back");
  point(thrown.length === 0, `${thrown.length} tip${thrown.length === 1 ? "" : "s"} failed the app's bar${thrown[0] ? `: ${thrown[0].reason}` : ""}`);
  const all = (accepted || []).map((t) => `${t.title} ${t.body} ${t.carry_item || ""}`).join("\n");
  point(testCase.wants.test(all), `missing ${testCase.wants.source.slice(0, 40)}`);
  const repeats = (testCase.already || []).filter((re) => (accepted || []).some((t) => re.test(`${t.title} ${t.carry_item || ""}`)));
  point(!repeats.length, `repeated ${repeats.map((r) => r.source).join(", ")}`);
  return { score, max: 4, notes };
}

/** Wallet tips: parses, at most three, every program_id real, the planted fact reached. */
export function scoreWallet(tips, testCase) {
  const notes = [];
  let score = 0;
  const point = (ok, note) => {
    if (ok) score++;
  ...
  // The Wallet's programs are made up, so a model that follows "if you cannot
  // verify a rule, leave the tip out" to the letter can find nothing to check
  // and say nothing. That is a defensible reading, not an invented one: it
  // keeps the two points for not inventing programs and not over-sending, and
  // loses the two for never reaching the fact the Wallet itself states.
  if (!list.length)
    return testCase.madeUp
      ? { score: 2, max: 4, notes: ["held back: the made-up programs could not be looked up"] }
      : { score: 0, max: 4, notes: ["no tips came back"] };
  point(list.length > 0, "no tips came back");
  point(list.length <= 3, `${list.length} tips, at most three allowed`);
  const bad = list.filter((t) => !testCase.ids.includes(String(t?.program_id || "")));
  point(!bad.length, `program_id not in the Wallet: ${bad.map((t) => t?.program_id).join(", ").slice(0, 40)}`);
  point(list.some((t) => testCase.wants.test(`${t?.title} ${t?.body} ${t?.act_by || ""}`)), `missing ${testCase.wants.source.slice(0, 40)}`);
  return { score, max: 4, notes };
}

/** Menu search: the wanted keys come back first, the unwanted ones do not, nonsense gets nothing. */
export function scoreNav(keys, testCase) {
  const notes = [];
  if (keys == null) return { score: 0, max: 2, notes: ["unparseable"] };
  let score = 0;
  if (testCase.empty) {
    if (!keys.length) score += 2;
    else notes.push(`returned ${keys.join(", ")} for nonsense`);
    return { score, max: 2, notes };
  }
  if (testCase.want.every((k) => keys.slice(0, 3).includes(k))) score++;
  else notes.push(`top three were ${keys.slice(0, 3).join(", ") || "nothing"}`);
  const wrong = (testCase.notWant || []).filter((k) => keys.includes(k));
  if (!wrong.length) score++;
  else notes.push(`included ${wrong.join(", ")}`);
  return { score, max: 2, notes };
}

/** About-you priors: one point per slot settled right, one for guessing nothing else, one for verbatim quotes. */
export function scorePriors(priors, testCase) {
  const notes = [];
  if (!priors || typeof priors !== "object") return { score: 0, max: Object.keys(testCase.truth).length + 2, notes: ["not JSON"] };
  let score = 0;
  for (const [slot, value] of Object.entries(testCase.truth)) {
    if (priors[slot]?.value === value) score++;
    else notes.push(`${slot}=${JSON.stringify(priors[slot]?.value ?? null)}, wanted ${value}`);
  }
  // A slot the case allows (a clear implication the production instructions
  // let through at medium confidence) is not a guess when it has that value.
  const allowed = testCase.allowed || {};
  const extra = Object.keys(priors).filter((k) => !(k in testCase.truth) && priors[k]?.value !== allowed[k]);
  if (!extra.length) score++;
  else notes.push(`guessed ${extra.join(", ")}`);
  const loose = Object.entries(priors).filter(([, v]) => !v?.quote || !testCase.paragraph.includes(String(v.quote).trim()));
  if (!loose.length) score++;
  else notes.push(`${loose.length} quote${loose.length === 1 ? "" : "s"} not word for word`);
  return { score, max: Object.keys(testCase.truth).length + 2, notes };
}
