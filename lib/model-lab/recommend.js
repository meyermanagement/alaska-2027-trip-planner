 ...
 */
/** The 90th-percentile of a list of numbers (nearest rank), or null when empty. */
export function p90(values) {
  const list = values.filter((v) => Number.isFinite(v)).sort((a, b) => a - b);
  if (!list.length) return null;
  return list[Math.min(list.length - 1, Math.ceil(0.9 * list.length) - 1)];
}
...
export function aggregate(results, { overrides = {}, today = new Date() } = {}) {
  ...
  for (const g of groups.values()) {
    ...
    const judged = g.list.filter((r) => !dropped(r) && !r.unsupported);
    ...
    // Repeats of one case: how many cases were asked more than once, and on how
    // many of those the marks disagreed with each other.
    const byCase = new Map();
    for (const r of judged) {
      if (!r.caseId) continue;
      if (!byCase.has(r.caseId)) byCase.set(r.caseId, []);
      byCase.get(r.caseId).push(r.ok && r.max ? r.score / r.max : 0);
    }
    const repeated = [...byCase.values()].filter((l) => l.length > 1);
    rows.push({
      scenario: g.scenario,
      model: g.model,
      effort: g.effort,
      ...
      dropped: g.list.filter((r) => dropped(r)).length,
      // The model refused the thinking level itself: nothing to judge.
      notOffered: g.list.filter((r) => r.unsupported).length,
      ...
      ms90: p90(answered.map((r) => r.ms || 0)),
      repeated: repeated.length,
      varied: repeated.filter((l) => l.some((x) => x !== l[0])).length,
      cost: price && costs.length ? costs.reduce((a, b) => a + b, 0) / costs.length : null,
      priced: Boolean(price),
      notes: [...new Set(g.list.filter((r) => !r.unsupported).flatMap((r) => r.notes || (r.error ? [r.error] : [])))].slice(0, 6),
    });
  }
  return rows;
}
