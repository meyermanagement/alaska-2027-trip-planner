...
export function aggregate(results, { overrides = {}, today = new Date() } = {}) {
  ...
  for (const g of groups.values()) {
    ...
    const shares = g.list.map((r) => (r.ok && r.max ? r.score / r.max : 0));
    ...
    rows.push({
      ...
      effort: g.effort,
      runs: g.list.length,
      failures: g.list.length - answered.length,
      quality: shares.reduce((a, b) => a + b, 0) / shares.length,
...
export function recommend(rows) {
  ...
  for (const scenario of [...new Set(rows.map((r) => r.scenario))]) {
    const mine = rows.filter((r) => r.scenario === scenario && r.failures === 0);
    if (!mine.length) {
      out[scenario] = null;
      continue;
    }
