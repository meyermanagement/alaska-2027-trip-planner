...
// Anthropic refuses an enum beside a list of types ("text or empty"), which
// OpenAI's strict form uses for every optional choice. Say the same thing as
// two alternatives instead: one of the choices, or nothing.
export function claudeSchema(schema) {
  if (!schema || typeof schema !== "object") return schema;
  if (Array.isArray(schema)) return schema.map(claudeSchema);
  const out = {};
  for (const [k, v] of Object.entries(schema)) {
    if (k === "properties") out.properties = Object.fromEntries(Object.entries(v).map(([n, c]) => [n, claudeSchema(c)]));
    else if (k === "items" || k === "anyOf") out[k] = claudeSchema(v);
    else out[k] = v;
  }
  if (Array.isArray(out.type) && Array.isArray(out.enum)) {
    const types = out.type.filter((t) => t !== "null");
    const { type, enum: choices, description, ...rest } = out;
    const alts = [{ ...rest, type: types.length === 1 ? types[0] : types, enum: choices.filter((c) => c !== null) }];
    if (type.includes("null")) alts.push({ type: "null" });
    return description ? { anyOf: alts, description } : { anyOf: alts };
  }
  return out;
}
...
export function anthropicRequest(model, spec, { sampling = true } = {}) {
  ...
  if (json?.schema) {
    body.output_config = { ...(body.output_config || {}), format: { type: "json_schema", schema: claudeSchema(openai.strictSchema(json.schema)) } };
  } else if (json) {
