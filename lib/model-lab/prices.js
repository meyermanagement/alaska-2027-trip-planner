...
// Claude: $10 per 1,000 web searches, pages read billed as input. Writing to
// the prompt cache costs 1.25 times input (5-minute cache); reads are listed
// per model because Opus 5.5, Fable 5.1 and Mythos 5.1 discount them further.
const ANTHROPIC = "https://platform.claude.com/docs/en/about-claude/pricing";
const ANTHROPIC_SEARCH = 0.01;
const claude = (input, cached, output) => ({
  input, cached, output, write: input * 1.25, search: ANTHROPIC_SEARCH,
  source: ANTHROPIC, checked: "2026-09-26",
});
...
export const PRICES = {
  ...
  "claude-haiku-4-5-20251001": claude(1, 0.1, 5),
...
export function costOf(usage, price) {
  ...
  // Tokens written to a prompt cache, where the vendor bills that separately
  // (Claude does, at 1.25 times input). Part of input, not on top of it.
  const wrote = Math.min(Number(usage.cacheWriteTokens) || 0, input - cached);
  const output = Number(usage.outputTokens) || 0;
  const searches = Number(usage.searches) || 0;
  const write = Number.isFinite(price.write) ? price.write : price.input;
  return (
    ((input - cached - wrote) * price.input + wrote * write + cached * price.cached + output * price.output) / 1e6 +
