...
const ANTHROPIC_LIST = "https://api.anthropic.com/v1/models?limit=1000";

// Oldest family worth offering. Anything older is not a candidate for this app.
const MIN_GEMINI = 3;
const MIN_OPENAI = 5.5;
// Claude 4.5 is the oldest generation still sold at a price worth comparing.
const MIN_CLAUDE = 4.5;
...
/**
 * A Claude model id the lab could test, or null. Ids look like claude-sonnet-5,
 * claude-opus-5-5 or claude-haiku-4-5-20251001; a dated id counts as its
 * version, since some models are only listed by date.
 */
export function anthropicCandidate(id) {
  const name = String(id || "");
  const m = name.match(/^claude-(haiku|sonnet|opus|fable|mythos)-(\d+)(?:-(\d))?(-\d{8})?$/);
  if (!m) return null;
  const version = Number(m[3] ? `${m[2]}.${m[3]}` : m[2]);
  if (version < MIN_CLAUDE) return null;
  return { id: name, vendor: "anthropic", version, tier: m[1], preview: false, dated: Boolean(m[4]) };
}

// Newest first, then the cheaper tier first within a version.
const TIER_ORDER = ["flash-lite", "flash", "pro", "luna", "mini", "terra", "base", "sol", "astra", "haiku", "sonnet", "opus", "fable", "mythos"];
...
export function inUse() {
  ...
  return {
    ...
    tips: { models: gemini.modelList(), setting: "GEMINI_MODELS (tips search the web, so they stay on Gemini)" },
    wallet: { models: gemini.modelList(), setting: "GEMINI_MODELS (Wallet tips search the web)" },
    nav: { models: gemini.modelList(), setting: "GEMINI_MODELS" },
    priors: { models: textModels.length ? textModels : ["gemini-3.8-flash", "gemini-3.6-flash", "gemini-3.5-flash-lite"], setting: "GEMINI_TEXT_MODELS, else lib/travelers/extractAboutMePriors.js" },
    long: { models: ask, setting: "ASSISTANT_PROVIDER, OPENAI_MODELS, GEMINI_MODELS" },
  };
}
...
export async function discover() {
  ...
  // Claude is tested here only. The app never sends a family's data to it:
  // lib/agent/llm.js routes to disclosed vendors, and Anthropic is not one.
  if (process.env.ANTHROPIC_API_KEY) {
    jobs.push(
      getJson(ANTHROPIC_LIST, { "x-api-key": process.env.ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01" })
        .then((json) => {
          const rows = (json.data || []).map((m) => anthropicCandidate(m.id)).filter(Boolean);
          // Where a model is listed both by alias and by date, offer the alias.
          const plain = new Set(rows.filter((r) => !r.dated).map((r) => `${r.tier}:${r.version}`));
          for (const r of rows) if (!r.dated || !plain.has(`${r.tier}:${r.version}`)) add(r);
        })
        .catch((e) => out.errors.push({ vendor: "anthropic", message: e.message })),
    );
  } else out.errors.push({ vendor: "anthropic", message: "ANTHROPIC_API_KEY is not set" });
