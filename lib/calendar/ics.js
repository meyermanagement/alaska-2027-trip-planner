...
//
// What goes in it is deliberately less than the app knows. A calendar entry ends
// up on a work laptop, a shared iPad and a phone lock screen, and it is read
// through a URL that cannot ask anybody to sign in. So the feed carries the facts
// a calendar is for -- what, when, where, and a link back -- and none of the
// household's own writing: no note typed onto an itinerary item, no detail typed
// onto a task, and no traveler's name. Whoever wants those opens the trip.
//
  ...
  for (const trip of trips) {
    ...
    events.push(
      eventLines({
        ...
        // The destination as the place rather than as description text: same
        // fact, and it is the field a calendar app will offer to map.
        location: trip.destination || "",
        detail: link(trip) ? `Details in Alyeska: ${link(trip)}` : "",
        url: link(trip),
        zone,
        stamp,
      }),
  ...
  for (const item of itinerary) {
    ...
    events.push(
      eventLines({
        ...
        // Not item.notes. That field is where a family writes the confirmation
        // number, the gate code, who is allergic to what -- the things they wrote
        // into the app on purpose and did not ask us to publish through a
        // token-only URL. The pointer replaces it.
        detail: link(trip) ? `Details in Alyeska: ${link(trip)}` : "",
  ...
  for (const task of tasks) {
    ...
    events.push(
      eventLines({
        ...
        // Neither the typed detail nor the assignee's name. "For: Veda" in a
        // feed anybody holding the address can read is a child's name published
        // by a link, and the task title already says what has to happen.
        detail: link(trip) ? `Details in Alyeska: ${link(trip)}` : "",
        url: link(trip),
        zone,
        stamp,
