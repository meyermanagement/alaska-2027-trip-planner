...
/**
 * What the red number on Reminders counts, and what the page paints red: a task
 * that wants doing about now.
 *
 * High priority alone used to be enough, which is why the badge read eleven on a
 * family whose next departure was ten months off. High priority says how much a
 * task matters, not when; booking a Katmai flight matters enormously and is not
 * this week's problem. So the test is a window as well as a weight -- already
 * past due at any priority, or high priority and inside the next seven days --
 * and the number falls back to zero on its own as dates go by, which a count of
 * everything marked high never did.
 *
 * A high priority task with no workable date at all -- a stage to hit on a trip
 * whose dates are not set yet -- lands outside the window and is not counted. It
 * still sits under "No date yet" on the page, which is the honest place for it:
 * nothing can be said about when it is due until the trip has dates.
 */
export function needsAttention(row, today = todayISO()) {
  const trip = row.trip || row.trips;
  if (!trip || isPastTrip(trip)) return false;
  const bucket = bucketOf(dueInfo(row.task || row, trip, today).date, today);
  if (bucket === "overdue") return true;
  return bucket === "week" && priorityOf(row.task || row) === "high";
}

export function countNeedingAttention(rows, today = todayISO()) {
  return (rows || []).filter((row) => needsAttention(row, today)).length;
}
