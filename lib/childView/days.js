...
// Assignment is enforced by the server projection, never by this date filter.
export function dayPackOnDay(trip, day) {
  if (!day || day === "Unscheduled") return [];
  return (trip.day_pack || []).filter(item => !item.item_date || item.item_date === day);
}
