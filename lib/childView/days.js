import { isPastTrip, isCurrentTrip, lastDayOf } from "@/lib/format";
...
export function childOpeningTab(trip, today) {
  if (!trip || !today || trip.archived_at ||
      ["draft", "complete", "archived", "cancelled", "canceled"].includes(trip.status)) return "itinerary";
  if (isCurrentTrip(trip, today)) return "live";
  const start = new Date(`${trip.start_date}T12:00:00Z`);
  if (!Number.isFinite(+start)) return "itinerary";
  start.setUTCDate(start.getUTCDate() - 1);
  return start.toISOString().slice(0, 10) === today ? "packing" : "itinerary";
}
