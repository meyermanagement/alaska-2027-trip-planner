 ...
 * @param {object} o      { span, key } -- key makes the filter ids unique per card
...
export function contourSvg(at, w, h, land, o = {}) {
  ...
  return [
    `<svg class="trip-contour" viewBox="0 0 ${w} ${h}" width="${w}" height="${h}" preserveAspectRatio="xMidYMid slice" aria-hidden="true" focusable="false">`,
    `<defs>${rings
      .map(
        (wd) =>
    ...
    `<path d="${grat}" fill="none" stroke="var(--map-line)" stroke-width=".5" opacity=".5"/>`,
    ...
    landPath ? `<use href="#l-${key}" fill="var(--map-land)"/>` : "",
    ...
    landPath
      ? rings
          .map(
            (wd, i) =>
              `<use href="#l-${key}" fill="none" stroke="var(--map-line)" stroke-width="1" opacity="${(
                0.16 +
                i * 0.05
              ).toFixed(
                2,
      ...
      : "",
    landPath
      ? `<use href="#l-${key}" fill="none" stroke="var(--map-line)" stroke-width="1.1"/>`
      : "",
    `</svg>`,
  ].join("");
}
