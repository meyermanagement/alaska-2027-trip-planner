...
// Cool only, which is the whole of this list's history. Amber and rose were two
// of the five, and they were why a column of trips read as a muddy set rather
// than as this app: every skin's furniture is cool -- teal accent, glacier and
// plum through the aurora, sand paper -- so a warm ground arrived as maroon and
// olive on a blue-black page, and as dusty pink and tan on a pale one. Amber and
// rose are also the app's warning and money colors, and spending them as
// decoration weakened them where they carry meaning. What is left is the run the
// aurora already makes -- teal, glacier, plum -- plus two mixes along it, which
// is still five values told apart at the size a card is actually drawn.
const TOKENS = [
  // Teal carried towards glacier rather than teal itself, for the same reason
  // the old first ground was a mix: a plate in the accent, under a band in the
  // accent, opened as a screen of one color.
  "color-mix(in srgb, var(--color-teal) 62%, var(--color-glacier))",
  "var(--color-glacier)",
  // The far end of the same run: glacier most of the way to plum.
  "color-mix(in srgb, var(--color-glacier) 60%, var(--color-plum))",
  ...
  "var(--color-plum)",
  // The deep end of teal. Near the accent, but a plate is a ground under a
  // photograph and a scrim, and the wash over a photograph is now light, so this
  // one does not come out as the button beside it.
  "color-mix(in srgb, var(--color-teal) 74%, var(--color-plum))",
];
