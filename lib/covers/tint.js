...
const TOKENS = [
  // Not the accent, which is what this used to be. The accent is the loudest
  // color in the app and the one the trip screen already spends most freely:
  // the band above the header, the active pill, every primary button. A trip
  // whose ground hashed to it opened as a screen of one color -- a picture
  // printed as a duotone in the accent, sitting under a band of the same accent
  // -- and the picture stopped reading as a picture. So the fifth ground is a
  // mix of two accents that is close to neither, and no state color anywhere
  // uses it, which is the whole point of a ground.
  "color-mix(in srgb, var(--color-amber) 52%, var(--color-plum))",
  "var(--color-glacier)",
  "var(--color-amber)",
  "var(--color-rose)",
