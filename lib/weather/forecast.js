...
import { fetchNwsAll, phraseSaid } from "@/lib/weather/nws";
...
export async function fetchForecast(
  lat,
  lon,
  { days = 3, signal, nws = true } = {},
) {
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) return null;
  const [one] = await fetchForecasts([{ lat, lon }], { days, signal, nws });
  return one ?? null;
}
...
function forecastUrl(coords, models, { days, hourly }) {
  return (
    ...
    // No models named means the service picks the best available for each point
    // itself. One model instead of nine, which is the whole point of the fallback.
 ...
 *
 * Asked twice, because the failure that costs a day its weather is not a refusal.
 * The service answers 200 with a plain-text body -- "Unexpected error while
 * streaming data: timeoutReached" -- when its own upstream read runs long, which
 * happens under load and clears on its own within a second. A single attempt
 * turned that into a day with no forecast at all, so a blip upstream read on the
 * screen as a missing feature. One retry after a short pause is enough: the
 * second attempt returns in about a tenth of a second when the first did not.
 *
 * `res.json()` is what throws on that body, so both attempts sit inside the same
 * catch. A genuinely down service costs one extra request and still ends at null,
 * which the whole day view is built to survive.
 */
async function ask(url, signal, tries = 2) {
  for (let n = 0; n < tries; n += 1) {
    try {
      const res = await fetch(url, { signal, next: { revalidate: 1800 } });
      // 4xx is a request this code got wrong and a retry will get wrong again.
      if (!res.ok && res.status < 500) return null;
      if (res.ok) return await res.json();
    } catch (err) {
      // A caller that gave up -- a closed tab, a route past its budget -- should
      // not be retried at.
      if (err?.name === "AbortError") return null;
    }
    if (n < tries - 1) await new Promise((r) => setTimeout(r, 400));
  }
  return null;
}
...
export async function fetchForecasts(
  ...
  // The blend and the American offices are asked at the same time, not one after
  // the other, because the offices are a fan-out of their own and a day view that
  // waited for nine models and then for three requests per point would be waiting
  // twice. `nws: false` is for tests and for anywhere the office should be left
  // out of it.
  const [blended, offices] = await Promise.all([
    ask(forecastUrl(coords, MODELS, { days, hourly: true }), signal),
    nws ? fetchNwsAll(points, { signal }) : Promise.resolve([]),
  ]);

  // Nine models, five hourly variables and several points is a heavy question,
  // and it is the weight that fails: the service answers 200 with a plain-text
  // "timeoutReached" when its own read runs long, and does it for minutes at a
  // stretch rather than for one unlucky request. Retrying the same heavy question
  // just fails again.
  //
  // So the fallback asks a lighter one -- the same fields with no model panel, so
  // the service picks a single best model per point. It costs the agreement
  // between models, which is a real loss: `spread` comes back null and the day
  // stops being able to say the forecasters disagree. But a day band saying 79/61
  // from one model beats a day band saying nothing, and the shaping already
  // reports how many models spoke, so nothing downstream is misled about what it
  // is looking at.
  const raw =
    blended ??
    (await ask(forecastUrl(coords, [], { days, hourly: true }), signal));
  if (!raw) return points.map(() => null);

  const list = Array.isArray(raw) ? raw : [raw];
  return points.map((_, i) => preferNws(shape(list[i]), offices?.[i]) || null);
}
    ...
    return {
      ...
      spread:
        highs.length > 1
          ...
          : null,
      models: highs.length,
      // Where this day's numbers came from. Overwritten by the Weather Service
      // overlay in the United States; everywhere else the blend is the answer.
      source: "models",
    };
  });
...
/**
 * Let the local forecast office have the number.
 *
 * The blend is a median of nine model runs. In the United States a forecaster at
 * the local office reads those same runs, applies what they know about the place,
 * and publishes a high and a low -- the number on weather.gov, on the local news,
 * and in every American weather app. Where that exists it wins, because a family
 * checking their phone against this app should not find two different Thursdays.
 *
 * What the office does not publish, it does not overwrite. Sunrise and sunset stay
 * astronomical. The model spread stays, and stays worth saying: "the models differ
 * by fourteen degrees" describes real uncertainty about Thursday even after a
 * human has picked 93 out of that range. And the count of models stays honest
 * about how many runs were behind the comparison.
 *
 * Days the office does not reach -- day six of a seven-day view, since its hourly
 * grid runs about six and a half days out -- keep the blended numbers, so a week
 * view degrades one day at a time instead of all at once.
 *
 * @param blended  a shaped forecast from this file, or null
 * @param nws      the office's answer from lib/weather/nws.js, or null
 * @returns a shaped forecast; the blended one untouched when there is no overlay
 */
export function preferNws(blended, nws) {
  if (!blended) return blended;
  if (!nws || !Array.isArray(nws.days) || nws.days.length === 0) return blended;

  const office = new Map(nws.days.map((d) => [d.date, d]));
  const days = blended.days.map((day) => {
    const said = office.get(day.date);
    if (!said || !Number.isFinite(said.high)) return day;
    const sky =
      said.code === null || said.code === undefined
        ? { said: day.said, glyph: day.glyph }
        : describeCode(said.code);
    // The office's own words for the day, hedges intact; the glyph still comes
    // from the code, since a picture cannot say "slight chance".
    const words = phraseSaid(said.shortForecast) || sky.said;
    return {
      ...day,
      high: said.high,
      // A first or last day in range can have a high with no matching night, so
      // the blended low is kept rather than blanked.
      low: Number.isFinite(said.low) ? said.low : day.low,
      said: words,
      glyph: sky.glyph,
      rainChance: Number.isFinite(said.rainChance)
        ? said.rainChance
        : day.rainChance,
      source: "nws",
      office: nws.office || null,
    };
  });

  // The hours follow the days. Mixing an office high with model hours is how the
  // day band came to read 94 beside a three o'clock of 95 -- the same mistake in
  // a different coat -- so an hour is only replaced on a day the office spoke
  // about, and only when the office has an hour for it.
  const covered = new Set(
    days.filter((d) => d.source === "nws").map((d) => d.date),
  );
  const byHour = new Map();
  for (const h of nws.hours || []) {
    if (covered.has(h.date)) byHour.set(`${h.date} ${h.hm}`, h);
  }
  const hours = blended.hours.map((h) => {
    const said = byHour.get(`${h.date} ${h.hm}`);
    if (!said) return h;
    const sky =
      said.code === null || said.code === undefined
        ? { said: h.said, glyph: h.glyph }
        : describeCode(said.code);
    return {
      ...h,
      temp: said.temp,
      feels: Number.isFinite(said.feels) ? said.feels : h.feels,
      rainChance: Number.isFinite(said.rainChance)
        ? said.rainChance
        : h.rainChance,
      wind: Number.isFinite(said.wind) ? said.wind : h.wind,
      said: sky.said,
      glyph: sky.glyph,
      source: "nws",
    };
  ...
  return { ...blended, days, hours };
}

/**
 * Who said so, when it is worth saying.
 *
 * "National Weather Service" earns its space: it is why the number changed, it is
 * checkable, and it is the difference between an app quoting a forecast and an app
 * inventing one. The blend gets no label on the day band -- nine model names in a
 * phone-width strip is noise -- and callers that want it can read day.source.
 */
export function sourceSaid(day) {
  if (day?.source !== "nws") return null;
...
export function spreadSaid(day) {
  ...
  // "models", not "forecasts", now that the forecast on the screen can be a
  // forecaster's rather than the models' own middle. What disagrees is the raw
  // model runs behind it, and saying so keeps the sentence true either way.
  return `models differ by ${Math.round(spread)}\u00b0`;
}
