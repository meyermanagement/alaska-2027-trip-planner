 ...
 *
 * Asking Open-Meteo without naming a model gets "best_match", which for a given
 * point is one weather model. On the last Sunday in August it had Thursday in
 * Altoona, Iowa at 102 degrees -- and 106 the Tuesday before, which would be a
 * September record. The European model said 88 for the same Thursday and the
 * German one said 98. The number was not a bug in the arithmetic here; it was one
 * model running hot, printed as though it were the weather.
 *
 * So three global models are asked every time and the middle number is the one
 * shown. A median is the specific defence against this failure: one model can be
 * an outlier by fifteen degrees without moving the answer, where an average would
 * carry a third of that error straight onto the screen. When they disagree badly
 * the day says so, rather than picking a winner quietly.
 *
 * All three are global and free, and they arrive in one request, so this costs a
 * larger response and no extra round trip.
 */
export const MODELS = ["ecmwf_ifs025", "gfs_seamless", "icon_seamless"];
