...
export function asksForPlaces(said) {
  ...
  // A second helping should still offer a third, and this is the exact sentence
  // the button itself sends.
 ...
 *
 * The interesting half of the question is the second sentence. A second helping
 ...
 */
export function findMoreRequest(shown) {
  ...
  const kinds = new Set(
    (Array.isArray(shown) ? shown : []).map((place) => place?.kind),
  );
  const what =
    kinds.size === 1
      ? kinds.has("eat")
  ...
  const not = names.length
    ? ` Not ${names.join(", ")}, or anything else already on screen.`
    : "";
  return `Find me more ${what}.${not} For each new one, say plainly what it gives up compared with the ones you showed me first — what is worse about it, not another reason it is good. And if the first set really was the best of them, say that instead of finding six more.`;
}
