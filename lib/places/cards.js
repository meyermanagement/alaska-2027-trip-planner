...
const KINDS = new Set(["eat", "stay", "do", "car"]);
...
export const KIND_LABELS = {
  ...
  stay: "Place to stay",
  do: "Thing to do",
  // Not a place at all, and shown as a card for the same reason the others are:
  // a rental named in a paragraph is a search the family still has to run.
  car: "Car to rent",
};

/**
 * Where a suggestion is booked, when that is not the place itself.
 *
 * A whole-house rental and a car lent by its owner are the right answer often
 * enough that leaving them out is a gap: three people wanting a kitchen and a
 * second bedroom are better served by a two-bedroom rental than by two hotel
 * rooms, and on an island where every counter is sold out a car from Turo is
 * the difference between a trip with a car and one without. The platform is
 * held here rather than folded into the name, because it changes what the card
 * can honestly say -- there is no front desk, no loyalty program, and the
 * damage cover their cards carry does not follow them onto a stranger's car.
 */
...
// The platform's own hostnames, so a link that claims to be the listing has to
// actually be on the platform it claims.
const PLATFORM_HOSTS = {
  airbnb: /(^|\.)airbnb\.[a-z.]+$/i,
  vrbo: /(^|\.)(vrbo\.[a-z.]+|homeaway\.[a-z.]+)$/i,
  turo: /(^|\.)turo\.com$/i,
};

// Where the platform's own search lives, for when there is no listing link to
// give. A card that cannot link the listing still gets the family to the right
// search on the right site, which beats a dead card by some distance.
const PLATFORM_SEARCH = {
  airbnb: (what) =>
    `https://www.airbnb.com/s/${encodeURIComponent(what)}/homes`,
  vrbo: (what) => `https://www.vrbo.com/search?q=${encodeURIComponent(what)}`,
  turo: (what) =>
    `https://turo.com/us/en/search?location=${encodeURIComponent(what)}`,
};

/** The platform a suggestion is booked through, or null for the place itself. */
export function platform(value) {
  const said = text(value, 40)
    .toLowerCase()
    .replace(/[^a-z]/g, "");
  if (!said) return null;
  if (PLATFORM_LABELS[said]) return said;
  // "Airbnb.com", "VRBO listing", "book on Turo" all arrive sooner or later.
  for (const key of Object.keys(PLATFORM_LABELS))
    if (said.includes(key)) return key;
  return null;
}

/** The platform's own search for where they are going. */
export function platformSearch(via, what) {
  const build = PLATFORM_SEARCH[via];
  const said = text(what, 80);
  if (!build || !said) return null;
  return build(said);
}

function onPlatform(url, via) {
  const pattern = PLATFORM_HOSTS[via];
...
export function normalizePlace(raw) {
  ...
  const via = platform(raw.via);
  // A link that says it is the listing has to be on the platform: a grounded
  // search hands back a redirect through Google as often as the real address,
  // and a "listing" that opens somewhere else is worse than a search would be.
  const said = website(raw.website);
  const direct = via ? onPlatform(said, via) : false;
  const link = via ? (direct ? said : platformSearch(via, area || name)) : said;
  return {
    name,
    ...
    // Whether the link is the listing itself or the platform's search for the
    // area. The button reads differently for each, because "View on Airbnb" on
    // a search result is a promise the link does not keep -- which is exactly
    // what it looked like from the other side.
    listing: via ? direct : null,
    ...
    // Which site it is booked on, when that is not the place's own front desk.
    via,
    website: link,
    // A listing's title is not a place -- "Sunny 2BR walk to Kaanapali" finds
    // nothing on a map -- so the map link goes to the area it is in instead,
    // which is the thing worth looking at anyway. A car has nowhere to map.
    maps:
      kind === "car" ? null : via ? mapsLink(area, "") : mapsLink(name, area),
 ...
 */
// The line over the cards, so the answer and the cards read as two things: what
// Aly said, and then the places she is putting in front of you. It counts and
// names what is there, in the words the cards already use for themselves.
const SHORTLIST_NOUNS = {
  eat: ["place to eat", "places to eat"],
  stay: ["place to stay", "places to stay"],
  do: ["thing to do", "things to do"],
  car: ["car to rent", "cars to rent"],
};

export function shortlistLabel(places) {
  const list = (Array.isArray(places) ? places : []).filter((p) => p?.name);
  if (!list.length) return "";
  const kinds = new Set(list.map((p) => (SHORTLIST_NOUNS[p.kind] ? p.kind : "do")));
  const n = list.length;
  if (kinds.size === 1) {
    const [one, many] = SHORTLIST_NOUNS[[...kinds][0]];
...
export function splitPlaceCalls(calls) {
  ...
  // Two calls in one reply - eating and doing, say - are one shortlist, and
  // their words are read in the order they were written.
  return {
    calls: kept,
...
const PLACE_NOUN =
  /\b(eat|eating|dine|dining|dinner|lunch|breakfast|brunch|food|restaurant|restaurants|cafe|cafes|bar|bars|stay|stays|staying|sleep|hotel|hotels|resort|resorts|lodge|lodges|inn|airbnb|vrbo|turo|rental|rentals|car|cars|to do|to see|worth seeing|visit|attraction|attractions|activity|activities|excursion|excursions|tour|tours|museum|museums|beach|beaches|hike|hikes|park|parks|sight|sights|nearby|around here)\b/i;
...
/**
 * Should a shortlist offer a second helping?
 *
 * This used to try to recognize a request for places from the words typed, and
 * it was wrong far more often than it was right. "What should we do on the
 * Thursday", "snorkeling spots near the resort", "recommend a steakhouse",
 * "anything for kids", "where to snorkel", "ideas for our free day in
 * Anchorage" -- every one of those is a request for a shortlist, every one of
 * them got a shortlist, and not one of them matched, so the two buttons under
 * the newest cards simply were not there. A list of nouns and asking words
 * cannot keep up with the ways a person asks for somewhere to eat.
 *
 * So the question is asked the other way around. Cards on screen are the
 * evidence that a shortlist was wanted: Aly only draws them when she is
 * recommending places. The buttons are offered by default, and withheld only
 * where the question was plainly about the plan rather than about places --
 * what to book first, in what order, how much it has all cost -- because those
 * answer with places named in the prose without being a request for more of
 * them.
 */
export function offersMorePlaces(said) {
  const text = String(said == null ? "" : said).trim();
  // Cards with no question above them -- a conversation read back from the
  // database that opens on an answer -- still get the offer.
  if (!text) return true;
  // The exact sentences the two buttons send always qualify, whatever else is
  // in them: a second helping should still offer a third.
  if (/^find me more\b/i.test(text)) return true;
  if (/^show me the highest regarded\b/i.test(text)) return true;
  return !NOT_A_SHORTLIST.test(text);
}
...
/**
 * What tapping "Best-rated, ignoring our preferences" should ask on their behalf.
 *
  ...
  const what =
    place?.kind === "stay"
      ...
      : place?.kind === "eat"
        ? "as a meal"
        : place?.kind === "car"
          ? "as the car we are renting"
          : "as an activity";
  // Where it is booked belongs in the item, because it is the first thing they
  // will want to know when they go looking for the reservation later.
  const how = PLATFORM_LABELS[place?.via]
    ? ` (booked through ${PLATFORM_LABELS[place.via]})`
    : "";
  return `Add ${name}${where ? ` (${where})` : ""} to the itinerary ${what}${how}. Ask me which day if you are not sure.`;
}
