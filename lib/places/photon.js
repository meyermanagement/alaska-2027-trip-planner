...
/**
 * Places named on a trip's own days, as things to look up.
 *
 * A destination is sometimes a region -- "New England" -- and a region has no
 * point: Photon answers with the town of New England, North Dakota, and answers
 * it convincingly, because the name really is the name. The days know better.
 * They name Portland, Bar Harbor and Edgartown, and a handful of those outvote
 * one wrong town.
 *
 * Two shapes are dropped rather than looked up:
 *   - a journey, "Whittier to Denali" or "STL to PWM", which is two places and
 *     an arrow, and geocodes as neither
 *   - a street address's house number, so "49 Neal Street, Portland, ME" is
 *     asked as its town rather than rejected for not being called Portland
 */
export function itineraryStops(locations = [], max = 6) {
  const out = [];
  for (const raw of locations) {
    ...
    if (/\bto\b/i.test(text)) continue;
    const parts = text.split(",").map((part) => part.trim());
    if (parts.length > 1 && /^\d/.test(parts[0])) parts.shift();
    text = parts.join(", ");
    if (text.length < 3) continue;
    if (out.some((had) => plain(had) === plain(text))) continue;
    out.push(text);
...
/**
 * One point for a whole trip, out of every place it is known to go.
 *
 * Bad lookups are dropped first, then the stop with the least total distance to
 * the others is taken: the middle of the trip rather than the first thing typed,
 * which is what put an Alaska cruise-tour in British Columbia.
 */
export function tripPoint(candidates = []) {
  return biasPoint(withoutOutliers(candidates));
}

/**
 * Whether this write moves the trip somewhere else.
 *
 * Compared after flattening, so fixing a comma or a capital in "Vancouver,
 * Inside Passage" is not treated as a move and costs no lookups. An emptied
 * destination is not a move either: there is nothing new to look up, and the old
 * point is better than none.
 */
export function destinationMoved(trip, patch) {
  if (!patch || !("destination" in patch)) return false;
  const next = plain(patch.destination);
  if (!next) return false;
  return next !== plain(trip?.destination);
}
