...
// What a question about paying for something looks like. "How should I pay for
// the Loews?" came back with four hotel cards under an answer about which card
// earns what: the question named no place worth carding, but the reply said
// "stay", "hotel" and "breakfast", which is what a reply about paying for a hotel
// has to say. The place is already chosen; what they wanted was the card.
const ABOUT_PAYING =
  /\b(pay|paying|paid|payment|payments|charged|credit cards?|debit|(?:which|what|my|our) card|card to use|points(?! of interest)|(?:use|using|with|redeem) (?:my |our )?miles|miles (?:or|vs\.?|versus) cash|redeem\w*|cash back|cashback|amex|mastercard|sapphire|deposit|refund\w*|book(?:ed|ing)? (?:it )?(?:through|with|via|using)|portal)\b/i;

// Words that still ask for somewhere, even alongside a question about paying:
// "Where should we stay, and which card should we put it on?"
const ASKS_FOR_PLACES =
  /\b(where|what to do|things to do|worth doing|regret missing|recommend\w*|suggest\w*|alternatives?|somewhere|anywhere)\b/i;

/** Is this a question about how to pay, rather than about where to go? */
export function asksHowToPay(said) {
  const q = String(said || "");
  return ABOUT_PAYING.test(q) && !ASKS_FOR_PLACES.test(q);
}
 ...
 *
 * All four parts matter. Cards already on screen means there is nothing to fix.
 ...
 * thing to answer with six cards. And a question about paying for a place is not
 * a question about places, however many of them the answer mentions.
 */
export function needsCards(said, text, places = []) {
  ...
  // A question about paying is answered about paying. Its reply names the
  // place it is paying for, and that is not a request for a shortlist.
  if (asksHowToPay(said)) return false;
...
export function showThePlaces(said, text) {
  return [
    ...
    "Call show_places once, now, with the places you just named — the same ones, in the same order, with the same reasoning in each why. Do not write anything else, and keep reply to one short line: your words are already printed above the cards and will not be replaced, so anything longer is ignored.",
...
/**
 * The words above the cards, when show_places brought its own.
 *
 * The first turn used to call show_places and write nothing, and the app paid
 * for a second whole turn to get the words: 14 of 16 questions on Sept 22 did,
 * about four seconds each. show_places now carries the words in reply, so the
 * answer and its cards arrive in one call. Which words win:
 *
 * - No usable reply (empty, or the names read out again): the turn's own text,
 *   exactly as before, and the route's checks decide whether to finish.
 * - The turn's own text is empty or a roll call: the reply.
 * - Both are real: the longer one. The shorter is nearly always a preamble
 *   ("Here are a few picks") or a second copy of the same answer, and printing
 *   both would say it twice.
 */
export function wordsWithCards(text, reply, places = []) {
  const said = String(text || "").trim();
  const written = String(reply || "").trim();
  const list = Array.isArray(places) ? places : [];
  if (!written || !list.length) return said;
  if (needsWords(written, list)) return said;
  if (!said || needsWords(said, list)) return written;
  return words(written).length > words(said).length ? written : said;
}
