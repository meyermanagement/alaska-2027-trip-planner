...
export async function lookUpAddress(q, { signal = null, bias = null } = {}) {
  ...
  if (geo.why === "none") return geo;
  const text = await textSearch(query, key, signal, bias);
  return text.hit ? text : geo;
}
...
export function addressTrouble(why, detail) {
  if (why === "off") {
  ...
  if (why === "denied") {
    return `Google refused the address lookup${detail ? `: ${detail}` : ""}. The key needs the Geocoding API, or the Places API, switched on.`;
  }
  if (why === "slow") {
