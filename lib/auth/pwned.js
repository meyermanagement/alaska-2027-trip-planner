...
export async function leakedPasswordCount(password) {
  ...
  try {
    ...
    text = await res.text();
    // A redirect to a sign-in page comes back as HTML with a 200, and every line
    // of it fails to match a hash suffix — which would read as "this password is
    // clean" rather than "the check did not run". Anything that is not the
    // SUFFIX:COUNT shape is an unknown answer, not a pass.
    if (!/^[0-9A-F]{35}:\d+/m.test(text)) return null;
  } catch {
    return null;
  }
