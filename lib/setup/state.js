...
/**
 * How much of the four-things checklist a household still has ahead of it.
 *
 * The screen at /welcome/next-steps names four things worth doing after the
 * interview -- the rest of the family's own words, the Wallet, forwarding, and
 * the trips already taken -- and until now it was shown once, on a path the
 * owner walks a single time, and then never mentioned again. Nothing carried
 * the ask forward, so the four things it asked for were the four things nobody
 * did. This is what carries them: the menu marks the rows the work is behind
 ...
 *              numbers and insurance, and demanding all four would leave a
 ...
 * load stops at that one row. The six probes below run only while there is
 * something genuinely outstanding, which is a window of days on a new account
 * and never again.
 *
 * The stamp is written from here when the last of the four lands, so the menu
...
export async function loadSetupState(
  ...
  // An invited member is not being asked to do any of this. All four rows are
  ...
  const [people, moments, documents, programs, mail, past] = await Promise.all([
