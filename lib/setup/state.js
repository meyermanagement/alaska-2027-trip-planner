...
export async function loadSetupState(
  ...
  const hasAbout = (p) => String(p.about_me || "").trim().length > 0;
  const described = (p) => hasAbout(p) && withMoment.has(p.id);

  // Who is holding the family row up, and over which half. Without this the
  // row asks for two things and says which of them is missing for nobody, so a
  // household who wrote a paragraph for everyone and no moments reads a lit
  // row as the app having ignored the work they did.
  ...
  const waitingOn = {
    about: others
      ...
      .filter(Boolean),
    moments: others
      .filter((p) => hasAbout(p) && !withMoment.has(p.id))
      .map(name)
      .filter(Boolean),
  };
  ...
  return {
    ...
    // Named only for rows still outstanding: a row with a tick on it has
    // nothing left to be waiting on.
    waitingOn: complete.others ? null : waitingOn,
  };
}
