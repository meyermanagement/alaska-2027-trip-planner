...
export async function parseInboxMessage({ messageId, signal } = {}) {
  ...
  const { data: message, error: readError } = await supabase
    .from("inbox_messages")
    .select(
      "id, family_id, from_email, subject, text_body, html_body, classification, attributed_traveler_id, parse_status",
    )
    .eq("id", messageId)
    .maybeSingle();
  ...
  if (message.parse_status === "running" || message.parse_status === "succeeded") {
    return { ok: false, reason: `already ${message.parse_status}` };
  }
  ...
  return { ok: true, items: staged.length };
}
