...
export const ALY_ABILITIES = [
  ...
  },
  {
    ...
    // is a receipt, and the useful version of this is the one that shapes the
    // draft before anything is booked. Then the running total, which is the
    // mechanical part. Then where to cut, which is the question a family
    // actually asks out loud -- and it is worded as advice about their own trip,
    // not as a saving, a deal or a fare, because advice is the part that is
    // real.
    body: "Tell me what you want to stay under and I build the trip to it -- the same week in Alaska planned for $4,000 and for $6,000 is two different trips, and I would rather say which one you are looking at while it is still a draft. Every line carries a price against one running total, so when the plan changes the number moves the same day instead of after the trip. And when it is over, I say where your family would feel a cut least: the dates either side that cost less, the night in a plainer room because you are out all day anyway, the meal worth keeping.",
...
- Only describe what the given description of that ability actually covers. Do not invent features, integrations, partners, prices, savings, discounts, or deals. Where the description covers advice about a family's own money -- planning to a budget they set, or which part of their own plan to trim -- that advice is yours to give; a fare you would find, a price you would hold, a discount or a deal is not.
