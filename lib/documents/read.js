...
//                                uses.
//
// Both take the kind of document being read -- "identity" for a passport or a
// licence, "policy" for a travel insurance certificate -- which is what picks
// the prompt and the schema on the server. Left at identity when unsaid, because
// that is the reader that existed first and every caller of it means that.
//
...
export async function readStoredFields(
  storagePath,
  { kind = "identity", signal } = {},
) {
  if (!storagePath) throw new Error("Missing document path.");
  return post({ path: storagePath, kind }, signal);
}

export async function readFileFields(file, { kind = "identity", signal } = {}) {
  if (!file) throw new Error("Missing file.");
  const data = await fileToBase64(file);
  return post(
    { data, mime: file.type || "application/octet-stream", kind },
    signal,
  );
}
