...
// text variants -- 3.1 Pro Preview first because personal identity documents
// are the kind of thing where five extra seconds is worth getting the date
// right, and 3.6 Flash as the honest fallback when Pro is busy.
//
...
// shows the model live. The 3.x replacements are pro-preview for the careful
// first pass, flash for the fallback. Override with GEMINI_TEXT_MODELS if a
// later model becomes the workhorse without a code change.
...
const POLICY_PROMPT = `You are reading one travel insurance document — a certificate of insurance, a policy schedule, a confirmation of coverage, or the plan document that came with a trip. It may be a single page or it may be thirty pages long. What matters is on the schedule of benefits and the declarations page; marketing pages, sample claim forms and the state-by-state disclosure pages at the back are not what you are reading for.
...
- kind: "annual" if the document covers every trip inside a plan year, "trip" if it was bought for one journey.
...
If the document is not travel insurance at all — an everyday health plan, an explanation of benefits from a doctor at home, a car insurance card, a booking confirmation — return an empty provider and set confidence to "low".`;
...
const POLICY_SCHEMA = {
  ...
  properties: {
    ...
    kind: { type: "string", enum: ["trip", "annual"] },
...
function normalizePolicyFields(raw) {
  return {
    ...
    kind: pickEnum(raw.kind, ["trip", "annual"]) || "trip",
