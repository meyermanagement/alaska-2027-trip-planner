...
const RULES = [
  ...
  },
  {
    ...
    // The generic words first, then the names of the money itself. The names
    // matter because this rule's own task is titled "Get some Canadian dollar
    // before you leave" and contains not one of the generic words -- so for as
    // long as the predicate was only the first line, the rule could not see its
    // own work and filed another copy on every look. Forty-four of them
    // accumulated on the Alaska trip before anybody counted.
    already: (text, { signals } = {}) => {
      if (
      ...
      const names = (signals?.currencies || []).flatMap((c) =>
        [c?.name, c?.code].filter(Boolean).map((v) => String(v).toLowerCase()),
      );
      // On whole words, because a three-letter code is a substring of ordinary
      // English: "cad" sits inside "decade", and a task about a decade of
      // anything should not silence the money.
...
/** A title reduced to the words in it, for comparing one against another. */
...
export function taskFloor({
  ...
  // Every rule carries a predicate that recognizes its own job, and a predicate
  // written by hand can be wrong -- one of them was, for weeks. So the exact
  // titles already on the trip are also held here, and a row whose title is
  // word-for-word one of them is dropped whatever the predicate said. A rule can
  // still miss a differently worded task and file a second one; it can no longer
  // file the same sentence twice.
  const seen = new Set((tasks || []).map((t) => titleKey(t?.title)));
  const rows = [];
  const fired = [];
  for (const rule of RULES) {
    if (!rule.applies(signals)) continue;
    if (rule.already(text, { signals, facts, trip })) continue;
    const produced = rule
      .rows({ signals, facts, trip })
      .filter(Boolean)
      .filter((row) => !seen.has(titleKey(row.title)));
    if (!produced.length) continue;
    produced.forEach((row) => seen.add(titleKey(row.title)));
    fired.push(rule.id);
    rows.push(...produced);
  }
