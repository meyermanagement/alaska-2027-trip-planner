...
//     would therefore email every single morning forever. A stage speaks on its
//     day only: a stage is a rough intention rather than a promise, and forty
//     "before the trip" tasks nagging daily is how a useful email gets filtered.
 ...
 * @returns {null | {date: string, exact: boolean, note: string|null, late: boolean}}
 */
export function dueOn(task, trip, today) {
  ...
  return info.date === today ? { ...info, exact: false, late: false } : null;
}
...
export function remindersDueToday({
  ...
  // last week outranks the thing you have all day for. Then high priority, then a
  // named date over a stage, then alphabetically so a run reads the same twice.
