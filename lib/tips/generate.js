...
import { memberLines, MEMBER_RULE } from "./members";
...
function factsBrief(trip, itinerary, memberships = [], travelers = []) {
  ...
  return [
    ...
    "",
    "THEIR LOYALTY STANDINGS, levels included:",
    memberLines(memberships, travelers).join("\n"),
    "",
...
function windowsFrom(value) {
  ...
  for (const row of value.slice(0, 10)) {
    ...
    out.push({
      ...
      applies_to_status: str(row.applies_to_status, 60),
