...
// Bumped to 3 when the sheet learned to carry the languages spoken where the trip
// goes. A sheet written before that has no languages at all, and the translation
// tip cannot be honest without them, so the older shape is treated as stale.
export const WINDOWS_VERSION = 3;
...
{"leaves_country":true|false,"countries":["…"],"currencies":[{"code":"…","name":"…"}],"languages":["…"],"mains_voltage":"…","plug_types":["…"],"coverage_note":"…","entry_note":"…","booking_windows":[{"name":"…","applies_to":"…","applies_to_status":"…","opens_days_before":0,"anchor":"trip_start|trip_end|item","opens_time":"…","opens_on":null,"closes_on":null,"note":"…"}]}
  ...
  currencies      the money a visitor actually needs on this trip, most useful first, as ISO code and English name — like [{"code":"ANG","name":"Netherlands Antillean guilder"}]. Include the US dollar only where it is genuinely accepted or is the local currency. Empty array when dollars are all they need for the whole trip. Do not list a currency merely because it is legal tender somewhere nearby.
  languages       the languages actually spoken where this trip goes, most widely spoken first, in English — like ["Papiamentu","Dutch","Spanish"]. Include a language only if a visitor would meet it on signs, menus, or in conversation. ["English"] when that is genuinely all they need.
...
export async function researchFacts({
  ...
  // The slowest thing the app asks for. Measured against the real Disney trip on
  // the model production uses, it takes about forty seconds: twenty-three stops,
  // twenty loyalty programs, and a booking window to work out for each thing that
  // has one. So it is asked in two stages, the same way tips are — most of the
  // time to the searching pass, and enough held back for one unsearched answer, so
  // a slow search costs the family verified dates rather than the whole look.
  const searchShare = 0.75;
  ...
  const askFor = (grounded, until) =>
    callModel({
      ...
      // connection is cut, and a cut connection is not an error the app can
      // explain — the browser only knows the load failed.
  ...
  const list = (value, max) =>
    Array.isArray(value)
      ? value
          ...
          .slice(0, max)
      : [];
  // Sorted, and sorted here rather than where they are read.
  //
  // A rule builds its title out of these — "Hair dryer will not survive Curaçao
  // and United States on an adapter" — and a tip's identity is its title. This
  // list is composed by a model, which is asked for every country the trip
  // touches and not for an order, so the same two countries came back as
  // ["Curaçao", "United States"] one week and ["United States", "Curaçao"] the
  // next. Each flip rewrote the title, which rewrote the fingerprint, which
  // filed the same advice a second time and let a cleared copy back onto the
  // screen. Sorting at the point of storage means the phrase is a function of
  // which countries they are and nothing else.
  //
  // Locale-aware, so ç sorts with c rather than after z.
  const sorted = (value, max) =>
    list(value, max).sort((a, b) => a.localeCompare(b));
  return {
    facts: {
      leaves_country:
        typeof json.leaves_country === "boolean" ? json.leaves_country : null,
      countries: sorted(json.countries, 12),
      currencies: currencyRows(json.currencies),
      languages: sorted(json.languages, 6),
...
/**
 * Currencies, cleaned before they are stored. A pre-departure task is generated
 * off these — "get some Netherlands Antillean guilder before you leave" — so a
 * mangled row becomes a wrong instruction about money. Anything without a name
 * is dropped rather than patched up. Null is preserved as null, because "never
 * researched" and "needs nothing but dollars" are different answers and the task
 * floor treats them differently.
 */
function currencyRows(value) {
  ...
  for (const row of value.slice(0, 6)) {
    ...
    out.push({ code: /^[A-Z]{3}$/.test(code) ? code : "", name: name || code });
  }
  return out;
}
 ...
 * @param {string[]} input.subjects titles of every prior tip in this place
 *                                  (whatever their status), matched against
 *                                  subject-word overlap so a rephrased tip
 *                                  cannot resurrect a cleared one
...
export async function tipsForPlace({
  ...
  //
  // Raised from 0.7, from the other side: what is held back only has to cover an
  // unsearched answer, and those were measured at 4.5 to 17.7 seconds. A quarter
  // of the budget is 22 seconds, which covers all of them with room to spare, and
  // the seconds that buys on the other side matter more — the grounded tail runs
  // to 65 and 74 seconds, and 0.7 stopped just short of the 65.
  const searchShare = 0.75;
  const left = () => (deadline ? deadline - Date.now() : Infinity);
  const askFor = (grounded, until) =>
    callModel({
