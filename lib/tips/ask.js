...
const SCOPES = new Set(["trip", "packing", "item"]);
...
export function foundLine(found, look = null) {
  ...
  const where = look?.scope === "packing" ? "the packing list" : "this";
...
export function stepsFor(look, itinerary = [], today = "") {
  ...
  const upcoming = (itinerary || [])
    .filter((row) => row?.id && row?.item_date && (!today || row.item_date >= today))
