...
export async function runLook({
  ...
  // after the bare word Looking and is not worth them after a sentence: on a
  // 360-point phone the subject line plus the long count runs to three lines in
  // a bubble that also carries the clock, so a line meant to reassure becomes
  // the biggest thing on the screen. With a subject in front of it the number
  // has enough context to stand on its own.
  const counted = (done) =>
    lead === "Looking"
