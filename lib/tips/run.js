...
// The Wallet's buttons now run these questions separately. The same loop reports
// progress and leaves whatever was found saved if somebody closes the screen.

import { WALLET_SCOPES } from "./tip";

/**
 * What a look started by a button is about, in the places it is about to walk.
 *
 * A look off a question can name the subject the family raised. A button has no
 * question behind it, but it does know where it is going before it sets off, so
 * it can still say something truer than the word Looking: the places are decided
 * up front and cannot be contradicted by what the look comes back with. Names of
 * places, not counts, because the count is already said beside it.
 *
 * There is deliberately no line for a day here. A day pack is not somewhere a
 * look can be aimed: the refresh route accepts trip, packing and item and coerces
 * anything else to trip, the model's own find_tips offers the same three plus the
 * Wallet, and daypack is applied afterwards by lib/tips/tip.js when the model
 * hands back a for_date. So a step never carries a day, and a line about one
 * would be describing a look that cannot be started and could not name the date
 * if it were.
 *
 * @param {Array<{scope?: string}>} steps
 * @returns {string}
 */
export function lookOpening(steps) {
  const list = Array.isArray(steps) ? steps : [];
  if (!list.length) return "Looking…";
  const scopes = list.map((step) => step?.scope || "trip");
  const has = (name) => scopes.includes(name);
  const wallet = scopes.every((scope) => WALLET_SCOPES.includes(scope));
  if (wallet) {
    return has("offers") && has("wallet")
      ? "Looking at the cards you hold and the ones you do not."
      : has("offers")
        ? "Looking at what other cards are offering."
        : "Looking at the programs you already hold.";
  }
  // One place, said as that place. The trip look is the common chain and gets a
  // sentence naming the three kinds of place it walks rather than listing five.
  if (list.length === 1) {
    if (has("packing")) return "Going through what is already in the bag.";
    if (has("item")) return "Looking at this booking.";
    return "Looking at the trip itself.";
  }
  const parts = [];
  if (has("trip")) parts.push("the trip");
  if (has("packing")) parts.push("the bag");
  if (has("item")) parts.push("what is booked");
  if (!parts.length) return "Looking…";
  const said =
    parts.length === 1
      ? parts[0]
      : `${parts.slice(0, -1).join(", ")} and ${parts[parts.length - 1]}`;
  return `Looking at ${said}.`;
}
...
const MAX_ROUNDS = 3;
// How many places are asked about at the same time, once the first has gone
// alone. A trip look has four places behind the first -- the packing list and
// the next three bookings -- and at three abreast the fourth waited for a whole
// second wave. Four abreast is one wave. Every one of them is a grounded model
// call, so this is not raised further: the first place still goes alone, and a
// burst refusal is waited out once by the provider rather than retried blind.
const AT_ONCE = 4;
 ...
 * @param {string} [input.opening]  what the look is about, in the caller's words
...
export async function runLook({
  ...
  steps,
  onNote = () => {},
  // What this look is about, said in the words of whoever started it. A look run
  // off a question in the panel opens with the subject the family raised -- see
  // openingLine in lib/agent/waiting.js -- so the wait reads as being about their
  // question rather than about the software. A look started by a button has no
  // question behind it and keeps the plain word.
  opening = "Looking…",
  ...
  // The opening without its final stop or ellipsis, so a count can be joined to
  // it without a full stop landing in the middle of the line.
  const lead = String(opening || "Looking…").replace(/[.\u2026]+$/, "");
  // "5 checks done" spells out what the number means, which is worth three words
  // after the bare word Looking -- all that is left of it now is a look with no
  // places to name -- and is not worth them after a sentence: on a 360-point
  // phone the subject line plus the long count runs to three lines in a bubble
  // that also carries the clock, so a line meant to reassure becomes the biggest
  // thing on the screen. With a subject in front of it the number has enough
  // context to stand on its own.
  const counted = (done) =>
    lead === "Looking"
      ? `${lead}\u2026 ${done} of ${plan.length} checks done`
      : `${lead} \u2014 ${done} of ${plan.length}`;

  const say = () => {
    const done = Math.min(ran, plan.length);
    onProgress({ done, total: plan.length });
    if (plan.length < 2) return onNote(`${lead}\u2026`);
    ...
    // five places to look.
    onNote(done >= plan.length ? "Nearly there…" : counted(done));
  };
  say();
...
const WHERE = {
  ...
  // Filed onto one day rather than onto a screen. It is read in two places -- in
  // the day itself on the Itinerary, and as an index on Packing -- and Packing is
  // where somebody goes to prepare all of them the night before, so that is where
  // this sends them.
  daypack: { label: "the day packs", tab: "packing", order: 3 },
    ...
    try {
      const res = await fetchImpl(
        wallet ? "/api/tips/wallet" : "/api/tips/refresh",
        {
          ...
          body: JSON.stringify(
            wallet
              ? { scope: step.scope, ...(step.automatic === true ? { automatic: true } : {}) }
              : {
                  tripId,
                  scope: step.scope,
