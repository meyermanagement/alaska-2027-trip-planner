...
export function bookingWindowTips({ trip, facts, itinerary = [], today }) {
  ...
  for (const window of windows.slice(0, 8)) {
    ...
    if (open) {
      ...
      bodyBits.push(
        `Booking opens ${opensOn}${at ? ` at ${at}` : ""}${tier ? ` for ${tier}` : ""}${anchorDate ? `, counted back from ${anchorDate}` : ""}.`,
      );
    }
    if (tier) {
      bodyBits.push(
    ...
    out.push(
      stamp({
        ...
        because: tier
          ...
          : `${applies || name} is on this trip, and the window is counted from ${anchorDate || opensOn} rather than from today.`,
