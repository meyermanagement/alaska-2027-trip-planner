...
import { coverageAgainstTrip, travelersMissingFrom } from "../insurance/policy";
import { isComing } from "../pets/pets";
...
/**
 * Two things the record can say about an insurance policy without judgement.
 *
 * The first is arithmetic: a policy whose window does not reach the whole trip.
 * A single-trip policy bought before the dates moved, or an annual plan whose
 * year ends mid-holiday, both look fine on the Insurance tab -- there is a
 * policy, it has dates, nothing is blank -- and the only way to notice is to
 * compare two windows, which is what this does.
 *
 * The second is a list comparison: somebody on the trip who is not named on the
 * policy. Names are read off the document or ticked by hand, so an empty list is
 * not a claim that nobody is covered, and this stays quiet in that case rather
 * than telling a family with an unread policy that none of them are insured.
 *
 * Neither is a claim about whether a claim would be paid. Both are the record
 * disagreeing with itself, said out loud with a date to act on.
 */
export function insuranceTips({
  trip,
  policies = [],
  insuredByPolicy = new Map(),
  travelers = [],
  today,
}) {
  const start = iso(trip?.start_date);
  const end = iso(trip?.end_date) || start;
  const now = iso(today);
  // Nothing to do about a policy for a trip that is over.
  if (!start || (now && end && end < now)) return [];
  if (!policies.length) return [];

  const goingIds = (travelers || [])
    .filter((p) => p && p.is_person !== false && p.id)
    .map((p) => p.id);
  const nameOf = new Map(
    (travelers || []).map((p) => [p?.id, String(p?.name || "").trim()]),
  );

  const out = [];
  for (const policy of policies) {
    // Provider and plan, not the policy number: this goes in a title somebody
    // reads at a glance, and the number is the part they look up rather than
    // recognize.
    const named =
      [policy?.provider, policy?.plan_name]
        ...
        .filter(Boolean)
        .join(" ") || "the policy";
    const window = coverageAgainstTrip(policy, trip);

    if (window?.status === "gap") {
      const from = iso(policy?.coverage_start);
      const to = iso(policy?.coverage_end);
      const early = from && start < from;
      const said = early
        ? `The trip starts ${day(start)} and this policy does not begin until ${day(from)}.`
        : `The trip runs to ${day(end)} and this policy stops ${day(to)}.`;
      out.push(
        stamp({
          family_id: trip?.family_id,
          trip_id: trip?.id,
          itinerary_item_id: null,
          scope: "trip",
          title: `${named} does not cover the whole trip`.slice(0, 90),
          body: `${said} Ask the insurer to move the dates — most will while the trip is still ahead — or find out in writing what the policy says about the ${early ? "days before it starts" : "days after it ends"}. A delayed flight home is exactly the day this matters on.`.slice(
            0,
            560,
          ),
          because: said.slice(0, 200),
          urgency: "now",
          act_by: start,
        }),
      );
    ...
    const missing = travelersMissingFrom({
      insuredIds: insuredByPolicy.get(policy.id) || [],
      goingIds,
    });
    if (missing.length) {
      const who = niceList(missing.map((id) => nameOf.get(id)).filter(Boolean));
      if (who) {
        out.push(
          stamp({
            family_id: trip?.family_id,
            ...
            scope: "trip",
            title:
              `${who} ${missing.length === 1 ? "is" : "are"} not named on ${named}`.slice(
                0,
                90,
              ),
            body: `${who} ${missing.length === 1 ? "is" : "are"} on this trip and not named on the policy. Either the policy needs them added — most insurers will, for a premium, up to the day of departure — or they are named on it and nobody has ticked them on the Insurance tab. Worth knowing which before you leave rather than at a clinic.`.slice(
              0,
              560,
            ),
            because: `Named on ${named}: ${
              (insuredByPolicy.get(policy.id) || [])
...
/**
 * An animal is coming, and somewhere on this trip is a place to sleep.
 *
 * Advice rather than a contradiction, and the line between the two is worth
 * stating because this rule was nearly written as one. Whether a particular hotel
 * takes a particular animal is a question with an answer, and it is not an answer
 * anybody in this codebase has: it varies by property inside the same chain, by
 * room type, by weight, by season, and it is often not on the page. So this file
 * cannot say the plan is impossible. What it can say is that three lodging
 * bookings and an animal means three phone calls nobody has made yet, and that is
 * exactly the kind of thing a tip is for -- it can be argued with, waved off for
 * the barn that already knows the horse, and cleared.
 *
 * The naming matters more here than in most of these. "Check pet policies" is
 * advice anybody could have written; naming Valverde Lisboa and the two others is
 * a fact about this trip, and it is also the list to work down.
 */
export function petLodgingTips({
  trip,
  itinerary = [],
  pets = [],
  petLinks = [],
  ...
  const leave = iso(trip?.start_date);
  if (!leave || (today && leave < today)) return [];

  // Coming, not merely written down. An animal boarding at home is the reason
  // this list exists to be filtered.
  const coming = new Set(
    (petLinks || [])
      .filter((link) => isComing(String(link?.arrangement || "").trim()))
      .map((link) => String(link?.pet_id || "").trim())
      .filter(Boolean),
  );
  const animals = (pets || []).filter((pet) =>
    coming.has(String(pet?.id || "").trim()),
  );
  if (!animals.length) return [];

  const stays = linesOfKind(itinerary, ["lodging"]);
  if (!stays.length) return [];

  const who = niceList(animals.map((pet) => pet.name).filter(Boolean));
  const named = niceList(stays);
  const many = stays.length > 1;
  const plural = animals.length > 1;
  ...
  return [
    stamp({
      ...
      scope: "trip",
      title:
        `Ask ${many ? "each place" : "the place you are staying"} whether ${who} can stay`.slice(
          0,
          90,
        ),
      body: `${named} ${many ? "each set their own" : "sets its own"} rule about animals, and a chain name is no guide to it — two properties under the same brand often answer differently, and the answer can turn on weight, room type or the time of year. Ask ${many ? "each of them" : "the property"} directly rather than reading the website: whether ${they} ${are} taken at all, what ${they} cost${plural ? "" : "s"} a night, whether ${they} can be left in the room alone, and where on the grounds ${they} ${are} allowed. A “no” you find today is a booking you can still change; a “no” at the desk is not.`.slice(
        0,
        560,
      ),
      because:
        `${who} ${plural ? "are" : "is"} coming on this trip, and ${many ? `there are ${stays.length} places booked to sleep` : "there is a place booked to sleep"}.`.slice(
          0,
          200,
        ),
      urgency: "soon",
