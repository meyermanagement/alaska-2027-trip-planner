...
export function parsePlanImpacts(result, items, trip, window, now = new Date()) {
  ...
  for (const finding of parsed.impacts) {
    ...
    if (!item || item.status === "cancelled" || item.item_date < window.today || item.item_date > window.through) continue;
    ...
    if (!Number.isFinite(reported) || reported > now.getTime() + 5 * 60000 ||
        ...
        finding.applies_on !== item.item_date || title.length < 6 || detail.length < 20 || action.length < 10) continue;
    ...
    const fingerprint = `operational:${trip.id}:${item.id}:${item.item_date}:${finding.kind}:${finding.effect}${band}`;
    if (seen.has(fingerprint)) continue;
    seen.add(fingerprint);
    impacts.push({
      family_id: trip.family_id, trip_id: trip.id, itinerary_item_id: item.id,
      scope: "item", title, body: `${detail} ${action}`, because: `Affects ${item.title} on ${item.item_date}.`,
      urgency: finding.effect === "blocked" || item.item_date === window.today ? "now" : "soon",
      act_by: item.item_date, for_date: null, carry_item: null,
...
export function conditionsBrief(trip, items, window, timeZone, now = new Date()) {
  return JSON.stringify({
    ...
    items: items.map(i => ({
      id: i.id, date: i.item_date, start_time: i.start_time, end_time: i.end_time,
