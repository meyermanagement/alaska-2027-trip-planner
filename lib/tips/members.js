...
/**
 * Does this text name the company these words describe?
 *
 * The same reading the trip matching uses — the company's own name, a property or
 * ship that belongs to it, or a flight code that is it, with one slipped letter
 * forgiven. Exported because the booking windows need it too: a window that says
 * "Castaway Club members get earlier access" has to be recognized as Disney Cruise
 * Line's before a level can be attached to it.
 */
export function mentionsOperator(text, ...names) {
  const tokens = brandTokens(...names);
  if (!tokens.length) return false;
  return namedIn(String(text || "").toLowerCase(), tokens, familiesFor(tokens));
}
...
export function memberLines(rows = [], travelers = [], bucket = "applies") {
  ...
  return (rows || []).slice(0, 30).map((row) => {
    ...
    bits.push(
      tier
        ? `level: ${tier}`
        : "member, no level recorded — assume no status, whatever else this line says",
    );
    bits.push(whoOf(row, byId));
    // The program's ladder is not a level they hold. It used to arrive in the
    // perks field, which read like one.
    const perks = clip(withoutTierLadder(row.perks), 200);
