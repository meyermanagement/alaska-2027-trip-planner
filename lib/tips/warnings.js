...
export function passportWarnings({ trips = [], documents = [], today }) {
  ...
  for (const trip of trips) {
    ...
    for (const person of trip.going || []) {
      ...
      // The id travels with the name so the Family screen can show one person
      // their own paperwork rather than everybody's.
      if (standing === "expired")
        expired.push({ id: person.id, name: person.name, expiry });
      else if (standing === "short")
        short.push({ id: person.id, name: person.name, expiry });
      else if (standing === "missing")
        missing.push({ id: person.id, name: person.name });
    }
 ...
 */
// Same reason as in rules.js: a warning is a sentence, so its dates are said the
...
export function headlineFor({
  ...
  if (expired.length) {
    ...
    return `${who} passport expires before you fly home from ${trip.name} on ${day(back)}.`;
  }
  if (short.length) {
    const who = niceList(short.map((p) => `${p.name}'s (${day(p.expiry)})`));
    return `${who} passport is valid for ${trip.name} but expires before ${day(mustLastUntil)} — six months past your ${day(back)} return, which is the date the border checks.`;
  }
