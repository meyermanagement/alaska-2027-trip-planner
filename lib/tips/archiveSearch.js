...
// Conservative regular plurals, not substring matching ("fan" != "fantastic").
...
export function archiveFilter(terms, tripIds = []) {
  ...
  const variants = [...new Set(safe.flatMap(term => [term, term.split(" ").map(wordRoot).join(" ")]))];
  const parts = variants.flatMap(term => ["title", "body", "because", "about"].map(field => `${field}.ilike.%${term}%`));
...
export function directMatches(rows, terms) {
  ...
  const tokens = text => String(text || "").normalize("NFKD").replace(/\p{M}/gu, "")
    .toLowerCase().split(/[^a-z0-9]+/).filter(Boolean).map(wordRoot);
...
export function selectedMatches(text, rows) {
  ...
  const seen = new Set();
  const matches = result.matches.slice(0, 30).flatMap(match => {
    if (!known.has(match?.id) || seen.has(match.id)) return [];
    seen.add(match.id);
    return [{ ...known.get(match.id), matchReason: String(match.reason || "").slice(0, 240) }];
  });
  // A nonempty response with no usable IDs is a broken response, not evidence
  // that the archive contains no matches.
  return result.matches.length && !matches.length ? null : matches;
}
