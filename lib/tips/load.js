...
import { visibleHeaderTips } from "./update";
...
export async function loadHeaderNotices(supabase, today) {
  ...
  // The tips need nothing from the trips, so they are asked for first and
  // collected at the end rather than waiting behind the trips and passports.
  // A query builder sends nothing until it is awaited; `.then` sends it now.
  const tipsRead = supabase
    .from("pro_tips")
  ...
  const { data: tips } = await tipsRead;
  ...
  return {
    ...
    urgent: bannerTips(visibleHeaderTips(tips || []), today),
    current,
    upcoming,
  };
}
