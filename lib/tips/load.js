...
import { isCurrentTrip, lastDayOf } from "../format";

/**
 * @param {object} supabase  a server client for the signed-in visitor
 * @param {string} today     ISO date
 * @returns {{warnings: Array, urgent: Array, current: object|null}}
 */
export async function loadHeaderNotices(supabase, today) {
  ...
  const { data: trips } = await supabase
    .from("trips")
    .select(
      "id, name, slug, destination, start_date, end_date, status, cover_emoji, family_id, trip_facts (leaves_country, countries), trip_travelers (travelers (id, name, is_person))",
    )
