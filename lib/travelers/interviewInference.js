...
import { questionFor, optionForAnswer, isInterviewBaseRow } from "./interview";
...
/**
 * How a slot's answer reads back inside "You said ...".
 *
 * The abstract sentence where an option's label is a picture of the preference
 * rather than the preference: an explanation that quoted the card back -- "You
 * said nine, before anybody else is up" -- handed somebody a claim about an
 * hour when what they answered was a question about crowds, and that
 * explanation is saved as the reason on the derived preference row, so the
 * picture outlived the screen it was drawn for. Labels with no sentence already
 * say the preference plainly and read back as they are.
 */
function labelFor(slot, value) {
  const question = questionFor(slot);
  const opt = (question?.options || []).find((o) => o.value === value);
  if (opt?.sentence) return stripPeriod(opt.sentence);
...
const RULES = [
  ...
  // room as the thing worth paying up for, which is the whole of The room.
  {
    ...
    whyFragments: [
      "paying up for on the room",
      "pay more to stay in a walkable location",
    ],
  },
  ...
  // the other direction: a different reason for it, the same place to spend.
  {
    ...
    whyFragments: [
      "paying up for on the room",
      "pay more for accommodation with parking included",
    ],
  },
...
export function inferAnswer(slot, prior) {
  ...
  // A willingness to spend on something is not a ranking of it above every
  // other priority. Budget predictions always remain guesses to confirm,
  // including when an older rule called a shared reason "sure".
  const firing = firingRules(slot, answers).map((rule) =>
    question.kind === "rank" ? { ...rule, strength: "likely" } : rule,
  );
...
export function priorAnswersFrom({ slots, preferences } = {}) {
  ...
  for (const row of slots || []) {
    ...
    const opt = optionForAnswer(question, labelForRules);
  ...
  for (const row of preferences || []) {
    ...
    if (!body) continue;
    const question = questionFor(slot);
    if (isInterviewBaseRow(question, body)) continue;
