...
export function inferAnswer(slot, prior) {
  ...
  // about what comes first and says nothing about the order below it.
...
export function priorAnswersFrom({ slots, preferences } = {}) {
  ...
  for (const row of slots || []) {
    ...
    const firstLabel =
      question.kind === "rank" ? note.split(", then ")[0].trim() : note;
    const opt = (question.options || []).find((o) => o.label === firstLabel);
