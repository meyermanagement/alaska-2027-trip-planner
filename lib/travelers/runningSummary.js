...
export function summaryForAnswer(answer, destination) {
  ...
  // instead of a shape: "I won't book anything before 7:30 am" is a promise
  // they can check, where "you're a late family" was a label they never chose.
  if (answer.kind === "band" && typeof answer.picked === "string") {
    ...
    if (band) {
      ...
      return lateEnd
        ? `Your day runs ${start} to ${end}. On ${dest}, I won't book anything before ${start}, and I'll leave the evening long enough for dinner and whatever follows it.`
        : `Your day runs ${start} to ${end}. On ${dest}, I won't book anything before ${start}, and I'll aim dinner to land inside those hours.`;
    }
  }
