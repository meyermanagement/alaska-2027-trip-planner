...
// ---------------------------------------------------------------------------
// The same answer, on the animal and on the trip.
//
// A plan filed only as a household rule is a sentence nobody's code reads. The
// two places that change what the app actually does are the animal's own record
// -- which decides whether a flight or a haul rig is even a question -- and the
// per-trip arrangement, which is what the packing list and the day plan follow.
// Both are filled from the interview answer, and both are filled only as far as
// the answer honestly goes.
// ---------------------------------------------------------------------------

/**
 * The animal's usual way of travelling, worked out from its plan and what kind
 * of animal it is, or null when the answer does not say.
 *
 * "Stays home" is the one case the plan settles outright. "Comes with us" does
 * not say HOW: a dog might fly in the cabin, fly as cargo or only ever ride in
 * the car, and picking one would be inventing a fact the family would then have
 * to find and correct. The exception is an animal whose kind leaves only one
 * way to travel -- a horse goes by trailer, a rabbit goes by car -- where the
 * plan and the species together do settle it.
 */
export function travelStyleForPlan(plan, styles) {
  const list = (Array.isArray(styles) ? styles : [])
    .map((s) => (typeof s === "string" ? s : s?.id))
    .filter(Boolean);
  if (plan === "home") return list.includes("stays_home") ? "stays_home" : null;
  if (plan !== "comes") return null;
  const traveling = list.filter((id) => id !== "stays_home");
  return traveling.length === 1 ? traveling[0] : null;
}

// Who looks after an animal that stays home, read out of the family's own
// words. Boarding first because a kennel is the most specific thing said, and
// somebody who wrote "the kennel takes them both" has said more than "somebody
// has them".
const CARE_WORDS = [
  ["boarding", /\b(kennel\w*|catter\w*|board(?:s|ed|ing)?)\b/i],
  ["sitter", /\b(sitter\w*|pet ?sit\w*|house ?sit\w*|dog ?walker)\b/i],
  [
    "family",
    /\b(family|friend\w*|grandma|grandpa|granny|parents|mom|mum|dad|in-?laws|neighbou?r\w*)\b/i,
  ],
];

/**
 * What to record for one trip, or null to record nothing.
 *
 * "Comes with us" is a decision about every trip, so it files as coming. A plan
 * that leaves the animal at home settles that it is not travelling but not who
 * has it, so it files only when the family's own words name the arrangement --
 * otherwise nothing is written and the app goes on asking, which is the truth.
 * "Depends on the trip" is the family telling us to ask per trip, so it never
 * files an answer to a question they said they would answer later.
 */
export function arrangementForPlan(plan, words) {
  if (plan === "comes") return "coming";
  if (plan !== "home") return null;
  const text = String(words || "");
  if (!text.trim()) return null;
  for (const [arrangement, re] of CARE_WORDS) {
    if (re.test(text)) return arrangement;
  }
  return null;
}
