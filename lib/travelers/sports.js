 ...
 *     home_lat and home_lon at welcome time. That address is the source of
 *     truth for where home is, and resolveHomePoint in lib/places/homePoint
 *     turns it into the coordinate this file wants: the stored pair when the
 *     row has one, the typed words sent to the geocoder when it does not, and
 *     only failing both of those the coarse position of the request itself.
...
const TEAM_METROS = [
  ...
  },
  {
    ...
    lat: 32.7767,
    lon: -96.797,
    teams: ["the Cowboys", "the Texas Rangers", "the Mavericks", "the Stars"],
  },
  {
    name: "Houston",
    lat: 29.7604,
  ...
  },
  {
    ...
    lat: 38.9072,
    lon: -77.0369,
    teams: ["the Commanders", "the Nationals", "the Wizards", "the Capitals"],
  },
  {
    name: "Miami",
    lat: 25.7617,
    lon: -80.1918,
    teams: ["the Dolphins", "the Marlins", "the Heat", "the Florida Panthers"],
  },
  {
    name: "Philadelphia",
    lat: 39.9526,
  ...
  },
  {
    ...
    lat: 33.4484,
    lon: -112.074,
    teams: ["the Arizona Cardinals", "the Diamondbacks", "the Suns"],
  },
  {
    name: "Detroit",
    lat: 42.3314,
  ...
  },
  {
    ...
    lat: 44.9778,
    lon: -93.265,
    teams: ["the Vikings", "the Twins", "the Timberwolves", "the Wild"],
  },
  {
    name: "San Diego",
    lat: 32.7157,
  ...
  },
  {
    ...
    lat: 39.7392,
    lon: -104.9903,
    teams: ["the Broncos", "the Rockies", "the Nuggets", "the Avalanche"],
  },
  {
    name: "St. Louis",
    lat: 38.627,
  ...
  },
  {
    ...
    lat: 43.6532,
    lon: -79.3832,
    teams: ["the Maple Leafs", "the Blue Jays", "the Raptors"],
  },
  {
    name: "Montreal",
    lat: 45.5017,
...
/**
 * Aliases for places that belong to a metro in the table under a different
 * name. Not exhaustive and not meant to be: the point is that somebody who
 * typed the suburb or the other half of a twin city still gets their own
 * teams rather than a generic list. A place that matches nothing falls back
 * to the general sports categories, which is the honest answer.
 */
const METRO_ALIASES = {
  brooklyn: "New York",
  queens: "New York",
...
/**
 * Match a typed place to a metro in the table, by name.
 *
 * The coordinate path above is the one the real screen uses, because a family
 * has a geocoded home and a coordinate answers "which metro" better than any
 * string can. This path exists for the rehearsal, where the only thing that
 * has been said about home is the words somebody typed a screen ago and there
 * is no row to read and no geocoder in front of it. So: longest metro name
 * first, then the alias table, and null when the words match nothing -- which
 * gives the general sports list rather than somebody else's city's teams.
 */
export function metroFromPlaceWords(place) {
  const text = String(place || "").toLowerCase();
  if (!text.trim()) return null;

  const named = [...TEAM_METROS]
    .sort((a, b) => b.name.length - a.name.length)
    .find((metro) => text.includes(metro.name.toLowerCase()));
  if (named) return named;

  const alias = Object.keys(METRO_ALIASES)
    .sort((a, b) => b.length - a.length)
    .find((key) => text.includes(key));
  if (!alias) return null;
  return TEAM_METROS.find((m) => m.name === METRO_ALIASES[alias]) || null;
}

/**
 * The chip list for a place that was typed rather than geocoded. Same output
 * as buildSportsChipItems, and the same general list when the words are empty
 * or match no metro in the table.
 */
export function sportsChipItemsForPlaceWords(place, maxItems = 12) {
  const metro = metroFromPlaceWords(place);
  if (!metro) return GENERAL_SPORTS.slice(0, maxItems);
  return buildSportsChipItems(metro.lat, metro.lon, maxItems);
}
 ...
 *
 * The two arguments are a coordinate that has already been resolved, from
 * resolveHomePoint in lib/places/homePoint. Do not read families.home_lat and
 * home_lon straight into here: a family can have typed a good address that the
 * geocoder could not place, and reading the columns alone gives them the
 * general list as though they had never said where they lived.
 */
