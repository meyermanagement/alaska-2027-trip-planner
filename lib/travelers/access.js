...
export async function resolveAccess(supabase, user) {
  ...
  // Both reads at once. The traveler row used to wait for the household to be
  // known so it could filter on it, which put a second round trip in front of
  // every screen. Asking for this person's traveler rows in every household
  // and keeping the one in the chosen household gives the same answer in one.
  const [{ data: memberships }, { data: seats }] = await Promise.all([
    supabase
      .from("family_members")
      // Only the id. The household's name and invite code used to be read here
      // and handed back untouched to twenty-seven callers, none of which ever
      // looked at them -- two columns carried on every page load for nobody.
      .select("family_id")
      .eq("user_id", user.id),
    supabase
      .from("travelers")
      .select("id, name, access_level, is_person, family_id")
      .eq("user_id", user.id)
      .eq("is_person", true)
      .order("sort_order"),
  ]);
  ...
  const traveler =
    (seats || []).find((row) => row.family_id === familyId) || null;
  const level = traveler?.access_level === SECONDARY ? SECONDARY : PRIMARY;

  return {
    user,
    familyId,
    // Every household this person belongs to, for the screens that read across
    // all of them and used to fetch the memberships a second time to know.
    familyIds: memberships.map((row) => row.family_id),
    traveler,
