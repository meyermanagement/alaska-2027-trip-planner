...
/**
 * The carriers we know how to spell, in one spelling each.
 *
 * Not a closed list and not a picker: people are still free to type whatever
 * their provider is called. This exists so that the same provider typed three
 * ways is stored one way, because a roaming rule groups travelers by carrier and
 * writes the group into a tip title. "T-mobile" on one person and "TMobile" on
 * another read as two providers, so the title said "Sort T-mobile and TMobile
 * out" one week and "Sort T-Mobile out" the next, and the tip a family had
 * cleared came back under the new wording.
 */
...
/** Case, spaces and punctuation thrown away, so "AT&T" and "at and t" agree. */
const carrierKey = (text) =>
...
/**
 * The shorter names people actually say, mapped onto the full one.
 *
 * Deliberately short. Every entry has to be a name for exactly one provider --
 * "Metro" is Metro by T-Mobile and nothing else -- because folding two real
 * providers together would put the wrong advice on somebody's phone. Sprint is
 * kept as Sprint rather than rewritten to T-Mobile: the networks merged, but
 * somebody who says Sprint is telling us what is printed on their bill.
 */
const CARRIER_ALIASES = new Map([
  // "AT&T" reduces to atandt, so the way most people type it needs saying.
  ["att", "AT&T"],
  ["atnt", "AT&T"],
  ["tmo", "T-Mobile"],
  ["tmobileus", "T-Mobile"],
  ...
  ["straighttalkwireless", "Straight Talk"],
  ["totalwirelessbyverizon", "Total Wireless"],
  ["totalbyverizon", "Total Wireless"],
  ["h2o", "H2O Wireless"],
  ["lyca", "Lycamobile"],
  ["threeuk", "Three"],
  ["o2uk", "O2"],
  ...
  ["uscc", "UScellular"],
  ["uscellularwireless", "UScellular"],
]);

const CARRIER_BY_KEY = new Map([
  ...KNOWN_CARRIERS.map((name) => [carrierKey(name), name]),
  ...CARRIER_ALIASES,
]);

/**
 * One spelling of a provider, or the name as it was given.
 *
 * Matching ignores case, spaces, hyphens and ampersands, so "T-mobile",
 * "TMobile" and "t mobile" are all T-Mobile. A provider we do not know comes
 * back trimmed and capped and otherwise exactly as typed, because there are
 * thousands of small carriers and guessing at their capitalization would be
...
export function normalizeCarrier(raw) {
  const text = String(raw == null ? "" : raw)
    ...
    .trim();
  if (!text) return null;
  return CARRIER_BY_KEY.get(carrierKey(text)) || text.slice(0, 60);
}
...
export function carrierGroups(travelers) {
  ...
  for (const t of travelers || []) {
    if (!t || t.is_person === false) continue;
    // Normalized here as well as at the save, so a row written before the app
    // started normalizing still groups with the same provider spelled properly.
    const carrier = normalizeCarrier(t.phone_carrier);
    if (!carrier) continue;
...
export function profileSentence(person) {
  ...
  const carrier = normalizeCarrier(person.phone_carrier) || "";
...
// The five long example paragraphs that used to live here, and the block that
// rendered them at the bottom of About you, are gone. They were written to
// prove there was no right answer, and instead they were the longest thing on
// a screen whose whole argument is that four short boxes are easier than one
// big one. Anybody reading five polished paragraphs before typing has been
// handed a standard to meet. The four placeholders carry the same job in a
// tenth of the words, and the chip drawers show range by being tappable
...
/**
 * The five short prompts that make up the About-you screen.
 *
 * These are the only entry points now. The old freeform textarea was honest but
 * hard \u2014 people stared at nine rows and wrote two sentences. The five prompts
 ...
 * just written in five bites instead of one paragraph.
 *
 * The questions are about the person, not about a trip. An earlier version asked
 * what somebody was like "on a trip" and got answers shaped like an itinerary,
 * which is the one thing Aly can already see. What she cannot see is the
 * standard somebody holds a hotel to, whether they drink, whether they only book
 * places other people rated highly, whether they are up at five or up at ten.
 *
 * The order matters. What you love doing first, because it is the easiest one to
 * answer. What you are interested in second: subjects, places and teams, which
 * are likes rather than a state of mind \u2014 an earlier version asked what somebody
 * was into "right now" and dated an answer that is not usually temporary.
 * Particular third: that is where cleanliness, review ratings, drinking and the
 * rest of somebody's non-negotiables live, and it is the question that changes
 * the most recommendations. Skip fourth, because dislikes are constraints Aly
 * weighs early. What you are like last, for temperament, health, work, faith and
 * family.
 */
export const ABOUT_ME_MICRO_PROMPTS = [
  // `question` is displayed in the UI; `label` remains the stable storage
  // heading so revised wording never reassigns an existing answer.
  {
    key: "love",
    label: "What you love doing",
    question: "What do you enjoy doing?",
    placeholder: "For example: I enjoy hiking, cooking, and seeing live music.",
  },
  {
    key: "into",
    label: "What you're interested in",
    question: "Which subjects, places, or teams interest you?",
    placeholder:
      "For example: I love architecture, mountain towns, and following baseball.",
  },
  {
    key: "standards",
    label: "What you're particular about",
    question: "What matters to your comfort when you travel?",
    placeholder:
      "For example: I need a quiet room, prefer an aisle seat, and like alcohol-free options.",
  },
  {
    key: "skip",
    ...
    placeholder:
      "For example: I'd rather avoid crowded places and overnight flights. Group tours are fine if they're small.",
  },
  {
    key: "else",
    label: "What you're like",
    question: "What else should Aly know about you?",
    placeholder:
      "For example: I like slow mornings and having a plan. I need breaks during long walks.",
  },
];

/**
 * The five prompts, stitched back into one paragraph for storage.
 *
 * Aly reads about_me as one block of text, so the boxes serialize with their
 * labels inline. The label plus colon prefix is on purpose \u2014 it tells a model
 * that "cruises" under "What you'd rather skip:" is a constraint, not a
...
/**
 * Older labels the same five boxes have shipped under, so a paragraph written
 * before a heading was reworded still loads back into the box it came from
 * rather than falling through to the last one.
 */
const ABOUT_ME_LEGACY_LABELS = {
  "What you're into right now": "into",
  "What you love to do": "love",
  "Anything else worth knowing": "else",
  "What you'd rather skip on a trip": "skip",
};

/**
 * One stored paragraph, read back into the five boxes it was written in.
 *
 * The inverse of aboutMeFromParts, and the piece that lets any screen edit this
 * paragraph rather than only write it for the first time. Blocks are separated
 * by a blank line and start with a heading and a colon; a heading we recognise
 * decides the box, and everything else \u2014 a paragraph Aly wrote out of the
 * interview, or free text somebody typed years ago before the headings existed
 * \u2014 lands whole in the last box. Nothing is dropped, which matters more here
 * than a tidy parse: somebody opening this to fix one word should never find a
 * sentence of theirs missing.
 */
export function splitAboutMe(text) {
  const parts = Object.fromEntries(
    ABOUT_ME_MICRO_PROMPTS.map((p) => [p.key, ""]),
  );
  const raw = String(text || "").trim();
  if (!raw) return parts;

  // Headings are matched on a flattened form of themselves: lowercased, curly
  // apostrophes folded to straight ones. A paragraph that has been through a
  // text editor, a mail client or a model comes back with typographic quotes,
  // and a heading that fails to match costs somebody a section.
  const flatten = (value) =>
    String(value || "")
      .replace(/[\u2018\u2019]/g, "'")
      .trim()
  ...
  ABOUT_ME_MICRO_PROMPTS.forEach((p) => byLabel.set(flatten(p.label), p.key));
  Object.entries(ABOUT_ME_LEGACY_LABELS).forEach(([label, key]) =>
    byLabel.set(flatten(label), key),
  );

  const lastKey = ABOUT_ME_MICRO_PROMPTS[ABOUT_ME_MICRO_PROMPTS.length - 1].key;
  const append = (key, value) => {
    const clean = String(value || "").trim();
    if (!clean) return;
    parts[key] = parts[key] ? `${parts[key]} ${clean}` : clean;
  };
  ...
  raw
    ...
    .forEach((block) => {
      ...
      if (at > 0) {
        const key = byLabel.get(flatten(block.slice(0, at)));
        if (key) {
          append(key, block.slice(at + 1));
          return;
        }
      }
      append(lastKey, block);
    });
...
export const ABOUT_ME_CHIP_GROUPS = [
  ...
  },
  {
    ...
    items: [
      "don't drink at all",
      "drink socially",
      "like a good wine list",
      "like craft beer",
      "prefer alcohol-free options",
      "want somewhere with a bar",
    ],
  },
  {
    key: "skip",
  ...
  {
    key: "temperament",
    label: "Routine and personality",
    prefix: "I'm",
    target: "else",
    items: [
      "an early riser",
      "a night owl",
      "happiest with a plan",
      "happier improvising",
      "quiet in a crowd",
      "the one who talks to everyone",
      "slow in the mornings",
      "happiest staying active",
      "sensitive to cold",
      "sensitive to heat",
    ],
  },
...
export const ABOUT_ME_PLACEHOLDER =
  "What do you enjoy doing? Which subjects or places interest you? What helps you feel comfortable, and what would you rather avoid? A few words are enough. Share only what you're comfortable sharing.";
