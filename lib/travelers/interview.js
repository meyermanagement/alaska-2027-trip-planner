...
//
// An option's label and detail are illustrations. "A kayak on the water" is a
// picture of a preference, not the preference -- the question is asking whether
// somebody would rather do a thing or look at a thing, and the kayak is there
// because an abstract axis is hard to answer and a kayak is easy. The trouble
// was that the label was also what got saved: picking it filed "Doing
// something, or looking at something: A kayak on the water" as a preference, so
// a family who has never been in a kayak came out of the interview on record as
// wanting one. Mark reported it.
//
// So an option that illustrates carries `sentence`, the same answer said in the
// abstract, and that is what is written to the travel file. The label stays on
// the screen where it is doing its job, and the kayak never leaves it.
