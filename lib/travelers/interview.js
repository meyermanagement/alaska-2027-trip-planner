...
export function rankSentence(question, values) {
  ...
  const ranked = rankLabels(question, values);
...
/**
 * The same order, written for the screen rather than for the travel file --
 * "The room, then the meals". Used by the running summary and the practice
 * recap, both of which want the order back in the primary's own labels
 * without the sentence around it.
 */
export function rankLabels(question, values) {
  const options = question?.options || [];
  const out = [];
