...
import { SLOT_BY_ID, slotFromWords } from "./slots";
...
export function rehearsal(
  ...
  const state = {
    ...
    // What Aly has told this person's page about themselves and the moments she
    // has caught so far. Reflected the moment they arrive, so the very next turn
    // sees pass three shorten by whatever the paragraph and the moments cover.
    moments: moments.map((row) => ({ ...row })),
  ...
  function apply(call) {
    ...
    if (FILE_WRITERS.has(call?.name)) {
      ...
      if (call.name === "add_person" && a.name) {
        state.people.push({
          id: `rehearsal-${stamp}-p${n}`,
          name: String(a.name).trim().slice(0, 60),
          is_person: true,
          sort_order: state.people.length + 1,
        });
      }
      // A paragraph about themselves is what the ledger reads to retire blanks
      // in their own words. Threaded back into state.people right away so pass
      // three shortens on the next turn rather than at the end of the run.
      if (
        call.name === "set_person_details" &&
        typeof a.about_me === "string" &&
        a.about_me.trim()
      ) {
        const who = state.people.find(
          (p) =>
            p?.name &&
            a.whose &&
            p.name.toLowerCase() === a.whose.toLowerCase(),
        );
        if (who) who.about_me = a.about_me;
      }
      // A moment Aly caught belongs on state.moments right away, for the same
      // reason as the paragraph above: the very next turn's ledger reads it.
      if (call.name === "add_favorite_moment" && a.body && a.whose) {
        const who = state.people.find(
          (p) => p?.name && p.name.toLowerCase() === a.whose.toLowerCase(),
        );
        if (who) {
          state.moments.push({
            id: `rehearsal-${stamp}-m${n}`,
            traveler_id: who.id,
            body: String(a.body).trim().slice(0, 1200),
            sort_order: state.moments.length + 1,
          });
        }
      }
      if (call.name === "add_pet" && a.name) {
        state.pets.push({
          id: `rehearsal-${stamp}-a${n}`,
          name: String(a.name).trim().slice(0, 60),
  ...
  function standing() {
    ...
    return {
      ...
      skipped: by("skipped"),
      open: by("open"),
      // Blanks their own writing already answers, which the interview skips.
      told: by("told"),
    };
  }
