 ...
 * a row they cannot find is worse than a row taking up space. So the day view
 * folds a past item down to a single line -- its time, its icon, its title -- and
 * opens it again on a tap. It keeps its place in the day rather than moving to
 * the bottom or disappearing, and it still prints in full, because a printed
 * itinerary is read after the fact.
 *
 * These classes are what an opened one wears: flatter border, lighter card, no
 * emphasis. Nothing here shrinks the text itself; unreadably small is not the
