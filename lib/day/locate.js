...
// A route phrase names both ends of a journey: "Altoona, IA to St. Louis", or
// with an arrow, "Denver -> Boulder". The item pins on the destination, not the
// origin, because the whole reason the family typed a route into a single item
// is that the drive is one line on the day and the useful point on the map is
// where they are heading. Photon does not care which half wins, but the day
// view does: measured from the origin, the leg is zero long, and measured from
// the previous item to the origin it is measuring to the wrong town.
const ROUTE_PHRASE = /^\s*(.+?)\s+(?:to|->|\u2192|\u2794|\u21D2)\s+(.+?)\s*$/i;
function routeDestination(text) {
  const m = String(text || "").match(ROUTE_PHRASE);
  if (!m) return null;
  const dest = m[2].trim();
  return dest.length >= 2 ? dest : null;
}

// Whether an item is the family coming back to their own driveway. The word
// "home" in either field is enough when the household has a home address on
// file: an item labeled "Drive home", regardless of what the location field
// says, is a journey that ends at the house.
const HOME_WORD = /\bhome\b/i;
function looksLikeHome(item, house) {
  if (!house?.address) return false;
  if (!Number.isFinite(house.lat) || !Number.isFinite(house.lon)) return false;
  if (sameAddress(house.address, item?.location)) return true;
  if (HOME_WORD.test(item?.location || "")) return true;
  if (HOME_WORD.test(item?.title || "")) return true;
  return false;
}
...
export function geoQuery(item, destination = "") {
  ...
  // A route phrase in either field pins to the destination. "Altoona, IA to
  // St. Louis" belongs on St. Louis; "Drive Home to St. Louis" belongs on
  // St. Louis (or on Home, higher up, when that is what the family means).
  const routed = routeDestination(loc) || routeDestination(title);
  const base = routed || loc || title;
  if (!base) return null;
...
export async function locateItems(
  ...
  for (const item of todo.slice(0, max)) {
    ...
    // The word "home" in the title or the location counts too, so "Drive home"
    // pins on the driveway rather than on whatever city the geocoder guessed
    // from the words on either side of it.
    if (house && looksLikeHome(item, house)) {
