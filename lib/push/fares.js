...
export function fareArrivalPayload({ messageId, count }) {
  return {
    title: count === 1 ? "A new fare came in" : `${count} new fares came in`,
    body: "Your forwarded fares are ready to review.",
    url: "/someday#fares",
...
export async function pushFareArrival({
  supabase, familyId, messageId, count,
  ...
  for (const subscription of eligible) {
    ...
    let result;
    try {
      result = await deliver({ subscription, payload: fareArrivalPayload({ messageId, count }) });
    } catch {
